...
6. 返款账单详情的订单返款数据行须保留业务订单号、`应付返款`、货款原始币种、各指定扣减费项和最终账单计算金额，不展示尾程运单号、来源金额、到付金额或到付附加费总额。包裹级回款记录作为订单费用报表导入的核对事实，在返款账单详情的`汇兑损益`页签中按包裹逐行展示，展示规则见[COD 订单回款记录与汇兑损益怎样展示？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-remittance-712a4d3e)；本节只定义来源字段含义，不重复定义展示区域字段。
