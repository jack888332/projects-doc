...
2. `实回货款`和`回款汇率`来自订单费用报表导入，直接挂靠对应尾程包裹，继续作为回款链路的金额和汇率事实。`ofp_ofdb1.sale_order_package_fee`下的金额均为尾程包裹级导入值，不是OIS机算值；包裹导入手续费与订单级机算手续费必须分别保留形成方式和挂靠对象，不能互相覆盖或冒充。二者表示同一笔业务费用时，仍只允许形成一笔返款扣减。
...
| 读取金额及来源事实 | 按业务订单读取到付总额、应付返款、代收货款手续费、到付附加费和指定扣减费项；同时保留各尾程包裹的回款、来源实返货款、回款汇率和来源返款汇率，具体规则见<a href="#section-11-2-1">11.2.1 返款金额从哪里取？</a>。 | 得到订单级返款金额和可追溯的包裹级回款事实。 |
| 计算返款金额 | 按<a href="#section-11-2-3">11.2.3 到付金额、代收货款和实际返款如何区分？</a>，以业务订单为单位依次计算代收货款本金、扣减前返款金额和最终实付返款。 | 形成订单级返款结果和账单汇总金额。 |
...
返款相关金额同时存在于OIS输出的订单级机算结果和订单费用报表导入形成的包裹级金额中。OIS机算形成的`应付返款`及其手续费直接下挂业务订单，BMS只读取结果；`ofp_ofdb1.sale_order_package_fee`中的所有金额均为尾程包裹级导入值，不是OIS机算值。包裹侧字段记录实收回款、来源实返货款、导入手续费和来源汇率等事实。系统必须保留两类数据各自的形成方式和挂靠对象，不得因字段名称或金额相同而互相覆盖。
...
| `ofp_ofdb1.sale_order_package_fee.receivable_collection_amount` | 包裹侧导入的代收货款手续费 | 直接挂靠对应尾程包裹，是导入费项而非机算结果；命中返款账单指定扣减规则时可作为扣减费项，但不得与同一笔订单级机算手续费重复入账。 |
...
4. 同一笔手续费同时存在订单级机算结果和包裹级导入值时，只允许形成一笔返款扣减；系统必须保留实际采用值的形成方式、来源字段和挂靠对象，并阻止重复归集，不得按字段名称直接覆盖或累加。
