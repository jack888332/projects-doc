...
<a id="doc-45DB5A5118-writeoff-sync-correction"></a>
### 1.4 核销同步与纠错

收款核销和付款核销提交后，系统自动将核销结果同步至金蝶。“提交”和“保存并提交”均适用本规则；核销单状态与金蝶同步结果分别记录。

已同步的核销记录发现错误时，先在金蝶中人工删除原核销记录，并清理对应的原同步关联记录，再由有打回权限的人员在本系统执行打回。打回后，出纳可继续维护或删除该核销记录；继续维护后可重新提交。

打回的操作资格、状态变化与通知见[付款核销打回](#doc-45DB5A5118-section-04227ab78bed)和[收款核销打回](#doc-45DB5A5118-section-fb0ff2d54db5)；收款核销打回对客户额度的影响由后者定义。

**已同步核销记录纠错**

```plantuml
@startuml goldjet-writeoff-synced-correction
...
本图只适用于已同步且需要纠错的核销记录；金蝶删除与同步关联清理的责任分工未明确，因此不指定其执行角色泳道。末端表示重新发起同步，不代表金蝶处理必然成功；同步失败、重试及重复提交的处置尚待明确。
...
管理员按[财务结算操作权限](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第067篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-finance-actions)在付款核销列表页面点击“打回”按钮时，系统打开“打回确认”对话框。只有状态为“已提交”的付款核销显示“打回”按钮。

已同步记录的外部纠错与打回顺序见[核销同步与纠错](#doc-45DB5A5118-writeoff-sync-correction)。

**操作与反馈**

- 确认：完成打回，返回“付款核销列表”界面，该付款核销单的状态变为“已打回”。

付款核销打回的通知规则见[付款核销打回通知规则](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第069篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-732bf646708d)。
...
- “已提交”仍可由管理员打回，不是不可逆终态。收款核销完成后的客户额度释放见[应收结算流程](PRD.高捷物流系统.第048篇.财务基础与结算流程.40E841D3A6.md#doc-40E841D3A6-section-23c66131aa44)；打回时的额度扣减见[收款核销打回](#doc-45DB5A5118-section-fb0ff2d54db5)，该额度变化不套用于付款核销。
...
管理员按[财务结算操作权限](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第067篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-finance-actions)在收款核销列表页面点击“打回”按钮时，系统打开“打回确认”对话框。只有状态为“已提交”的收款核销显示“打回”按钮。打回时扣减客户本次额度，客户额度可能变为负数；业务员录入单据时若发现客户额度为负，财务应完成核销提交。
...
收款核销打回的通知规则见[收款核销打回通知规则](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第069篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-754a95745c41)。
