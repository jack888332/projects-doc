...
CREATE TABLE `bill_config` (
  ...
  `billing_currency` varchar(16) NOT NULL COMMENT '默认结算币种',
...
CREATE TABLE `bill_config_fee_currency_rule` (
  ...
  `charge_currency_mode` varchar(32) NOT NULL DEFAULT 'CONFIG_DEFAULT' COMMENT '结算币种模式：CONFIG_DEFAULT默认结算币种，SOURCE来源币种，FIXED固定结算币种',
...
CREATE TABLE `main_order` (
  ...
  `bill_currency` varchar(16) DEFAULT NULL COMMENT '归属结算币种',
...
CREATE TABLE `ar_bill` (
  ...
  `bill_currency` varchar(16) NOT NULL COMMENT '默认结算币种',
  `fin_currency` varchar(16) NOT NULL DEFAULT 'CNY' COMMENT '财务本位币',
  `initial_receivable_amount` decimal(18,4) NOT NULL DEFAULT '0.0000' COMMENT '初始应收金额<结算币种>',
  `this_adjustment_delta_amount` decimal(18,4) NOT NULL DEFAULT '0.0000' COMMENT '本期调整增量<结算币种>',
  `previous_adjustment_delta_amount` decimal(18,4) NOT NULL DEFAULT '0.0000' COMMENT '往期调整增量<结算币种>',
  `late_fee_amount` decimal(18,4) NOT NULL DEFAULT '0.0000' COMMENT '滞纳金<结算币种>',
  `receivable_amount` decimal(18,4) NOT NULL DEFAULT '0.0000' COMMENT '应收金额<结算币种>',
  `paid_amount` decimal(18,4) NOT NULL DEFAULT '0.0000' COMMENT '已核销金额<结算币种>',
  `unpaid_amount` decimal(18,4) NOT NULL DEFAULT '0.0000' COMMENT '未核销金额<结算币种>',
...
CREATE TABLE `fee_detail` (
  ...
  `exchange_rate_to_bill` decimal(18,8) DEFAULT NULL COMMENT 'L1汇率快照：原始币种到结算币种汇率',
  `exchange_rate_level_to_bill` varchar(16) DEFAULT NULL COMMENT '汇率级别',
  `bill_currency` varchar(16) NOT NULL COMMENT '结算币种',
  `amount_bill_currency` decimal(18,4) NOT NULL COMMENT '费用金额<结算币种>',
  `exchange_rate_to_fin` decimal(18,8) DEFAULT NULL COMMENT 'L2汇率快照：结算币种到本位币汇率',
...
CREATE TABLE `payment_writeoff_detail` (
  ...
  `bill_id` bigint(20) unsigned NOT NULL COMMENT '账单ID',
  `bill_no` varchar(64) NOT NULL COMMENT '账单编号',
  `currency_summary_id` bigint(20) unsigned DEFAULT NULL COMMENT '核销的结算币种汇总ID',
  ...
  `bill_currency` varchar(16) NOT NULL COMMENT '结算币种',
  `exchange_rate_to_bill` decimal(18,8) DEFAULT NULL COMMENT '收款币种到结算币种汇率',
  `writeoff_amount_bill_currency` decimal(18,4) NOT NULL COMMENT '核销金额<结算币种>',
  `fin_currency` varchar(16) NOT NULL DEFAULT 'CNY' COMMENT '财务本位币',
  `exchange_rate_to_fin` decimal(18,8) DEFAULT NULL COMMENT '结算币种到本位币汇率',
