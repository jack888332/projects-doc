...
5. 包裹级`实收回款`不得复制到同一订单的其他尾程包裹，也不得自动拆分为订单下各包裹的回款金额。各包裹的回款状态仍按自身实收回款独立判定，见[包裹回款与返款状态判定](#doc-A4A4EF02F0-collection-c8c56052)。
...
1. 正常签收包裹的回款状态和返款状态，分别按[包裹回款与返款状态判定](#doc-A4A4EF02F0-collection-c8c56052)独立计算，不因返款账单主状态直接变化。
