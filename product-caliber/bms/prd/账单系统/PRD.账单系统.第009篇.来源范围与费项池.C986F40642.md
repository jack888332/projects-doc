...
3. 返款账单计算出现`实付返款（准）`负数时，按[应付返款不足以覆盖指定扣减费项时怎样处理？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-negative-amount)完整执行，本节只说明归属结果：`顺延到下期返款账单`只形成负数承接记录，`反向计入本期应收账单`在负数订单所属返款账单审核通过时生成关联原返款账单、业务订单和货款原始币种的独立应收调整费项；原指定扣减费项仍归属返款账单，不得再次进入应收账单。
