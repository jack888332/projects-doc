...
3. 返款账单计算出现`实付返款（准）`负数时，按[应付返款不足以覆盖指定扣减费项时怎样处理？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-negative-amount)执行：`顺延到下期返款账单`只形成负数承接记录，`反向计入本期应收账单`只生成关联原返款账单的独立应收调整费项；原指定扣减费项仍归属返款账单，不得再次进入应收账单。
