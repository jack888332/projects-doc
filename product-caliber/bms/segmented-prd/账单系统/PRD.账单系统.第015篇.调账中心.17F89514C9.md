...
| 导入域 | 账单类型、冲正归属、客户名称、原账单编号、原账期、归属账单编号、冲正费项、费项挂靠对象、业务订单号、尾程运单号、首程运单号、冲正理由、冲正前币种、原币金额变幅 |
...
金额冲正记录的字段名称与口径以本节为权威定义，页面、导入模板、导出模板与原型列名都必须与之保持一致。表内金额列名与[金额冲正列表、导入与成本冲正字段](#doc-17F89514C9-adjustment-center-9a76e6c3)中的列表域列名一一对应。

| 字段 | 定义 | 取值口径 |
| :--- | :--- | :--- |
| 原账单编号、原账期 | 金额差异所在原账单及其实际账期 | 与原账单快照一致，审核通过后不可变。 |
| 冲正费项 | 需要修正金额的费项 | 必须能对应到原账单中的费项记录；按费项原始金额识别差异时，金额以费项原始币种计量。 |
| 冲正前币种 | 原币金额所采用的币种 | 金额差异按费项原始金额识别时，使用`费项原始币种`；按原账单已经换算后的金额识别时，使用该费项在原账单中的结算币种（应收为`费项结算币种`，返款为`返款原始币种`）。 |
| 原币金额变幅 | 原币冲正后金额与原币原金额在冲正前币种下的带符号差额 | 导入只填写本字段；原币冲正后金额按原账单中原币金额自动计算。 |
| 原币冲正后金额 | 冲正完成后该费项在冲正前币种下的金额 | 必须等于原币原金额与`原币金额变幅`之和，列表与详情展示同一数值。 |
...
| `已移除` | 记录已逻辑删除，保留编号与提交内容，进入只读终态；默认不在列表、账单调整记录、金额汇总或导出中展示，只可通过筛选与内部审计追溯。 |
| 状态机起点 | 记录新建后进入 `待审核`。 |
...
3. `待审核`和`审核驳回`的金额冲正记录只在调账中心和账单的调整记录中展示，`已移除`记录默认不展示，两者均不得进入账单金额汇总、核销口径或客户导出。只有`审核通过`记录才允许影响对应应收账单或返款账单的金额和核销余额，或者影响成本调整记录、成本归属、成本核销结果和利润分析。
