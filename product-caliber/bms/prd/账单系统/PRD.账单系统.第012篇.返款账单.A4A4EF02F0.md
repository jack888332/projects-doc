...
当业务订单按`实付返款（准）= 应付返款 - 返款账单指定扣减费项`计算，且金额可计算的结果小于零时，说明该订单的`应付返款`不足以覆盖本次指定的扣减费项。负数只在货款原始币种中产生；系统不得把负数换算为`实付返款`，也不得生成负数待付返款、负数返款核销记录或负数账单汇总。`cod_price`、`dest_country_surcharge_amount` 等计算输入缺失或不可计算不属于本节范围，相关订单不得进入本期返款账单，按[返款金额从哪里取？](#doc-A4A4EF02F0-refund-bill-3b9f382c)的来源输入异常处理。
...
| 返款明细（订单返款数据行） | 业务订单号、包裹数、来源金额与币种、到付附加费总额、应付返款（即代收货款）、各指定扣减费项名称及扣减金额、实付返款（准）、实付返款 | 按业务订单逐行展示订单级金额链路；本账单实际占用的每个指定扣减费项都以独立金额列直接列在订单行中，没有命中时对应列显示零金额。订单行不展示返款汇率、已付返款、待付返款或核销状态，也不提供包裹回款展开行；返款汇率统一在`账单汇率`展示，返款支付与核销进度统一在汇总金额分桶展示。 |
