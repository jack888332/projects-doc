...
| 说明 | 1. 调账单号必须全局唯一，生成后不可变；当前实现采用 UUID。<br>2. 冲正前币种按冲正费项的原始币种或结算币种填写。 |

<a id="doc-17F89514C9-adjustment-center-59642159"></a>
#### 1.2.2 调账状态、操作与权限

调账记录的业务操作与审核状态强绑定。下表是可执行操作、执行人、批量能力和状态流转的完整定义。
...
3. 调账记录在各状态下的可执行操作、执行人、批量能力和状态流转，以[调账状态、操作与权限](#doc-17F89514C9-adjustment-center-59642159)中的操作矩阵为完整定义。
4. 审核人只能审核，不能代替提交人修改内容。
5. `审核通过` 记录进入只读态，不允许再次修改或移除，只能通过新增反向调账修正。
6. 批量移除、批量审核仅对待审核记录生效，混入其他状态时应禁用相应操作。
7. 归属账单未确认且系统未生成有效默认归属的调账记录，不允许审核通过。
8. 调账记录必须明确记录来源、去向、金额、币种、原因和凭证。
...
3. 调账通过审核后，才允许影响对应应收账单或返款账单的应收金额、实付返款或核销结果，或者影响成本调整记录、成本归属、成本核销结果和利润分析。
...
6. 同一客户、同一账期存在多份候选账单时，默认归属优先选择财务本位币汇总金额最高的账单。
7. 该优先级用于降低冲正金额叠加后单张账单出现负值的可能性。
...
2. 返款账单中的调账影响实付返款、已付返款和待付返款的展示与核销口径。
