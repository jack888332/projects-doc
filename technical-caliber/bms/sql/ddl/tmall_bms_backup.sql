...
CREATE TABLE `ar_bill` (
  ...
  `bill_currency` varchar(16) NOT NULL COMMENT '结算币种',
  `initial_receivable_amount_in_bill_currency` decimal(18,4) DEFAULT NULL COMMENT '初始应收金额<结算币种>',
  `this_bill_amount_adjustment_delta_in_bill_currency` decimal(18,4) DEFAULT NULL COMMENT '本期账单金额调整增量<结算币种>',
  `previous_bill_amount_adjustment_delta_in_bill_currency` decimal(18,4) DEFAULT NULL COMMENT '往期账单金额调整增量<结算币种>',
  `late_fee_in_bill_currency` decimal(18,4) DEFAULT NULL COMMENT '本期滞纳金额<结算币种>',
  `receivable_amount_in_bill_currency` decimal(18,4) DEFAULT NULL COMMENT '应收金额<结算币种>',
  `paid_amount_in_bill_currency` decimal(18,4) DEFAULT NULL COMMENT '实收金额<结算币种>',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='结算汇率';
...
CREATE TABLE `fee_detail` (
  ...
  `exchange_rate_c1` decimal(18,8) DEFAULT NULL COMMENT '锁定汇率<L1>',
  `exchange_rate_level_c1` varchar(2) DEFAULT NULL COMMENT '汇率级别<L1>',
  `bill_currency` varchar(16) DEFAULT NULL COMMENT '结算币种',
  `amount_in_bill_currency` decimal(18,4) DEFAULT NULL COMMENT '费用金额<结算币种>',
  `exchange_rate_c2` decimal(18,8) DEFAULT NULL COMMENT '锁定汇率<L2>',
  `exchange_rate_level_c2` varchar(2) DEFAULT NULL COMMENT '汇率级别<L2>',
