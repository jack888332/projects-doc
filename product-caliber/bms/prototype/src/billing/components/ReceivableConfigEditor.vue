...
let schemeSeed = 1
const createFeeRule = () => ({ feeCode: '', mode: 'SOURCE', currency: '' })
const createScheme = () => ({
...
const form = reactive({
  ...
  contractNo: `HT-${props.config.customerNo || 'CUSTOMER'}-2026`, contractFiles: [],
})
const allFolded = computed(() => form.branches.length > 0 && form.branches.every(item => item.folded))
const previewNo = computed(() => {
  if (!props.config.no || props.config.no === '新配置') return `ARB-${props.config.customerNo || '客户编码'}-Scheme-{时间戳}-v1`
...
function addFeeRule(scheme) { scheme.feeRules.push(createFeeRule()) }
function applyTemplate(scheme, value) {
  if (!value) return
  scheme.feeRules = [
...
<template>
  <div class="config-editor">
    <section class="rule-card">
      ...
      <div class="config-no-bar"><span>配置编号</span><el-tooltip content="格式：ARB-{客户编码}-Scheme-{创建时间戳}-v{版本号}。前段创建后不可变，仅版本号随保存递增。"><b>{{ previewNo }}</b></el-tooltip><small>{{ config.no === '新配置' ? '首次保存将生成 v1' : '保存后版本号自动递增' }}</small></div>
      ...
      <div class="scheme-section">
        ...
        <div class="scheme-body">
          ...
          <div class="setting-row matrix-row"><div class="setting-meta"><b>费项指定结算币种</b><small>未配置的费项采用默认结算币种</small></div><div class="matrix-wrap"><el-table :data="form.defaultScheme.feeRules" border empty-text="未配置，默认使用费项默认结算币种"><el-table-column label="费项" min-width="170"><template #default="{row}"><el-select v-model="row.feeCode" filterable placeholder="请选择费项"><el-option v-for="item in feeItems" :key="item.value" :label="item.label" :value="item.value" /></el-select></template></el-table-column><el-table-column label="结算币种" min-width="140"><template #default="{row}"><el-select v-model="row.mode" @change="changeMode(row)"><el-option label="按数据源币种" value="SOURCE" /><el-option label="固定币种" value="FIXED" /></el-select></template></el-table-column><el-table-column label="固定币种" width="130"><template #default="{row}"><el-select v-model="row.currency" :disabled="row.mode!=='FIXED'" clearable><el-option v-for="item in currencies" :key="item" :label="item" :value="item" /></el-select></template></el-table-column><el-table-column label="操作" width="70"><template #default="{$index}"><el-button link type="danger" :icon="Delete" @click="form.defaultScheme.feeRules.splice($index,1)" /></template></el-table-column></el-table><div class="matrix-actions"><el-select v-model="form.defaultScheme.template" clearable placeholder="引用费项币种模版" @change="applyTemplate(form.defaultScheme,$event)"><el-option v-for="item in templates" :key="item.value" :label="item.label" :value="item.value" /></el-select><el-button :icon="Plus" @click="addFeeRule(form.defaultScheme)">新增费项币种</el-button></div></div></div>
      ...
      <div v-for="(scheme,index) in form.branches" :key="scheme.id" :class="['scheme-section','branch-section',{conflict:hasConflict(scheme)}]">
        ...
        <div v-show="!scheme.folded" class="scheme-body">
          ...
          <div class="setting-row matrix-row"><div class="setting-meta"><b>费项指定结算币种</b><small>未配置的费项采用默认结算币种</small></div><div class="matrix-wrap"><el-table :data="scheme.feeRules" border empty-text="未配置，默认使用费项默认结算币种"><el-table-column label="费项" min-width="170"><template #default="{row}"><el-select v-model="row.feeCode"><el-option v-for="item in feeItems" :key="item.value" :label="item.label" :value="item.value" /></el-select></template></el-table-column><el-table-column label="结算币种" min-width="140"><template #default="{row}"><el-select v-model="row.mode" @change="changeMode(row)"><el-option label="按数据源币种" value="SOURCE" /><el-option label="固定币种" value="FIXED" /></el-select></template></el-table-column><el-table-column label="固定币种" width="130"><template #default="{row}"><el-select v-model="row.currency" :disabled="row.mode!=='FIXED'"><el-option v-for="item in currencies" :key="item" :label="item" :value="item" /></el-select></template></el-table-column><el-table-column label="操作" width="70"><template #default="{$index}"><el-button link type="danger" :icon="Delete" @click="scheme.feeRules.splice($index,1)" /></template></el-table-column></el-table><div class="matrix-actions"><el-select v-model="scheme.template" clearable placeholder="引用费项币种模版" @change="applyTemplate(scheme,$event)"><el-option v-for="item in templates" :key="item.value" :label="item.label" :value="item.value" /></el-select><el-button :icon="Plus" @click="addFeeRule(scheme)">新增费项币种</el-button></div></div></div>
...
<style scoped>
.config-editor{padding:0 4px 24px;color:#303746}.rule-card{border:1px solid #dfe4ec;background:#fff}.rule-head{height:54px;padding:0 18px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #e7eaf0;background:#f8f9fb}.rule-head>div{display:flex;align-items:center;gap:18px}.rule-head strong{font-size:16px}.config-no-bar{min-height:42px;padding:8px 18px;display:flex;align-items:center;gap:14px;background:#fff8e8;border-bottom:1px solid #f0dfb6}.config-no-bar span,.config-no-bar small{color:#767e8d}.config-no-bar b{color:#25334b}.scheme-section{border-bottom:1px solid #dfe4ec}.scheme-title,.branch-head{height:46px;padding:0 18px;display:flex;align-items:center;gap:14px;background:#f4f6f9}.scheme-title span,.add-branch span{font-size:12px;color:#8a93a2}.scheme-body{padding:0 18px}.setting-row{min-height:66px;display:grid;grid-template-columns:280px minmax(280px,400px);gap:22px;align-items:center;border-bottom:1px solid #edf0f4}.setting-meta{display:flex;flex-direction:column;gap:5px}.setting-meta b{font-size:13px;font-weight:600}.setting-meta small{font-size:12px;color:#8992a1;line-height:1.5}.setting-meta i{color:#e34d59;font-style:normal}.matrix-row{grid-template-columns:280px minmax(0,1fr);align-items:start;padding:15px 0}.matrix-wrap{min-width:0}.matrix-actions{display:flex;gap:10px;margin-top:10px}.matrix-actions .el-select{width:260px}.branch-head{border-left:3px solid var(--primary)}.branch-head .fold-button{border:0;background:transparent;color:#596579;cursor:pointer}.branch-head .el-button{margin-left:auto}.branch-head span{font-size:12px;color:#c84751}.branch-section.conflict{box-shadow:inset 3px 0 #d94a55}.scope-block{margin:14px 0;border:1px solid #e4e8ef;background:#fafbfc}.scope-block h4{margin:0;padding:10px 14px;border-bottom:1px solid #e4e8ef;font-size:13px}.scope-block .setting-row{padding:0 14px;grid-template-columns:246px minmax(280px,400px)}.add-branch{padding:18px;display:flex;align-items:center;gap:14px;border-bottom:1px solid #e4e8ef}.footer-terms{padding:0 18px}.footer-terms .setting-row:last-child{border-bottom:0}
@media(max-width:760px){.config-editor{padding:0}.rule-head{height:auto;min-height:54px}.config-no-bar{align-items:flex-start;flex-direction:column;gap:4px}.scheme-body,.footer-terms{padding:0 12px}.setting-row,.scope-block .setting-row{grid-template-columns:1fr;gap:8px;padding:12px 0;align-items:start}.setting-row>.el-select,.setting-row>.el-input,.setting-row>.el-date-editor,.setting-row>.el-input-number{width:100%!important}.matrix-row{grid-template-columns:1fr}.matrix-actions{flex-direction:column}.matrix-actions .el-select{width:100%}.branch-head{padding:0 10px}.branch-head span{display:none}}
</style>
