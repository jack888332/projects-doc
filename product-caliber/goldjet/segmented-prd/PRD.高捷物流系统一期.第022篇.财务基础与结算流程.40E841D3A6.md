...
**概念模型：** [数据字典 · 基础主数据实体](PRD.高捷物流系统一期.第034篇.数据字典.7A3D9E1F50.md#doc-7A3D9E1F50-domain-master-data)、[数据字典 · 财务结算实体](PRD.高捷物流系统一期.第034篇.数据字典.7A3D9E1F50.md#doc-7A3D9E1F50-domain-finance)
...
角色的操作授权见[财务结算操作权限](PRD.高捷物流系统一期.第035篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-finance-actions)。本篇保留结算流程中的场景分工：
...
- 步骤 4 通过后，业务员可执行步骤 5.1 打印单票账单；图中步骤 5.2 为财务审批，不把打印设为其前置条件。
...
费用审批、拒绝明细处理及对账调整回流共用[应收结算流程中的“费用审批与对账回流”图](#doc-40E841D3A6-section-23c66131aa44)；付款申请的审批回流与核销图在本表之后。
...
币种数据词典见 [数据字典 · 币种数据词典](PRD.高捷物流系统一期.第034篇.数据字典.7A3D9E1F50.md#doc-7A3D9E1F50-values-enum-currency)。
...
| 字段或控件特例 | 特例约束或业务结果 |
| --- | --- |
| 部门编码 | 候选列表展示2列（编码、名称）；候选范围见[财务数据访问范围](PRD.高捷物流系统一期.第035篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-finance-scope)。 |
...
- **区域级通用规则**：字段均只读；在[财务数据访问范围](PRD.高捷物流系统一期.第035篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-finance-scope)内按查询条件筛选，未设置查询条件时展示范围内全部记录。
