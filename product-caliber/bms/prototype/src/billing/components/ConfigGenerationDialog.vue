...
function confirm() {
  ...
  emit('confirm', { scopes:selectedRows.value, skippedCount:blockedCount.value, frozenCustomerCount:selectedCustomerCount.value, candidateCustomerCount:preview.value.exactCustomerCount, blockedCount:blockedCount.value, unselectedCount:unselectedCount.value, cutoff:form.cutoff, reason:form.reason.trim() })
  visible.value = false
}
</script>
...
<template>
  <el-dialog v-model="visible" class="module-dialog module-dialog-large config-generation-dialog" align-center append-to-body destroy-on-close :close-on-click-modal="false">
    <template #header><div class="drawer-title"><span>批量生成账单</span><small>{{ config?.name }} · {{ config?.no }} / {{ config?.version }}</small></div></template>
    <div class="generation-summary">
      <div><span>当前准确版本引用客户</span><strong>{{ preview.exactCustomerCount }}</strong></div>
    ...
    <el-alert title="本次只冻结当前配置编号与准确版本的客户引用；任务按客户、账单类型、方案标识和实际账期独立创建。" type="info" :closable="false" show-icon />
    ...
    <DataTableFrame class="generation-table" :total="preview.rows.length" :selected-count="selectedRows.length" selection-summary :pagination="false" :column-sort="false">
