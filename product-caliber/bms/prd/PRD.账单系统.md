...
| 来源准备 | 把稳定后的来源费项进入 `BMS费项池`；`BMS费项池` 是应收账单、返款账单、成本账单全部金额明细的统一归集根基。 | <a href="#section-9-5">9.5 第四步：来源费用怎样变成BMS费项？</a>、<a href="#section-17-2">17.2 业务系统基本面</a> |
...
| 金额处理 | 锁定币种、汇率、金额快照和核销口径。 | <a href="#section-13">13. 金额汇兑</a> |
| 差异闭环 | 对错误、遗漏、客户异议和历史差异做可追溯处理。 | <a href="#section-14">14. 调账中心</a>、<a href="#section-15">15. 非功能需求</a> |
...
5. 历史保护：应收账单、返款账单在审核后不得被静默覆盖；成本账单形成后不得被来源数据静默覆盖，已结清成本账单只能通过<a href="#section-14">14. 调账中心</a>形成成本金额冲正或成本核销冲正，且不得返结清。
...
2. 账期结束后，系统完成期末收口；已经生成的账单达到`待审核 + 已收口`后进入财务审核，尚未生成的账单由期末任务首次生成并收口。信用期、返款处理期和核销期是账期之后的资金处理口径。
...
4. `订单完结`与广义签收的当前系统口径见<a href="#section-17-2-5">17.2.5 广义签收</a>；订单类费用归集时间口径见<a href="#section-9-5-4">9.5.4 费用达到什么条件才能入池？</a>。
...
3. 条款生效周期内不允许重叠。
4. 货款结算币种的选择、匹配和兜底规则见<a href="#section-13">13. 金额汇兑</a>。
...
4. 费项结算币种、汇率快照和多币种汇总规则统一见<a href="#section-13">13. 金额汇兑</a>。
...
客户级汇率配置解决的是“账单生成前，不同客户、不同货币对默认采用什么汇率”的问题。它位于账单生成之前，是账单特调汇率快照的前置配置；账单生成之后，命中的汇率会进入账单侧快照，由<a href="#section-13">13. 金额汇兑</a>继续定义换算、汇总和核销规则。
...
账期结束前生成的账单为`待审核 + 未收口`，只能在财务内部处理，不得审核或发出。账单达到`待审核 + 已收口`并审核通过后，系统才按客户级账单配置向客户发出账单；`账单发出时间`不触发任务。
...
3. 客户、供应链、店铺、会员与业务订单的归属边界按<a href="#section-17-2-1">17.2.1 核心对象关系</a>执行。
4. 费项结算币种读取客户级账单配置已经保存的结果；默认汇率命中按<a href="#section-8-5">8.5 账单生成命中与快照</a>执行，金额换算按<a href="#section-13-3">13.3 汇率与金额换算</a>执行。
...
任务锁定规则后，系统先按<a href="#section-9-3-1">9.3.1 配置改了，旧账单怎么办，新费项归谁？</a>确定费项归属配置版本，再按该版本的账期、履约节点、限定情形和新增时间筛选本次可处理的来源数据。各类费项对应的来源表、金额字段、业务类型和首次产生时机见<a href="#section-17-2-3">17.2.3 费项数据源</a>。`BMS检阅标记`只表示来源数据是否已被BMS处理，不表示账单、支付或核销状态。
...
可入池的数据源、字段性质、挂靠对象、首次产生时机和横表 / 纵表结构统一按<a href="#section-17-2-3">17.2.3 费项数据源</a>执行。
...
1. 订单类费项的履约节点按<a href="#section-7-4">7.4 应收账单配置</a>执行；`订单完结`的判断按<a href="#section-17-2-5">17.2.5 广义签收</a>执行。
2. 费项金额的稳定边界按<a href="#section-17-2-4">17.2.4 费项重算窗口</a>执行。尚处于重算窗口内的横表来源不得入池。
...
费项支付方式按<a href="#section-17-2-2">17.2.2 费项支付方式</a>判断；应收与返款的费项用途、占用优先级、账期关系和展示口径按<a href="#section-11-2-2">11.2.2 返款账单和应收账单的关联性</a>执行。并发和负数处理还须遵守以下规则：
...
3. 读取账单配置已经保存的费项结算币种，并按<a href="#section-13-3">13.3 汇率与金额换算</a>和<a href="#section-13-4">13.4 多币种账单汇总</a>形成结算币种金额、财务本位币金额、币种分桶和账单总览。
...
2. 首次生成不要求账期已经结束。账期结束前手动生成成功时，账单进入`待审核`，账期收口状态为`未收口`；页面必须显示`账期未结束`提示并禁用审核和发送。
...
4. 已收口但尚未发出的账单出现会影响范围或金额的数据变化时，系统应在相关费项入池或账单生成任务创建成功时，立即把账期收口状态改为`未收口`并禁用审核和发送，不得等到任务开始执行。任务删除或结束后，系统重新执行收口校验；只有全部条件再次满足时才能恢复为`已收口`。已经发出的账单不得重新打开收口状态，晚到费用按调账、冲正或后续账单规则处理。
...
| 顺序 | 账期内发生的事情 | 账单结果 |
| :--- | :--- | :--- |
| 1 | 财务在账期结束前手动生成账单。 | 没有目标账单时执行首次生成；账单进入`待审核 + 未收口`，只能查看和继续补充，不能审核或发送。 |
...
6. 不得把重算后的最终应收或应返金额降到已核销金额以下，也不得直接形成无法结算的超收或超返结果。返款重算出现负数时，优先按<a href="#section-7-5">7.5 返款账单配置</a>选择的方式顺延到下期返款账单，或生成独立的应收调整费项，使当前返款账单不以负数结算；处理后仍与既有核销、回款或返款事实冲突时，再转入<a href="#section-14">14. 调账中心</a>。合法的红冲或负向调账明细不受该限制。
...
调账、冲正、费用重整、核销记录冲正及其审核和归属规则统一见<a href="#section-14">14. 调账中心</a>。
...
执行级别为`账单计算`的任务成功后，系统先保存本次结果和计算依据，再按`计算类型`、`生成方式`和`是否期末收口`处理账单主状态与账期收口状态。`结果快照`回答“这个金额是怎样算出来的”，主状态决定账单处于哪个财务环节，账期收口状态决定账单是否具备审核和发出条件。
...
只有`待审核 + 已收口`的账单才允许审核和发出。账单一旦发出，既有通知和资金事实必须保留；已结清账单不支持反核销，核销错误须通过<a href="#section-14">14. 调账中心</a>新增核销记录冲正。

<div id="section-9-7-3"></div>

#### 9.7.3 账单结清后，来源系统还要更新什么？
...
应收账单状态见<a href="#section-10-4">10.4 账单状态机</a>，返款账单状态见<a href="#section-11-4">11.4 返款账单状态机</a>，多币种结清条件见<a href="#section-13-5">13.5 多币种核销</a>。共同处理边界如下：

| 当前账单情况 | 允许处理 | 不允许处理 |
| :--- | :--- | :--- |
| 从未发出的`待审核`账单 | 补充生成、替换生成、重算；发生影响范围或金额的变化后重新校验账期收口 | 删除账单、覆盖原结果或跳过影响预览 |
...
| 应收账单详情 | 补充生成、替换生成、期末收口、汇率重算、补录、调账、冲正、重整、导出 | `待审核 + 未收口`不得审核或发送；替换生成仅对尚未发出的待审核账单开放；已作废和已结清不提供计算入口。 |
| 返款账单详情 | 补充生成、替换生成、期末收口、返款金额修正、扣减项调整、折返审核、导出 | 使用与应收账单相同的收口和替换边界；不得通过返款账单操作改写应收账单。 |
...
页面操作权限统一遵守<a href="#section-15-1">15.1 权限与职责边界</a>，并补充以下约束：
...
7. 只有`待审核 + 已收口`的账单才能审核和发出；账单发出后不得作废或替换生成。
...
6. 账期结束后，系统完成期末费项入池和账单收口；账单达到`待审核 + 已收口`后才允许审核。
...
| 主状态 | 定义 | 关键规则 |
| :--- | :--- | :--- |
| 待审核 | 账单已经生成，仍在财务内部处理 | 从未发出时允许补充生成、替换生成和重算；已经发出后折返时只允许按既有事实重算或调账；只有账期已收口才允许首次审核和发出 |
...
| 账期收口状态 | 定义 | 关键规则 |
| :--- | :--- | :--- |
| 未收口 | 账期尚未结束，或账期结束后仍有来源数据、配置变更等待处理 | 待审核账单可以查看、补充生成、重算或替换生成，但不得审核或发出 |
| 已收口 | 数据截止点已经覆盖账期结束时间，且账期内应处理费用和金额校验均已完成 | 与`待审核`同时满足时才允许审核并发出 |

补充规则：

1. 若账单内存在冲正记录，则必须待全部冲正记录审核通过后，账单才允许审核通过。
2. 冲正记录未全部审核通过，或账期收口状态为`未收口`时，账单应继续保留在待审核态，不得提前进入待结清口径。
...
PendingReview --> PendingSettlement : 已收口且审核通过
...
3. 账期结束后的自动或手动期末任务完成费项入池、账单生成和完整性校验后，把账期收口状态更新为`已收口`。只有`待审核 + 已收口`且冲正记录全部审核通过，财务才可以审核账单。
...
客户侧费项索引只服务应收账单和返款账单，费项名称由我司统一定义，用于财务导入、冲正和采集配置参考；不得被成本账单或成本明细引用。成本账单使用的成本费项索引见<a href="./成本中心.PRD.md">《成本中心 PRD》5.2.2 客户侧与成本侧费项索引分域</a>。具体金额换算、汇率快照和多币种汇总规则见<a href="#section-13">13. 金额汇兑</a>。
...
2. 待审核账单必须达到`已收口`后才能审核；未发出的待审核账单允许替换生成。
...
应收账单详情页用于查看一张应收账单的结算结果、金额快照和汇率快照；涉及币种分桶、金额换算、往期冲正和本位币折算的规则统一见<a href="#section-13">13. 金额汇兑</a>。
...
| 区域 | 业务字段 / 操作 | 关键业务约束 |
| :--- | :--- | :--- |
| 账单概况 | 账单编号、主状态、账期收口状态、账期类型、账期起止日、客户、会员编码、店铺、业务板块、运抵国；审核通过、导出账单 | `逾期未结清`仅作为筛选条件或标签；只有`待审核 + 已收口`允许审核；详情导出固定为当前账单的客户对账文件。 |
...
3. 货币对来源、换算方向、账单级锁定汇率和客户级默认汇率回显的规则统一见<a href="#section-13">13. 金额汇兑</a>；账单级锁定汇率配置不可关闭。
...
1. 应收账单详情内的`导出`固定为`导出给客户`，仅导出当前应收账单，并按<a href="#section-17-6-1">17.6.1 面向客户的对账报表导出</a>生成一份客户对账文件；该入口不展示用途选择，也不支持内部明细导出。
2. 应收账单列表的批量导出分为`导出给客户`和`导出给内部`。`导出给客户`按<a href="#section-17-6-1">17.6.1 面向客户的对账报表导出</a>逐账单生成客户对账文件；`导出给内部`按<a href="#section-17-6-2">17.6.2 应收账单内部明细导出</a>汇总所选账单的费项明细。
...
返款账单模块用于管理 COD 包裹签收后的货款回收、货款返还、账单核销追踪和账单结清进度；币种、汇率与汇兑口径统一见<a href="#section-13">13. 金额汇兑</a>。
...
返款账单用于承载 COD 包裹签收后的货款回收与返还结算，统一管理签收返款和回款返款两种模式下的账单归集、核销追踪和账单结清；币种、汇率与汇兑规则统一见<a href="#section-13">13. 金额汇兑</a>。
...
| `汇兑损益` | 到付币种与回款 / 返款币种不一致时的差额 | 在回款管理中计算与展示，具体口径见<a href="#section-13">13. 金额汇兑</a>。 |

关键规则：

1. 返款账单与应收账单可独立按各自账期运行，不要求同周期对齐。
2. 所有 COD 包裹均纳入回款管理；只有正常签收包裹进入回款、返款，并由回款管理计算汇兑损益，具体口径见<a href="#section-13">13. 金额汇兑</a>。
...
5. 若到付币种与回款 / 返款币种不一致，系统将回款 / 返款金额及快照交由回款管理计算汇兑损益，具体口径见<a href="#section-13">13. 金额汇兑</a>。
...
| :--- | :--- | :--- |
| 未收口 | 账期尚未结束，或账期结束后仍有来源数据、配置变更等待处理 | 待审核账单可以查看、补充生成、重算或替换生成，但不得审核或发出 |
| 已收口 | 数据截止点已经覆盖账期结束时间，且应处理返款费项和金额校验均已完成 | 与`待审核`同时满足时才允许审核并发出 |

补充规则：

1. 若账单内存在冲正记录，则必须待全部冲正记录审核通过后，账单才允许从待审核进入审核通过结论。
2. 冲正记录未全部审核通过，或账期收口状态为`未收口`时，账单仍应保留在待审核态，不得提前通过。
3. 冲正记录的审核统一在调账中心完成，账单侧只负责发起、展示和引用审核结果。
...
返款模式以回款返款为主流路径，签收返款作为少数客户的例外配置存在。系统需同时支持两种模式，但页面默认表达和业务描述应优先围绕回款返款展开；涉及币种换算、汇率快照和回款管理中的汇兑损益口径统一见<a href="#section-13">13. 金额汇兑</a>。
...
1. 系统识别到订单为正常签收。
2. 财务先向寄件人返还货款，金额口径与快照规则见<a href="#section-13">13. 金额汇兑</a>。
3. 系统按<a href="#section-13">13. 金额汇兑</a>计算应返金额，并将回款 / 返款金额供回款管理计算汇兑损益。
...
3. 财务确认回款到账后，系统按<a href="#section-13">13. 金额汇兑</a>锁定相关快照并计算应返金额，相关汇兑损益由回款管理计算。
...
返款账单配置统一见<a href="#section-7">7. 客户级账单配置</a>。本模块只描述返款账单的生成结果、返款模式、状态流转、回款管理和报表输出，不重复维护配置口径。

<div id="section-11-7"></div>

### 11.7 返款账单详情

返款账单详情页是账单视角页面，用于集中展示订单级返款账单的结算结果和核销轨迹。它只回答一张账单“怎么算、算到哪一步、核销到哪一步”，不负责判断包裹是否已回款、已返款；包裹级状态请到回款管理查看。涉及货款结算币种、回款 / 返款换算和汇率快照的规则统一见<a href="#section-13">13. 金额汇兑</a>。

页面划分为以下业务字段区域：

| 区域 | 业务字段 / 操作 | 关键业务约束 |
| :--- | :--- | :--- |
| 账单概况 | 账单编号、账单状态、客户、会员编码、店铺、目的国、账期类型、账期起止日、返款模式；退回待审核、登记返还、调账中心、导出明细 | 返款模式决定先回收后返还或先返还后回收；详情导出只包含当前返款账单。 |
...
回款管理是包裹视角列表，用于查看所有 COD 包裹的回款状态、返款状态、汇兑损益和对账状态。它只回答一个包裹“是否已回款、是否已返款、是否需要跟踪”，不承载返款账单本身的结算流程；返款账单只作为回款状态判定和追溯依据被关联引用。列表主粒度为尾程包裹，回款明细行数据来自订单费项报表导入后沉淀的 `sale_order_package_fee`。回款管理负责回款 / 返款金额换算、汇率快照和汇兑损益计算，具体口径见<a href="#section-13">13. 金额汇兑</a>。回款管理不提供单独的回款导入入口。
...
<div id="section-11-8-2"></div>

#### 11.8.2 页面结构与字段

| 区域 | 字段 / 操作 | 说明 |
| :--- | :--- | :--- |
| 操作区 | `查看汇兑损益` | 展示或隐藏按锁定快照计算的汇兑损益字段，具体金额口径见<a href="#section-13">13. 金额汇兑</a>。 |
...
| 返款信息列 | 返款时间、实返货款金额、返款模式、关联返款账单、应返货款金额 | 用于展示返款动作和结算结果，具体口径见<a href="#section-13">13. 金额汇兑</a>。 |

<div id="section-11-8-3"></div>
...
<div id="section-11-8-4"></div>

#### 11.8.4 状态联动

1. 正常签收包裹的回款状态和返款状态，分别按<a href="#section-11-8-3">11.8.3 业务规则</a>独立计算，不因返款账单主状态直接变化。
2. 返款账单从待结清进入已结清，只表示对应业务主单的返款闭环完成，不代表所有包裹状态同步变更。
3. 异常签收包裹固定显示为 `不回款` 和 `不返款`，并保留在回款管理列表中。
4. `查看汇兑损益` 打开时，页面展示的损益字段应保持与回款 / 返款锁定快照一致，不随市场汇率刷新；异常签收行和同币种行是否展示，按<a href="#section-13">13. 金额汇兑</a>处理。
...
成本中心已拆分为独立需求文档，完整需求见<a href="./成本中心.PRD.md">《成本中心 PRD》</a>。本章仅作为财务系统总 PRD 的模块入口；成本中心与金额汇兑、调账、非功能需求和内部审计的跨模块约束，继续分别承接<a href="#section-13">13. 金额汇兑</a>、<a href="#section-14">14. 调账中心</a>、<a href="#section-15">15. 非功能需求</a>和<a href="#section-16">16. 内部审计</a>。
...
5. 历史账单需要调整结算币种时，按<a href="#section-9">9. 账单生成/重算机制</a>和<a href="#section-14">14. 调账中心</a>处理，不得通过修改模板回写历史结果。
...
<div id="section-13-3-3"></div>

#### 13.3.3 费项结算币种汇率列表生成与锁定
...
<div id="section-13-3-4"></div>

#### 13.3.4 金额换算规则

1. 当费项原始币种与结算币种相同时，金额不做换算，直接沿用原始金额。
2. 当费项原始币种与结算币种不同时，按锁定汇率换算为结算币种金额。
3. 若需要进入财务本位币，再按<a href="#section-13-3-1">13.3.1 汇率层级</a>中对应的锁定汇率换算到财务本位币。
...
<div id="section-13-6-1"></div>

#### 13.6.1 应收账单换算总则

应收账单的币种间换算遵循“先分币种桶，再按锁定汇率换算费项，最后合并历史冲正并按需换算财务本位币”的顺序。也就是说，账单先在费项结算币种维度形成可核算结果，再在需要时折算到财务本位币；每一步的详细定义见<a href="#section-13-6-2">13.6.2 费项结算币种级锁定汇率规则</a>和<a href="#section-13-6-3">13.6.3 应收账单币种间换算规则</a>。

1. `原始应收金额` 只负责承接本账期内费项换算后的汇总结果，不包含历史账期冲正。
2. `往期账单冲正` 只在 `费项结算币种` 下计入，不在原始币种维度重复拆分。
3. 财务本位币只用于内部核算和对账，不改变客户侧结算币种。

<div id="section-13-6-2"></div>

#### 13.6.2 费项结算币种级锁定汇率规则

费项结算币种级锁定汇率的生成、启用、关闭和推导规则，统一见<a href="#section-13-3-3">13.3.3 费项结算币种汇率列表生成与锁定</a>。本节只说明金额换算如何引用这些锁定结果。

<div id="section-13-6-3"></div>
...
2. 每个币种桶内，系统先取该桶下费项的 `费项原始币种` 和 `费项金额`，再按<a href="#section-13-3-1">13.3.1 汇率层级</a>换算到 `费项结算币种`。
...
<div id="section-13-7"></div>

### 13.7 返款账单与回款管理币种口径

<div id="section-13-7-1"></div>
...
<div id="section-13-7-3"></div>

#### 13.7.3 回款管理币种口径

回款管理以尾程包裹为主粒度，只展示回款、返款和汇兑损益的包裹级结果，不承担返款账单的核销计算。

1. 回款管理中的回款金额、返款金额和未回款金额，均引用<a href="#section-13-7-1">13.7.1 返款账单换算总则</a>的换算结果。
...
7. 成本核销汇率、汇兑方向、锁定时间、确认人和关联结清记录必须形成不可覆盖的快照，并按<a href="#section-16">16. 内部审计</a>保留审计记录。
...
2. 原始币种金额、费项结算币种金额、货款结算币种金额、财务本位币金额和汇兑损益必须按<a href="#section-13">13. 金额汇兑</a>的口径计算与展示。
...
| 待审核 | 账单已经生成，仍在财务内部处理；只有账期已收口才允许审核和发出。 | 账期收口状态 | 表示账期数据是否完整，枚举值为`未收口`、`已收口`。 |
...
| 已结清 | 账单全部金额闭环，仅保留查询与审计。 | 锁定汇率 | 生成或审核时冻结的汇率快照，具体口径见<a href="#section-13">13. 金额汇兑</a>。 |
...
<div id="section-17-2-1"></div>

#### 17.2.1 核心对象关系

<div id="section-17-2-1-1"></div>
...
<div id="section-17-2-2"></div>

#### 17.2.2 费项支付方式

<div id="section-17-2-2-1"></div>
...
<div id="section-17-2-4"></div>

#### 17.2.4 费项重算窗口

<div id="section-17-2-4-1"></div>
...
上图说明的是订单从首次计费到二次计费完成的状态流转：只要订单仍可能从 `PAID`、`CALCED` 或 `MORE_CALCED` 回到 `MORE_READY`，就表示费用仍可能被重新计算或补充，不能把当前费项金额视为最终稳定结果。BMS 判断同步时点时，应同时参考本节的重算窗口和<a href="#section-17-2-3">17.2.3 费项数据源</a>中的字段产生时机，避免提前采集仍可能变化的费项金额。

<div id="business-baseline-generalized-delivery"></div>

<div id="section-17-2-5"></div>

#### 17.2.5 广义签收

<div id="section-17-2-5-1"></div>
...
| 财务日常 | 回款管理 | 登记和核对派送商回款，查看回款状态及回款金额 | <a href="#section-11-8">11.8 回款管理</a> |
| 财务日常 | 调账中心 | 处理补录、调账、冲正、重整及历史账单差异承接 | <a href="#section-14">14. 调账中心</a> |
...
| 财务日常 / 过程管控 | 基础配置 | 在费项管理和数据源规则中维护费项口径、数据来源及场景规则；维护费项结算币种模板，供财务编辑应收账单配置时快捷填充 | <a href="#section-9-9-3">9.9.3 系统从哪里知道该取哪些费用？</a>、<a href="#section-9-5">9.5 第四步：来源费用怎样变成BMS费项？</a>、<a href="#section-13-2">13.2 费项结算币种模板</a> |
| 财务日常 / 过程管控 | 导出管理 | 跟踪应收和返款账单导出进度，区分客户对账与内部明细用途，下载结果、重试失败任务并维护对账报表导出配置 | <a href="#section-17-6-3">17.6.3 导出管理</a> |
| 财务日常 / 过程管控 | 内部审计 | 查询账单、配置、导入、核销、调账、导出、结清、成本归属及分摊版本等关键操作记录 | <a href="#section-16">16. 内部审计</a> |
...
| 辅助测试 | 数据迁移 | 在测试环境中同步指定生产数据并重置测试所需标记 | <a href="#section-17-4-1">17.4.1 同步生产环境数据</a> |
| 辅助测试 | 报表比对 | 仅在财务试运营 BMS 阶段比对财务侧报表和系统侧报表，定位缺单、重复及费项差异 | <a href="#section-17-4-2">17.4.2 财务侧报表与系统侧报表比对</a> |

<div id="section-17-4"></div>

### 17.4 辅助测试功能

<div id="section-17-4-1"></div>
...
5. 权限继承：同步生产环境数据不得绕过<a href="#section-15">15. 非功能需求</a>的权限、审计、幂等、数据一致性和信息保护要求。

<div id="section-17-4-1-2"></div>
...
<div id="section-17-4-2"></div>

#### 17.4.2 财务侧报表与系统侧报表比对
...
4. 如邮件包含附件，附件必须使用<a href="#section-17-6-1">17.6.1 面向客户的对账报表导出</a>生成的客户对账文件，并遵循客户可见信息规则；<a href="#section-17-6-2">17.6.2 应收账单内部明细导出</a>生成的内部文件不得作为客户邮件附件。
...
<div id="section-17-6-1"></div>

#### 17.6.1 面向客户的对账报表导出

<div id="section-17-6-1-1"></div>
...
2. 金额口径承接<a href="#section-13">13. 金额汇兑</a>：应收账单只展示费项结算币种金额，返款账单只展示货款结算币种金额。
3. 对账报表不展示原始币种金额、汇率或财务本位币金额。

<div id="section-17-6-1-2"></div>

##### 17.6.1.2 导出文件与 Sheet 总览
...
<div id="section-17-6-1-3"></div>

##### 17.6.1.3 Sheet 1：账单汇总
...
5. 所有金额的数值、币种和精度遵循<a href="#section-17-6-1-6">17.6.1.6 金额、币种与精度</a>。

<div id="section-18-6-1-3-3"></div>

###### 18.6.1.3.3 对账报表导出配置

`对账报表导出配置`入口位于<a href="#section-17-6-3-2">17.6.3.2 管理面板</a>，以`应收账单`和`返款账单`为唯一配置维度，配置结果由所有客户共用。两类账单分别维护：
...
<div id="section-17-6-1-4"></div>

##### 17.6.1.4 Sheet 2：账单明细
...
<div id="section-17-6-1-5"></div>

##### 17.6.1.5 Sheet 3：往期账单冲正信息
...
<div id="section-17-6-2"></div>

#### 17.6.2 应收账单内部明细导出

应收账单内部明细导出用于财务集中查看和汇总多张应收账单的费项明细，不形成客户对账报表。返款账单不提供该导出用途。

<div id="section-17-6-2-1"></div>

##### 17.6.2.1 文件与 Sheet 结构
...
<div id="section-17-6-2-3"></div>

##### 17.6.2.3 费项明细

1. 费项明细字段、动态费项展开、日期时间格式、金额币种显示格式和 3 位小数精度，与<a href="#section-17-6-1-4">17.6.1.4 Sheet 2：账单明细</a>和<a href="#section-17-6-1-6">17.6.1.6 金额、币种与精度</a>中的应收账单口径一致。
...
<div id="section-17-6-3"></div>

#### 17.6.3 导出管理

导出管理统一承接应收账单和返款账单的导出发起、用途确认、任务查询、进度跟踪、结果下载，以及对账报表导出配置。

<div id="section-17-6-3-1"></div>

##### 17.6.3.1 发起导出

导出可以从账单详情或账单列表发起。账单详情只处理当前账单；应收账单和返款账单列表均提供行勾选框、当前页全选和`导出`操作，财务可以勾选一张或多张账单发起导出。

1. 在应收账单或返款账单详情内点击`导出`时，系统固定按`导出给客户`创建任务，仅包含当前账单，不展示导出用途选择；生成结果为一份符合<a href="#section-17-6-1">17.6.1 面向客户的对账报表导出</a>的 Excel。
...
4. 返款账单列表固定为`导出给客户`，沿用<a href="#section-17-6-1">17.6.1 面向客户的对账报表导出</a>，不展示内部导出选项。
...
8. `导出给内部`时，不论选择一张还是多张应收账单，均按<a href="#section-17-6-2">17.6.2 应收账单内部明细导出</a>生成一份 Excel；任务按账单逐张生成 Sheet，并以已生成 Sheet 数量计算进度。
9. 财务确认后，系统立即创建异步导出任务并反馈提交结果；财务可以离开发起页面，前往<a href="#section-17-6-3-2">17.6.3.2 管理面板</a>跟进进度。
