...
管理员、客服的订单操作授权见[地面业务操作权限](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第067篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-ground-actions)；订单来源、状态与修改条件仍按本篇场景规则执行。
...
两组字段口径均涉及航晟客服从“新建订单”入口手工建单，尚未明确各自适用范围。单号必填性、提送货时间的必填性和默认值存在差异，需确认后统一；不能据此把两组规则分别指定给手工订单和API订单。

**界面原型概述 · 用车订单创建口径一**

- **区域字段或业务元素**：客户订单号、委托方、委托方联系人、委托方联系电话1、委托方联系电话2、邮箱地址、件数（件）、体积（m³）、重量（kg)、尺寸（长度cm）、尺寸（宽度cm）、尺寸（高度cm）、期望提货时间、提货地点、提货联系人、提货联系电话1、提货联系电话2、监管车、特种车、尾板车、运输类型、期望送货时间、送货地点、送货联系人、送货联系电话、备注、业务类型。
...
| 委托方联系电话1 | 长度限制：8～20 个字符；选填；选择已有联系人后自动带出。 |
| 委托方联系电话2 | 长度限制：8～20 个字符；选填；与联系电话1分别录入和保存。 |
...
| 提货联系电话1、提货联系电话2、送货联系电话 | 各号码长度限制：8～20 个字符；选填；提货的两个号码分别录入和保存。 |
| 监管车 | 是或否；选填；初始不设值。 |
| 特种车、尾板车 | 选填；初始不设值。 |
| 运输类型 | 创建页面提供选择入口；候选项、必填性及多提货点时的归属尚未明确，不能仅凭报价中的同名字段推定。 |
...
**界面原型概述 · 用车订单创建口径二（差异待确认）**

- **区域字段或业务元素**：单号、委托方、委托方联系人、委托方联系电话1、委托方联系电话2、邮箱地址、件数（件）、方数（方）、重量（kg)、尺寸（长度cm）、尺寸（宽度cm）、尺寸（高度cm）、期望提货时间、提货地点、提货联系人、提货联系电话1、提货联系电话2、监管类型、特种车、是否尾板车、期望送货时间、送货地点、送货联系人、送货联系电话、备注。
...
- 每行提货信息中的同一联系人可记录联系电话1和联系电话2，分别保留两个号码。
...
| 提货联系人和联系方式、送货联系人及联系方式 | 联系人姓名与联系电话按“姓名：电话”格式展示；提货联系人填写两个号码时均展示，并分别标明联系电话1和联系电话2。 |
...
- 提货联系人及联系方式、送货联系人及联系方式按[运输订单列表栏界面原型概述](#doc-32C78B8106-section-b8ef337a5a66)显示。
...
- 若已调度，需要去掉具体的调度，则系统弹窗提示：“当前已调度，取消调度将产生相应的返空费。”若客服确认，则按照结算规则，生成相应的应收付“返空费”。计算规则见[用车运单自动计算费用](PRD.高捷物流系统.第040篇.陆运-运单管理.20AA542560.md#doc-20AA542560-section-50709de215e4)。
...
| 提货件数（件）、重量（kg）、体积（m³） | 若调度时录入了特定的重量、件数和方数，则显示录入的特定值，若没有特定值，则取订单上的相关值。若订单上也没此数值，则留空显示。 |
| 备注 | 取订单上的备注信息。 |
| 调度人 | 调度操作者的账户ID。 |
...
- 此页面只做基于订单维度的汇总展示，应收付的录入需要在统一的财务结算模块录入。应收应付生成、调整和导出流程见[结算管理流程](../PRD.高捷物流系统.第01册.需求框架/PRD.高捷物流系统.第011篇.DDP跨模块作业与结算流程.7579B595B8.md#doc-7579B595B8-section-a5b92ee23329)。
...
**专项规则：** 中转订单虽保留手动调度功能，但主要通过接收 WMS 反馈的实际调度信息（车牌等信息）进行自动调度匹配，由此自动生成相应的运单。中转运单的状态、费用与运输节点见[用车-运单管理](PRD.高捷物流系统.第040篇.陆运-运单管理.20AA542560.md#doc-20AA542560-section-c57389a7360b)。

**业务衔接：** [中转仓用车流程](../PRD.高捷物流系统.第01册.需求框架/PRD.高捷物流系统.第011篇.DDP跨模块作业与结算流程.7579B595B8.md#doc-7579B595B8-section-e4cd43033ac4)
...
完整触发条件、接收角色和邮件模板见[调度邮件通知规则](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第069篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-11ddcb71608e)。
...
完整触发条件、接收角色和消息模板见[订单企业微信通知规则](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第069篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-5abcb3a0e4eb)。
...
完整查询字段、统计口径和展示示例见[航晟陆运月报](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第070篇.报表管理.CF2AF4B8B2.md#doc-CF2AF4B8B2-section-336aafdb8518)。

<a id="doc-32C78B8106-platform-transport-service"></a>
## 2. 平台运输服务执行

### 2.1 服务来源、状态与调度

本场景接收平台订单勾选运输服务后生成的服务单，与航晟手工用车订单共用调度能力，但保留服务单号及上游订单关系。服务单初始为待接单；仅待接单可调度或批量调度，提交后为进行中、节点已调度，记录接单时间。通过状态管理登记到达时间后为已完成。

服务单的取消、终止、应收应付后果和状态管理按[干线及运输服务执行](PRD.高捷物流系统.第036篇.干线服务共用规则.76B3872565.md#doc-76B3872565-trunk-service-execution)处理。这里的“取消服务”不得替代[取消调度](#doc-32C78B8106-section-a943dda32b46)；服务状态与既有订单、运单状态的映射以及终止对在途运单的影响尚未明确。

查询按建单时间倒序；总运单号、订单号、服务单号支持精确批量查询，客户从档案模糊搜索。无条件时查询全部授权数据，重置清空条件。

**界面原型概述 · 平台运输服务查询与列表**

- **区域字段或业务元素**：总运单号、订单号、服务单号、客户、查询、重置；列表为服务单号、订单号、总运单号、分单号、客户、供应商、建单时间、接单时间、完成时间、服务单状态、服务节点、查看、调度、批量调度、编辑、状态管理、取消服务、终止服务。
- **区域级通用规则**：列表只读，客户与分单取上游，供应商取调度选择，接单时间取调度时间，完成时间取状态管理的到达时间；操作资格按本节服务状态控制。

### 2.2 单次与批量调度界面

**界面原型概述 · 平台运输服务调度资料**

- **区域字段或业务元素**：服务单号、财务组织、所属部门、业务类型、客户、客户名称、联系电话、邮箱；提货时间、提货地点、提货联系人、提货联系电话、送货时间、送货地点、送货联系人、送货联系电话；品名、件数、重量、体积、尺寸；供应商、车牌号、车型、车次、柜号、封条号、海关编号、车自重、架重、柜重、客户密码、备注、特定提货件数/体积/重量/尺寸、司机姓名/联系方式/身份证号。
- **区域级通用规则**：基本信息和客户信息从上游带入且只读；提送货信息及件数、重量、体积、尺寸带入并可编辑；调度区域沿用既有车辆和司机录入能力，以下仅列本来源差异。

| 字段或控件特例 | 界面特例或可见反馈 |
| --- | --- |
| 品名 | 单次调度字段表只读，批量调度字段表可改；是否有意区分待确认。 |
| 供应商 | 必填，待接单允许修改，进行中不允许修改。 |
| 车型 | 必填；单次从车型基础数据选择，批量来源只列柜车、吨车，两者是否为不同层级的类型待确认。 |
| 柜号、封条号 | 单次选填，最多 100 字；批量柜号最多 50 字且柜车时必填，封条号/海关关锁号最多 100 字。 |
| 车次 | 默认 1，允许数值；车次是否独立运单划分依据及上限未定义。 |
| 特定提货件数、体积、重量 | 件数为正整数，体积和重量支持两位小数；均选填。 |
| 特定提货尺寸 | 单次字段表允许两位小数，既有用车调度只允许正整数；批量表遗漏但截图仍显示，口径待确认。 |
| 海关编号、客户密码、架重、柜重 | 选填；架重、柜重支持两位小数。客户密码为业务现场使用的资料，来源未定义可见和脱敏范围，不视为登录密码或公开演示值。 |

**界面原型概述 · 排班与关联订单**

- **区域字段或业务元素**：新增调度、增加司机、排班查看；排班弹窗的司机检索、司机姓名/准驾类型/未完成任务数量/最近任务地址、车辆检索、车牌/车型/载重、提交、取消；关联服务单的查看与编辑。
- **区域级通用规则**：新增调度增加一组供应商及车辆资料，增加司机增加一行资料；排班选择提交后填入姓名、身份证、电话、车牌和车型。

来源要求排班弹窗展示全部司机和车辆；是否受供应商、财务组织、部门及任务占用限制未说明。关联订单按“相同提货点或相同卸货点”查找；与原用车要求“相同提货日期且相同提货点”不同，保留为服务场景差异待确认，不静默改写原关联订单范围。

批量调度可在添加订单弹窗查询后勾选服务单、确定加入调度列表；货物信息按客户页签切换，展示该客户的货物与附件。普通用车已定义的按车辆生成运单规则继续保留；来源没有明确平台服务是否完全复用其批量相容条件，不自动扩大混合调度范围。

**界面原型概述 · 运输服务附件**

- **区域字段或业务元素**：资料名称、附件类型、来源、操作时间、操作人、下载、添加附件、删除附件；批量页的客户信息与货物/附件页签。
- **区域级通用规则**：附件可勾选下载，添加时通过弹窗上传并确定，多选删除维护的附件；上游资料与本服务附件区分来源，删除上游原件的权限未定义。
