...
<template>
  <el-table :data="props.rows" class="clean-table rate-table" border height="100%" row-key="id">
    ...
    <el-table-column label="引用配置" min-width="245" show-overflow-tooltip><template #default="scope">{{ scope.row.config ? scope.row.configNo : '未配置' }}</template></el-table-column>
