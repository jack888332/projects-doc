...
本节后续的来源扫描规则只适用于`费项入池`和`账单生成`。`账单重算`直接读取目标账单当前有效明细和定向调账记录。
...
`账单重算`不执行本节，只读取目标账单已有明细及明确归属于目标账单的有效调账记录。普通来源费项尚未入池时，必须通过`费项入池`或`账单生成`处理；不得用重算把普通来源新费项补入账单。
...
5. 财务补录形成的新费项必须先形成可追溯的费项池记录，再参与账单计算；应收与返款账单的`金额冲正记录`不进入费项池，审核通过后作为账单级金额结果直接计入归属账单，规则见[调账中心](PRD.账单系统.第015篇.调账中心.17F89514C9.md#doc-17F89514C9-adjustment-center-98536490)。
...
1. 费项原始金额一旦成功入池，就成为已使用的历史依据。来源数据不得删除或改写该金额；如需修正，必须通过调账中心新增调整或冲正记录，处理路径以[调账中心](PRD.账单系统.第015篇.调账中心.17F89514C9.md#doc-17F89514C9-adjustment-center-98536490)为准。
...
4. `回款汇率`不是金额字段，表示`订单目的国币种 → 财务本位币（CNY）`的汇率，由订单费用报表导入；未导入回款汇率时，实收回款按目的国币种记录。具体字段口径见[OIS 有哪些客户侧费用字段？](PRD.账单系统.第021篇.OIS客户侧费项数据.A6A29BE637.md#doc-A6A29BE637-ois-fee-data-b7b2c0fa)。
5. 订单费用报表不导入`实付返款`和`返款汇率`。返款汇率由返款账单根据返款原始币种和返款结算币种取得并锁定，不从订单费用报表读取。
...
3. 返款账单计算出现`实付返款（准）`负数时，按[应付返款不足以覆盖指定扣减费项时怎样处理？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-negative-amount)完整执行，本节只说明归属结果：`顺延到下期返款账单`只形成负数承接记录，`反向计入本期应收账单`在负数订单所属返款账单审核通过时生成关联原返款账单、业务订单和返款原始币种的独立应收调整费项；原指定扣减费项仍归属返款账单，不得再次进入应收账单。
...
| `非费项` | 只用于呈现业务事实、返款核对或资金核对信息，不属于账单核销金额 | 返款账单详情中的回款记录等指定位置 | 不参与应收账单或返款账单的金额与核销汇总 |

`代收类`和`代付类`进入哪类账单，必须按[同一费项进入应收账单，还是返款账单？](#doc-C986F40642-fee-source-8c928b78)判定；同一费项不得同时进入应收账单和返款账单。

应收账单和返款账单中，凡以扣减方式参与账单金额计算的费项，其展示字段名必须由页面展示费项名称和紧随其后的`扣减`标签组成。`扣减`采用红色文字、红色描边、浅红底的圆角方框标签，不得作为名称前缀拼接。应收账单对`应收扣减类`费项使用该标签；返款账单对命中直接扣减配置的指定扣减费项使用该标签。例如，来源费项`满减活动优惠金额`展示为“满减活动优惠金额 + `扣减`标签”，返款侧页面展示费项`代收服务费`展示为“代收服务费 + `扣减`标签”。来源费项编码、来源费项名称和费项类型保持不变；来源名称已经以`扣减`开头时，页面展示前先去除名称前缀，再追加一个`扣减`标签。应收账单和返款账单的明细中，具体费项金额为`0`时统一显示`--`；非零费项金额按所选金额币种口径显示金额与币种。该规则不适用于应收金额、应付返款、实付返款、实收金额、已付金额、待收金额、待付金额等金额链路或汇总字段。
...
4. `非费项`只能用于展示和核对，不得计入应收账单或返款账单的金额与核销汇总。
