...
<template>
  <div class="module-page">
    ...
    <section class="module-panel filter-table-panel">
      <div class="module-toolbar"><div class="condition-filter-bar"><ConditionFilter v-model="query.keyword" label="关键词" type="text" search-placeholder="尾程运单号 / 订单 / 客户 / 返款账单" /><ConditionFilter v-model="query.recoveryStatus" label="回款状态" :options="['已回款','待回款','不回款']" /><ConditionFilter v-model="query.refundStatus" label="返款状态" :options="['已返款','待返款','不返款']" /><div class="condition-filter-actions"><el-button type="primary" @click="applyQuery">查询</el-button><el-button @click="resetQuery">重置</el-button></div></div><span class="module-result-count">{{ rows.length }} 个尾程包裹</span></div>
