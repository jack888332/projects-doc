...
`executeTask()` 使用 `TransactionTemplate` 显式开启生成事务，保证定时扫描、手工执行和同类内部调用都进入真实事务边界。

1. 账单、订单、费用、币种汇总等 BMS 写入在生成事务内完成。
2. 生成异常时回滚本次 BMS 写入。
3. 事务结束后使用 `REQUIRES_NEW` 独立记录任务 `FAILED` 和错误原因，避免失败状态随生成事务回滚。
