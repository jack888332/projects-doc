...
本文根据测试数据库中与账单配置、费项归集、账单生成、汇率、核销、返款及调账有关的结构，并结合[《账单系统 PRD》](../prd/账单系统/PRD.账单系统.第01篇.DOC-7822be08f7.md#doc-7822be08f7-overview)与[《成本中心 PRD》](../prd/PRD.成本中心.md)整理而成，用于统一 BMS 核心概念、字段口径及实体关系。
...
| 基准汇率、特调汇率 → 内部审计记录 | 基准汇率和特调汇率的新增、导入、修改、移除、确认和停用统一写入内部审计记录；页面不展示操作人和操作时间。 |
...
| 实体 | 字段 | 枚举性质 | 枚举值 | 技术约束 |
| :--- | :--- | :--- | :--- | :--- |
| `基准汇率`、`特调汇率`、`账单特调汇率` | `汇兑方向` | 条件枚举 | `左侧币种 → 右侧币种`、`右侧币种 → 左侧币种` | 与左右币种联合形成有效值。 |
| `特调汇率` | `适用层级` | 固定枚举 | `店铺级`、`客户组级`、`客户级` | 与适用对象联合形成规则适用范围。 |
| `基准汇率` | `确认状态` | 固定枚举 | `待确认`、`生效`、`停用` | 状态更新采用条件更新。 |
| `特调汇率` | `调整方式` | 固定枚举 | `百分比缩放`、`固定汇率差`、`固定汇率值` | 与调整方向和调整值联合校验。 |
| `特调汇率` | `调整方向` | 条件枚举 | `上浮`、`下浮` | 固定汇率值模式下为空。 |
| `特调汇率` | `规则状态` | 固定枚举 | `启用`、`停用` | 历史账单继续使用锁定快照。 |
| `账单特调汇率` | `汇率来源` | 固定枚举 | `财务人工调整`、`客户级特调汇率`、`客户组级特调汇率`、`店铺级特调汇率`、`基准汇率`、`店铺汇率`、`兜底汇率1`、`同币种汇率1` | 与命中层级和推导关系共同固化。 |
| `账单特调汇率` | `命中层级` | 固定枚举 | `账单人工级`、`客户级`、`客户组级`、`店铺特调级`、`基准级`、`店铺汇率级`、`兜底级` | 与汇率来源联合校验。 |
