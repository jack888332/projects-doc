...
<template>
  <div class="module-page">
    ...
    <template v-else-if="mode === 'audit'">
      ...
      <el-drawer v-model="detailVisible" size="760px" class="detail-drawer module-drawer"><template #header><div class="drawer-title"><span>审计详情</span><small>{{ selectedAudit?.objectNo }}</small></div></template><template v-if="selectedAudit"><dl class="detail-grid"><div><dt>业务模块</dt><dd>{{ selectedAudit.module }}</dd></div><div><dt>操作类型</dt><dd>{{ selectedAudit.action }}</dd></div><div><dt>操作人</dt><dd>{{ selectedAudit.operator }}</dd></div><div><dt>操作时间</dt><dd>{{ selectedAudit.time }}</dd></div><div><dt>执行结果</dt><dd>{{ selectedAudit.result }}</dd></div><div><dt>关联编号</dt><dd><el-button link type="primary" @click="action(`打开关联对象 ${selectedAudit.relation}`)">{{ selectedAudit.relation }}</el-button></dd></div><div class="span-2"><dt>操作原因</dt><dd>{{ selectedAudit.reason }}</dd></div></dl><div class="module-kpis three inline"><div class="module-kpi blue"><span>CNY 影响金额</span><strong>{{ selectedAudit.impactCny }}</strong></div><div class="module-kpi violet"><span>USD 影响金额</span><strong>{{ selectedAudit.impactUsd }}</strong></div><div class="module-kpi green"><span>关联对象数</span><strong>{{ selectedAudit.objectCount }}</strong></div></div><div class="snapshot-compare"><div><h4>前值快照</h4><pre>{{ selectedAudit.before }}</pre></div><div><h4>后值快照</h4><pre>{{ selectedAudit.after }}</pre></div></div></template></el-drawer>
    </template>
    ...
    <template v-else-if="mode === 'compare'">
      ...
      <section class="module-panel"><h3>费项映射</h3><el-table :data="feeMappings" border><el-table-column prop="finance" label="财务侧费项" /><el-table-column prop="system" label="系统侧费项" /><el-table-column label="操作" width="90"><template #default><el-button link type="primary" :icon="EditPen">调整</el-button></template></el-table-column></el-table></section>
    ...
    <template v-else>
      ...
      <section class="module-panel migration-panel">
        ...
        <h3>同步范围与批次</h3>
        <el-checkbox-group v-model="migrationScopes"><el-checkbox-button v-for="s in ['订单扩展','附加费','包裹费','费用明细','理赔单']" :key="s" :value="s" /></el-checkbox-group><el-input-number v-model="migration.batchSize" :min="100" :max="2000" :step="100" controls-position="right" style="margin-left:16px" />
        <template v-if="migrationPreviewReady"><h3>同步预览</h3><div class="module-kpis four inline"><div v-for="item in migrationPreviewStats" :key="item.label" :class="['module-kpi', item.tone]"><span>{{ item.label }}</span><strong>{{ item.value }}</strong></div></div><el-alert title="预览未写入目标环境，也未重置任何 BMS 标记。" type="info" :closable="false" /></template>
        <template v-if="migrationDone"><h3>同步结果</h3><div class="module-kpis four inline"><div v-for="item in migrationResultStats" :key="item.label" :class="['module-kpi', item.tone]"><span>{{ item.label }}</span><strong>{{ item.value }}</strong></div></div><dl class="detail-grid"><div><dt>来源筛选</dt><dd>{{ migration.sourceCustomer }} · {{ migration.orderNos }}</dd></div><div><dt>目标客户</dt><dd>{{ migration.targetCustomer }} · {{ migration.targetShop }}</dd></div><div class="span-2"><dt>同步后订单号</dt><dd>SO-260731-004188-MIG、SO-260731-004221-MIG</dd></div></dl><el-table :data="migrationResultRows" border><el-table-column prop="object" label="迁移对象" /><el-table-column prop="result" label="结果" /><el-table-column prop="reason" label="失败原因" /></el-table></template>
      </section>
    </template>
  </div>
</template>
