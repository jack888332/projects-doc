...
1. 页面顶部展示`待审核`、`待结清`、`已结清`、`已作废`等主状态，并在主状态旁独立展示`未收口`或`已收口`；`逾期未结清`作为筛选条件或标签展示，不作为主状态。
2. 账单概况区展示账单编号、客户、运抵国、账期起止日、账单发送日、信用期结束日等基础信息。
3. 金额展示区仅展示账单计算后的结果，不在本章节重复定义币种分桶、汇率锁定和换算公式。
4. 同一张账单可以同时存在多个费项结算币种桶，页面仅按<a href="#section-13">13. 金额汇兑</a>口径回显各币种桶结果。
...
5. 账单特调汇率区：锁定快照和换算结果，具体口径见<a href="#section-13">13. 金额汇兑</a>。
