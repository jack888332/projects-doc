...
- **区域字段或业务元素**：(子)单号、结算单位、成本项目、数量、计费单位、币种、税率(%)、含税单价(原币)、约定汇率、备注、附件。
- **区域级通用规则**：除所列特例外，录入字段均必填、无预设值且可编辑。

| 字段或控件特例 | 界面特例或可见反馈 |
| --- | --- |
| (子)单号 | 只能选择该订单及其关联的子单。 |
| 结算单位 | 支持手工录入关键字进行模糊匹配。 |
| 成本项目 | 支持手工录入关键字进行模糊匹配；按订单业务类型及其与成本项目的适用关系提供候选项。若缺少该业务类型所需的成本项目，联系IT或财务补充维护。 |
| 计费单位 | 手工录入。 |
| 币种 | 默认CNY；候选列表展示币种代码、币种名称2列。 |
| 税率(%) | 默认取合作方档案中对应客户结算单位的税率，可重新选择。 |
| 约定汇率 | 有默认值；默认值与联动规则见[订单成本金额与汇率规则](#doc-70AD0AF66B-section-f3a6b8c29d10)。 |
| 备注 | 选填。 |
| 附件 | 选填；可手工上传和点击查看，支持文本、图片格式；大小≤20M。 |

**界面原型概述 · 应收成本行（只读字段）**

- **区域字段或业务元素**：序号、状态、成本属性、单价参考值(人民币)、总金额(原币)、人民币金额、即期汇率、约定汇率超即期汇率(%)、更新人、更新日期、审批备注。
- **区域级通用规则**：字段均由系统生成或按业务记录带出，只读展示。

| 字段或控件特例 | 界面特例或可见反馈 |
| --- | --- |
| 序号 | 系统生成记录序号。 |
| 状态 | 新增时默认为“新建”。 |
| 成本属性 | 点击“新增应收”时默认为“应收”；费用项目为“押金”时改为“代收”。 |
| 单价参考值(人民币) | 展示、取值与带入规则见[订单成本金额与汇率规则](#doc-70AD0AF66B-section-f3a6b8c29d10)。 |
| 总金额(原币)、人民币金额 | 计算规则见[订单成本金额与汇率规则](#doc-70AD0AF66B-section-f3a6b8c29d10)。 |
| 即期汇率 | 取值与联动规则见[订单成本金额与汇率规则](#doc-70AD0AF66B-section-f3a6b8c29d10)。 |
| 约定汇率超即期汇率(%) | 计算、展示与提交提示见[订单成本金额与汇率规则](#doc-70AD0AF66B-section-f3a6b8c29d10)。 |
| 更新人 | 执行本次新增的用户。 |
| 更新日期 | 执行本次新增时的系统日期。 |
| 审批备注 | 审批后带出，默认只展示最后一次审批的备注。 |
...
| 计费单位 | 手工录入。 |
| 币种 | 默认CNY；候选列表展示币种代码、币种名称2列。 |
| 税率(%) | 默认取合作方档案中对应供应商结算单位的税率，可重新选择。 |
| 约定汇率 | 有默认值；默认值与联动规则见[订单成本金额与汇率规则](#doc-70AD0AF66B-section-f3a6b8c29d10)。 |
| 备注 | 选填。 |
| 附件 | 选填；可手工上传和点击查看，支持文本、图片格式；大小≤20M。 |
...
- **区域字段或业务元素**：序号、状态、成本属性、总金额(原币)、人民币金额、即期汇率、约定汇率超即期汇率(%)、更新人、更新日期、审批备注。
- **区域级通用规则**：字段均由系统生成或按业务记录带出，只读展示。
...
| 序号 | 系统生成记录序号。 |
| 状态 | 新增时默认为“新建”。 |
| 成本属性 | 点击“新增应付”时默认为“应付”；费用项目为“押金”时改为“代付”。 |
| 总金额(原币)、人民币金额 | 计算规则见[订单成本金额与汇率规则](#doc-70AD0AF66B-section-f3a6b8c29d10)。 |
| 即期汇率 | 取值与联动规则见[订单成本金额与汇率规则](#doc-70AD0AF66B-section-f3a6b8c29d10)。 |
| 约定汇率超即期汇率(%) | 计算、展示与提交提示见[订单成本金额与汇率规则](#doc-70AD0AF66B-section-f3a6b8c29d10)。 |
| 更新人 | 执行本次新增的用户。 |
...
- “财务审批通过”后的终点仅表示本行审批结束，不表示后续同步、对账与结算完成。财务审批通过后的金蝶同步见[结算费用新增](#doc-70AD0AF66B-section-036da482966a)，不作为本图的审批状态。
...
**补充约束**：提交及逐级审批的消息触发、接收角色和模板见[结算成本通知规则](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第068篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-817df2a7743c)。
