...
本文仅输出需求落地方案、调整计划和疑问点，**暂不修改任何代码和生产数据**。
...
6. 调整 `buildAdditionalRowsQuerySql()` / `buildAdditionalRowsQuerySqlSnapshot()`：去掉 `orderIds` 参数；旧方法仅供历史 SQL 快照展示或下线，不作为正式采集路径。
7. 调整 `markNonFeeOnlySources()`：非费项附加费同样走独立扫描。
