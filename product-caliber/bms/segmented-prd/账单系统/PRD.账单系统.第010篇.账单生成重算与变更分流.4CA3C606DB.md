...
| 账单重算 | 读取目标账单当前有效明细和定向调账记录，在锁定范围内重新计算，不扩大普通来源费项范围。 |
...
| 替换生成 | 全部受影响账单均为从未发出的`待审核`账单；不存在核销、返款登记、收款等账单侧资金事实，来源侧资金事实（如返款账单的包裹回款记录）允许存在但须保持映射只读，分类见[资金事实分为哪两类？](#doc-4CA3C606DB-billing-calculation-funds-fact-classification)；没有其他执行中任务占用相同范围；系统能够一次性确定完整的待替换账单集合。 |
...
`账单重算`跳过来源处理，读取目标账单当前有效明细和本次允许带入的调账记录。
...
4. 将本次新增或补录的费项纳入账单明细，并保留、重算归属本账单的`本期/往期账单金额冲正`等账单级调整结果，校验账单总额与明细汇总一致。
...
<a id="doc-4CA3C606DB-billing-calculation-funds-fact-classification"></a>
### 1.1 资金事实分为哪两类？

账单上的资金记录按产生侧不同分为两类；替换生成、提前收口和重算等变更边界对两类记录的处理不同。

1. `账单侧资金事实`：由本系统账单结算流程产生、与账单结算结果和状态绑定的资金结果，包括核销记录、返款登记和收款到账等。存在账单侧资金事实的账单不得替换生成或提前收口；重算、调账和折返也必须在不删除、不覆盖这些事实的前提下进行。
2. `来源侧资金事实`：由来源系统或业务环节先行形成、BMS 只读取用于归集判断、财务跟踪与分析的资金记录，例如返款账单中的包裹回款记录（尾程派送商把到付金额返还给财务的导入事实，直接挂靠尾程包裹）。它不构成账单的核销、返款或收款结果。
3. 变更边界按记录的实际作用判断：能影响核销、返款登记、收款到账和结清判断的记录必须按账单侧资金事实处理，不得因名称相近改为来源侧口径；只用于跟踪和分析的记录按来源侧资金事实处理。
4. 来源侧资金事实不阻断替换生成与提前收口，但系统必须保持其与来源导入记录、业务订单和尾程包裹的映射并只读呈现，不得删除、复制或改写。替换成功后新账单继续引用同一批来源侧资金事实，被移出账单范围的记录只更新跟踪展示，不再随新账单展示；提前收口截断后晚到的记录只更新跟踪，不改变账单结果。
5. 具体对象和输出规则由对应账单模块定义，返款账单的包裹回款记录见[COD 订单回款记录与汇兑损益怎样展示？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-remittance-712a4d3e)。

<a id="doc-4CA3C606DB-billing-calculation-9cc459c0"></a>
### 1.2 首次、补充和期末收口有何区别？

本节中的`账期结束`是时间和范围事实，`账期收口`是完成实际账期数据处理与校验后的业务结果；两者的统一定义和先后关系见[账期结束、账期收口与提前收口](PRD.账单系统.第007篇.BMS任务生命周期与操作.D94FE560DB.md#doc-D94FE560DB-task-lifecycle-984e810a)。
...
重算前必须同时锁定`目标账单`和`重算范围`。重算范围支持整张账单、选中订单、选中费项、账单特调汇率、币种分桶或已审核调账记录；未明确选择的范围不得随本次任务变化。

**重算读取什么**

1. 读取目标账单锁定的配置、费项规则、费项结算币种、原汇率快照和当前有效账单明细。
2. 读取财务本次明确提交的账单特调汇率。
3. 读取已经审核生效且明确归属于目标账单的补录费项与`金额冲正记录`：补录费项必须先形成费项池记录；金额冲正记录不进入费项池，审核通过后作为账单级结果直接计入对应冲正分桶。
...
6. 不得把重算后的最终应收金额或实付返款降到已核销金额以下，也不得直接形成无法结算的超收或超返结果。
7. 返款重算出现`实付返款（准）`负数时，按[应付返款不足以覆盖指定扣减费项时怎样处理？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-negative-amount)执行，使当前返款账单不以负数结算。
...
只要替换批次中的任一旧账单已经发出、已存在账单侧资金事实（来源侧资金事实的保留边界见[资金事实分为哪两类？](#doc-4CA3C606DB-billing-calculation-funds-fact-classification)）或不满足替换条件，整个替换批次都不得执行，不能只替换其中一部分。

**允许替换的条件**

1. 待替换账单集合中的每张账单主状态均为`待审核`。
2. 每张待替换账单的`账单发出时间`均为空，且尚未通过邮件、客户端或其他方式向客户开放查看、下载或送达。
3. 每张待替换账单均不存在核销、返款登记、收款等账单侧资金事实；来源侧资金事实（如返款账单的包裹回款记录）允许存在，但必须保持映射、只读，不复制、不删除、不修改，分类见[资金事实分为哪两类？](#doc-4CA3C606DB-billing-calculation-funds-fact-classification)。
...
1. 每条原账单有效明细在替换结果中必须有且只有一个明确去向：归入某张候选新账单、转为已审核调账记录，或标记为`配置移出待处理`。存在未确定去向或重复去向时，替换任务不得成功。
...
### 1.5 金额冲正与核销记录冲正如何参与计算？

应收与返款账单的`金额冲正记录`和`核销记录冲正`不进入`BMS费项池`：金额冲正是账单级金额结果，核销记录冲正只修正资金记录，两者都不是`账单生成`或`账单重算`的来源明细。分类、审核、归属与生效规则统一见[调账中心](PRD.账单系统.第015篇.调账中心.17F89514C9.md#doc-17F89514C9-adjustment-center-98536490)。

1. 普通来源费项与补录费项必须先形成费项池记录，再按`账单生成`或`账单重算`的任务范围参与账单明细。金额冲正记录审核通过后直接计入归属账单的`本期账单金额冲正`或`往期账单金额冲正`分桶，不得被任务当作费项池记录归集，也不得作为普通费项明细重复计入账单。
2. 尚未审核通过的金额冲正、核销记录冲正只用于调账中心和账单的调整/核销记录展示，不得进入任何账单金额、核销或导出结果。
3. 核销记录冲正按[应收与返款账单核销记录如何冲正？](PRD.账单系统.第015篇.调账中心.17F89514C9.md#doc-17F89514C9-adjustment-center-writeoff-reversal)审核生效，只更新已收/已付资金口径，不改变账单金额。
4. 两类任务都不得直接修改原费用、原核销记录或往期账单快照。
...
5. 调账、冲正、核销、回款、返款及返款负数处理结果（负数承接记录、负数承接冲抵特殊明细行、应收调整费项）的关联结果。
...
3. 账单不得存在核销、返款登记、收款等账单侧资金事实；来源侧资金事实（如返款账单的包裹回款记录）允许存在，但必须保持映射、只读，不复制、不删除、不修改（分类见[资金事实分为哪两类？](#doc-4CA3C606DB-billing-calculation-funds-fact-classification)）；未移除的金额冲正记录必须全部审核通过。
...
| 已有账单明细、账单特调汇率或已审核调账记录需要重新计算 | 账单重算 | 锁定目标账单和重算范围，只更新允许变化的计算结果，不扩大普通来源费项范围。 |
| 同时出现新费项和已有结果变化 | 先补充生成，再重算 | 先得到包含新增费项的完整结果，再以该结果为基础重算。 |
| 发生金额差异或核销记录错误 | 调账中心处理 | 通过调账中心新增金额冲正或核销记录冲正，不得由重算直接修改原费项或原核销记录。 |
...
| `已结清` | 不得重新打开或重算；通过调账中心或后续账单承接差异。 | 原订单号、原账期、原账单号、费用发生时间、归集时间、调账记录及承接账单。 |

<a id="doc-4CA3C606DB-billing-calculation-7ca5c0f8"></a>
### 3.3 返款账单还有哪些额外限制？

返款账单与应收账单的独立性、占用关系和扣减优先级见[返款账单和应收账单的关联性](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-2591ef54)，返款账单状态和`逾期未结清`边界见[返款账单状态机](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-890fe9a2)。返款重算不得改写应收账单；确需形成应收金额时，只能按返款账单配置生成独立的应收调整费项，进入应收账单的规则见[应收调整费项怎样进入应收账单？](PRD.账单系统.第011篇.应收账单.88A604AED3.md#doc-88A604AED3-receivable-bill-arr-adjustment)。
