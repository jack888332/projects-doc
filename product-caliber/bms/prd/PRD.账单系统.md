...
| 区域 | 业务字段 | 业务操作 | 关键业务约束 |
| --- | --- | --- | --- |
| 客户引用 | 仅展示查询时点存在当前有效引用的客户；字段包括客户编码、客户名称、会员编码、所属店铺、所属客户组、引用配置、配置标签；`引用配置`只显示配置编号，不显示配置名称、版本号或配置版本编号；应收配置另含分支方案数量和涉及账期类型；返款配置另含返款模式、账期类型和默认结算币种 | 查询、展开应收分支方案、更换配置、查看配置、按客户生成账单 | 客户与会员为同一主体；未配置或查询时点不存在有效引用的客户不进入本表，须从配置库的分配向导选择；列表不再重复展示引用状态和启用状态；所属店铺和所属客户组仅用于筛选与核对；同一客户、同一配置类型、同一生效时段只能有一份配置引用；应收分支方案数量和涉及账期类型按<a href="#section-7-4-1">7.4.1 应收配置列表怎样展示分支方案数量与账期类型？</a>展示 |
| 配置库 | 配置名称（选填）、当前生效配置版本编号、待生效配置版本编号及生效日期（存在时）、配置标签、当前有效引用客户数、状态；应收配置另含分支方案数量；返款配置另含返款模式和账期类型 | 新建配置、编辑并发布新版本、展开应收分支方案、分配客户、查看引用客户、按当前生效版本批量生成账单、停用 | 配置名称未填写时以配置版本编号作为主显示值；配置标签由当前有效客户引用数派生且不可编辑；新版生效时全部当前引用客户统一使用新版；分配客户只选择配置，不允许指定历史版本；应收分支方案数量按<a href="#section-7-4-1">7.4.1 应收配置列表怎样展示分支方案数量与账期类型？</a>展示 |
| 应收配置编辑 | 配置基本信息、配置版本编号、发布生效方式、默认方案、分支方案、费项结算币种、履约节点、账期类型、账单发出时间、方案生效周期、信用评级、信用期限、逾期滞纳金、垫资额度上限、合同编号、合同文件 | 新增或移除分支方案、新增或移除费项币种规则、引用费项结算币种模板、另存为新配置、发布新版本 | 编辑内容不保存为草稿；取消或关闭不产生版本；发布时默认立即生效，也可指定未来日期生效；客户需要差异化时须另存为新配置并仅切换该客户 |
| 返款配置编辑 | 配置基本信息、配置版本编号、发布生效方式、启用状态、返款模式、账期类型、半周起始日、账单发出时间、必要归集金额、直接扣减费项、货款原始币种、货款结算币种、客户收款账户、负数金额处理方式、条款生效周期 | 新增或移除币种账户规则、另存为新配置、发布新版本 | 编辑内容不保存为草稿；发布时默认立即生效，也可指定未来日期生效；账期仅支持周和半周；半周必须选择两个起始日且两个账期均不少于 3 天；币种账户规则必须保留一条末行兜底规则；已发布版本不得由客户局部覆盖 |
...
1. 应收配置和返款配置分别采用统一的可复用配置实体。配置编号必填、在同一配置类型内唯一且首次发布后不得修改；配置名称选填。除客户引用列表外，页面可以把配置编号和版本号组合展示为`配置版本编号`，格式为`配置编号-版本号`，例如`ARB-SCHEME-20260801101800123-V2`；组合展示不改变配置编号和版本号在版本快照、任务及账单中的独立记录。客户引用列表始终只显示配置编号；配置名称未填写时，配置库、分配向导、版本快照和生成弹窗以配置版本编号作为主显示值。
...
4. 客户不得在引用记录上覆盖配置版本中的单个字段，也不得指定继续使用历史版本。某个客户需要差异化时，财务须从该客户当前引用配置的生效版本执行`另存为新配置`。系统把该版本内容带入新配置编辑页；发布后形成新配置的`V1`，再通过分配向导把目标客户切换到新配置编号。原配置及其它客户引用不得因此改变。
...
1. 新建配置在本次编辑并发布成功后直接形成`V1`；已有配置从当前生效版本进入编辑，发布成功后形成下一版本。编辑过程不形成草稿记录；版本一经发布即不可修改，旧版本只用于历史任务、账单和审计追溯。
...
3. 发布时默认`立即生效`；选择`指定日期生效`时，系统保存不可修改的待生效版本，并在配置库展示配置版本编号及生效日期。到达生效时点后，系统把该版本置为当前生效版本，并将原当前版本转为历史版本。
...
1. 应收客户引用列表仅按存在当前有效引用的客户显示一条主行，未配置或查询时点不存在有效引用的客户不显示；应收配置库按配置编号显示一条主行，并组合展示当前生效配置版本编号。默认方案和每个分支方案不得重复客户、配置编号或版本信息形成多行。
2. 主行的`分支方案数量`只显示当前生效版本内已启用分支方案的数字数量；没有已启用分支时显示`0`。单元格不得附加`个分支`或其它单位文案，也不得显示默认方案的账期类型、结算币种或`仅默认方案`文案。
3. 应收客户引用列表另设`涉及账期类型`列。系统按默认方案在前、已启用分支方案顺序在后的顺序汇总账期类型，去重后使用`、`连接；停用分支不纳入汇总。账期类型值不带`账单`字样，例如默认方案为`月`、两个已启用分支分别为`月`和`周`时，显示`月、周`。
4. 财务展开主行后，系统展示默认方案编号，并按已启用分支方案逐行展示分支方案编号、订单类型、目的国、集运仓、账期类型、账单发出时间、应收账单履约节点，以及该分支费项结算币种末行兜底规则的结算币种。展开行只用于查看方案详情，不改变客户引用或配置库的主行粒度，也不提供独立发布、分配或生成账单操作。
5. 客户引用列表的分支方案数量、涉及账期类型和展开内容读取查询时点该配置编号的生效版本。指定日期待生效版本在实际生效前不得提前改变客户主行展示。
6. 配置库主行的分支方案数量和展开内容读取当前生效版本；历史版本仅在版本快照展开区查看，不改变配置库主行内容，也不得提供分配历史版本的入口。
...
返款账单配置不包含默认方案和分支方案。返款客户引用列表和返款配置库不得展示应收配置的`分支方案数量`或方案展开内容，继续按返款模式、账期类型和返款条款字段展示配置内容。
...
| 客户引用区 | 展示每个客户当前引用的特调配置及当前生效版本 | `客户（会员）`、`客户编码`、`会员编码`、`所属店铺`、`所属客户组`、`引用配置`、`配置标签`、`货币对`、`调整方式`、`特调后默认汇率`、`状态` | 按客户 / 会员、店铺、客户组、配置编号和状态筛选；选择店铺后只显示该店铺创建的客户组选项；切换配置；查看所引用配置。`引用配置`只显示配置编号，不显示配置名称、版本号或配置版本编号。 |
| 配置库区 | 维护可供一个或多个客户复用的特调规则集合 | `配置名称（选填）`、`当前生效配置版本编号`、`待生效配置版本编号及生效日期（存在时）`、`配置标签`、`当前有效引用客户数`、`规则数量`、`状态` | 新建配置、另存为新配置、编辑并发布新版本、分配客户、查看引用客户、停用；分配客户只选择配置编号，不选择历史版本。配置名称未填写时以配置版本编号作为主显示值。 |
...
3. 财务维护特调配置后，通过客户配置分配向导建立客户对配置编号的引用；同一客户、特调汇率配置类型和生效时段只能有一份引用。特调配置新版生效时，全部引用该配置的客户统一采用新版。
...
10. 新建特调配置时配置编号必填且唯一，配置名称选填；名称未填写时，客户引用区、配置库、分配向导和版本快照均以配置版本编号作为主显示值。财务配置货币对、调整方式和调整值后发布配置，再通过客户配置分配向导选择具体客户，不在规则编辑弹窗中直接保存店铺、客户组或多个适用对象。
