...
本文定义 BMS 成本中心的业务范围、供应商账单导入、成本标准化、成本归属、间接成本分摊、结清调账及利润分析要求。财务系统共享的金额汇兑、调账、非功能和审计规则，分别见[《财务系统 PRD》14. 金额汇兑](./财务系统.PRD.md)、[《财务系统 PRD》15. 调账中心](./财务系统.PRD.md)、[《财务系统 PRD》16. 非功能需求](./财务系统.PRD.md)和[《财务系统 PRD》17. 内部审计](./财务系统.PRD.md)。
## 1. 项目背景与核心痛点
...
下图只保留理解方案所需的核心实体和关系，字段级技术定义以《概念实体关系图》为准。
...
| 维护入口 | [《财务系统 PRD》9.5 客户侧费项配置工作台整合方案](./财务系统.PRD.md)。 | 成本中心的成本费项索引维护入口。 |
...
系统应区分明细、汇总、付款跟踪、报价说明和内部核对等工作表角色，默认优先识别明细工作表；识别方法见[《财务系统 PRD》18.7 Excel 数据区识别方法](./财务系统.PRD.md)。每个工作表分别展示二维表中的成本费项数量和零散数值字段数量，数量为`0`时仍须显示，便于财务判断该工作表是否包含拟入池数据。
...
5. 疑似重复仍按`5.5 校验与防错`提示财务确认，编号后缀只用于唯一识别，不作为重复判定结果。
...
5. 已形成成本账单的直接成本不得被来源数据后续变更静默覆盖；如需修正，应通过[《财务系统 PRD》15. 调账中心](./财务系统.PRD.md)形成成本调整记录。已结清成本账单不得返结清。
...
4. 切换动作必须填写原因，并记录切换前后成本类型、归属对象、分摊集、影响范围、操作人和操作时间；审计要求见[《财务系统 PRD》17. 内部审计](./财务系统.PRD.md)。
...
5. 补录成本是否纳入当前成本账单，由财务明确选择。当前成本账单处于`待结清`时，可按供应商确认结果纳入并保留调整记录；成本账单处于`已结清`时，必须通过[《财务系统 PRD》15. 调账中心](./财务系统.PRD.md)形成成本金额冲正及新的成本调整记录，不得直接覆盖历史结果。
...
财务调整分摊集处理方式、规则、拆分或合并分摊集、调整候选订单范围、调整因子或发起重算时，必须填写原因。将`已分摊`改为`不分摊`时，系统应使原有效分摊版本失效，从受影响业务订单中移除对应成本并重算利润；将`不分摊`改为`分摊至业务订单`时，必须先确认规则，再按正常流程生成新分摊版本。变更前后处理方式、不分摊原因、金额及币种、规则快照、受影响订单、操作人和操作时间均写入[《财务系统 PRD》17. 内部审计](./财务系统.PRD.md)。成本账单结清状态与处理方式相互独立，不因处理方式变更而改变。
...
6. 规则新增、复制、编辑、启用、停用和版本切换均写入[《财务系统 PRD》17. 内部审计](./财务系统.PRD.md)；分摊规则维护权限与分摊执行权限相互独立。
...
5. 分摊结果保留原始币种和原始金额；用于订单成本和利润时，再按[《财务系统 PRD》14. 金额汇兑](./财务系统.PRD.md)换算为财务本位币人民币。
...
4. 分摊规则、对象范围和有效版本的变更必须写入[《财务系统 PRD》17. 内部审计](./财务系统.PRD.md)，记录变更前后快照、变更原因、操作人和影响对象。
...
7. `已结清`成本账单发生供应商补收、减免、折让或漏项时，应通过[《财务系统 PRD》15. 调账中心](./财务系统.PRD.md)形成成本金额冲正及新的成本调整记录，不得直接改写原账单结果。
...
`已结清`表示原供应商账单在当时已经完成结清登记，是不可回退、不可覆盖的历史事实。成本账单不支持`返结清`，也不得通过修改或删除原结清记录把状态改回`待结清`。结清后的纠错统一进入[《财务系统 PRD》15. 调账中心](./财务系统.PRD.md)，并按错误对象分开处理：
...
6. 成本金额冲正、核销冲正及后续差额结清必须进入[《财务系统 PRD》17. 内部审计](./财务系统.PRD.md)，并能从原成本账单双向追溯。
...
4. 若成本账单形成后又出现供应商补收、减免、折让或漏项，待结清账单可按规则补充成本明细；已结清账单必须通过[《财务系统 PRD》15. 调账中心](./财务系统.PRD.md)承接差异，不得被静默覆盖。
