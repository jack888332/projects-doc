...
> 本篇定位：客户接口与数据交接；主要内容：客户预报、订单绑定、承运反馈、平台轨迹、节点时间推送与拼多多轨迹契约；文档角色：模块档；文档ID：58EB46CA94
...
统一接入架构见[共用能力与统一接入](../PRD.高捷物流系统.第01册.需求框架/PRD.高捷物流系统.第012篇.系统架构与非功能需求.B41CD8B36C.md#doc-B41CD8B36C-shared-capabilities)；本篇维护各对接方的具体业务契约。

字节客户通过TT平台与高捷交换大包配载预报、小包清关资料、主提单、承运重量及履约轨迹。客服和操作员在预报管理及出口订单中处理揽收、绑定、解绑、承运重量和报关通过资料；节点生成和人工维护完整规则见[在途跟踪](../PRD.高捷物流系统.第02册.订单管理/PRD.高捷物流系统.第026篇.空运-在途跟踪.8ADA3E9EFF.md#doc-8ADA3E9EFF-order-node-management)。
...
干线揽收与承运重量采用下列共同处理路径；两者的业务字段分别按本节和第3.2节填写。绑定、解绑采用前图的关联保存规则。

```plantuml
@startuml goldjet-customer-data-save-feedback
skinparam activityDiamondBackgroundColor #FFF4CC
|客服或操作员|
start
:填写资料并保存;
...
发送失败仍保留已保存的业务资料，且允许再次编辑；图中终点只表示本次保存处理结束，不表示揽收或运输完成。取消操作不保存也不发送，不进入本图。
...
“终止订单”与[综合订单](../PRD.高捷物流系统.第02册.订单管理/PRD.高捷物流系统.第013篇.综合订单管理.C97EF7E0FE.md#doc-C97EF7E0FE-lifecycle)中的订单取消、完成及服务终止尚未建立明确对应；本地终止资格、结果状态和取消承运的绑定范围待确认，不把服务终止规则直接套给整单，也不将该客户规则扩大到全部订单。
...
本表的“已完成”属于仓储服务单；[WMS出库结果回传](../PRD.高捷物流系统.第04册.仓储管理/PRD.高捷物流系统.第033篇.仓库-仓库订单管理.A77FC65070.md#doc-A77FC65070-section-8c46489e9d76)中的“已出库”属于仓储出库单。两者的同步关系、开始拣货事件标识及其触发状态待确认，不能把两类单据的状态视为同一字段。
...
本节维护总运单与关联小包裹的推送时间，查询、校验和返回行为采用[干线服务共用查询规则](../PRD.高捷物流系统.第05册.运输管理/PRD.高捷物流系统.第036篇.干线服务共用规则.76B3872565.md#doc-76B3872565-query-attachments-traceability)；各平台轨迹时间来源与文案见[客户平台路由轨迹](#doc-58EB46CA94-marketplace-route-traces)。
...
<a id="doc-58EB46CA94-pdd-bonded-contract"></a>
## 11. 拼多多保税仓轨迹契约

本章定义保税仓轨迹报送内容，节点产生与时间来源见[客户平台路由轨迹](#doc-58EB46CA94-marketplace-route-traces)。平台合作方身份partnerId、待签内容dataStr与签名分别传递，签名按MD5(dataStr+secret)约定核验，不在正文保存实际secret。

新增节点在30分钟内增量推送，每批不超过30条；来源开头的“全量”与增量规则冲突见INT-DOC-11，不把批量重发解释为新的履约事件。

### 11.1 轨迹内容与节点

每条保留订单号orderSn、轨迹ID、操作时间operateTime、状态、说明、仓库名称、监管区、申报企业及代码、仓库海关编号。申报、放行、失败节点的关区条件按契约填写；失败须提供failReason。关区条件中“FAI”与状态表“FAIL”的拼写差异见INT-DOC-11。

| 状态 | 含义 | 状态 | 含义 |
| --- | --- | --- | --- |
| APPLY | 申报 | CCLSUCCESS | 清关成功 |
| FAIL | 清关失败或异常 | ACCEPT | 接单 |
| ALLOCATE | 分配 | SORT | 分拣 |
| PACKAGE | 打包 | PRINT | 打印 |
| DEPARTURE | 出库 | OTHER | 其他 |
| CANCEL | 取消 | — | 不适用 |

| 保税清关失败码 | 原因 |
| --- | --- |
| FAIL_101、FAIL_102、FAIL_103 | 检疫审核未过、单证审核未过、海关抽检未过 |
| FAIL_104 | 订购人超过年度额度 |
| FAIL_151、FAIL_152、FAIL_153 | 海关维护、查验、订购人姓名或身份不一致 |
| FAIL_199 | 其他清关异常 |

临时维护、查验与最终退单的终态边界尚未定义，不能因同属FAIL就取消业务订单。

### 11.2 批量响应

外层成功与每条轨迹接收结果分别核对。外层0成功；9999失败、9990系统错误、9910缺签名、9920验签失败、9930解析失败、9940参数错误、9950不支持、9960非法合作方、9980未知错误保留对应原因。

逐条1000成功、1110缺必填、1120时间错误、1990未知错误；批量外层成功不能掩盖单条失败。重复推送和部分失败的恢复规则见INT-DOC-11。

<a id="doc-58EB46CA94-pdd-direct-contract"></a>
## 12. 拼多多直邮轨迹契约

### 12.1 订阅、推送与查询

拼多多调用物流方订阅接口subscribe，交接物流商ship_id、运单trackingNumber与物流订单trackingOrderNo；订阅后30分钟内补齐已有历史轨迹。新增轨迹经notify在30分钟内推送，每批最多30条。query返回当前完整轨迹，但来源同时限制最多30条，超过上限的分页或分批规则见INT-DOC-11。

城市字典cities每天获取一次，缓存城市代码、名称及刷新时间。保税仓和直邮属于不同契约，不共用同名但含义不同的FAIL二级码。

**订阅与历史补齐**

```plantuml
@startuml goldjet-pdd-direct-subscription
participant 拼多多 as PDD
participant 物流方 as Logistics
PDD -> Logistics : 订阅运单及物流订单
Logistics -> PDD : 订阅后30分钟内补齐已有历史轨迹
@enduml
```
...
```plantuml
@startuml goldjet-pdd-new-trace-delivery
participant 物流方 as Logistics
participant 拼多多 as PDD
Logistics -> Logistics : 已订阅运单产生新增轨迹
Logistics -> PDD : 30分钟内增量推送\n每批最多30条
PDD --> Logistics : 按轨迹ID与运单返回逐条结果
@enduml
```

两图分别说明历史补齐和新增推送，不表示先后执行。新增轨迹由实际事件触发，不以历史补齐成功作为已定义的前置条件。查询是独立入口，返回当前完整轨迹；超过30条的处理及失败恢复仍按INT-DOC-11保留待确认。

### 12.2 事件内容及条件字段

每条轨迹保留订单与运单、轨迹ID、一级状态、说明、发生时间operationTime、timeZone、站点名称/类型/编码、国家、省和城市代码。时区缺省+08:00，国家使用两位码、缺省CN；重量单位克。到件ARRIVAL与发件DEPARTURE成对，揽收GOT、派送SEND、签收SIGN保持业务先后关系。

| 情形 | 必须补充的资料 |
| --- | --- |
| 转单 | transferNo、transferPartnerId；未知转单公司使用180 |
| 到件、发件 | 下一站名称、类型、编码 |
| FAIL、REJECTION、IN_CABINET、OUT_CABINET、SIGN_ON_BEHALF、CLEARANCE_FINISH | 对应二级状态码 |
| SEND、SIGN_ON_BEHALF | 操作员及联系方式 |
| SIGN、SIGN_ON_BEHALF | 签收人 |
| 代签、入柜、出柜 | 代收地址 |

一级状态还包括STAY_IN_WAREHOUSE滞留仓库、OTHER其他、RETURN退回、CLEARANCE_START清关开始；完整状态保留原码，不将未识别节点强制当作签收。

### 12.3 二级状态

| 一级类别 | 二级码及含义 |
| --- | --- |
| FAIL派送异常 | 100异常天气、101电话无人接、102改派地址、103收货时间不确定或过长、104电话错误、105地址错误、106特殊区域不派送、107包装破损/污损、108系统问题、109不可抗力延迟、110其他 |
| FAIL直邮清关异常 | 601检疫审核未过、602单证审核未过、603海关抽检未过、604年度额度、651海关维护、652查验、653身份不一致、699其他清关异常 |
| REJECTION拒收 | 206强制开箱、207外包装破损、208外包装完好但内外商品不符、209包装破损且内外商品不符、210包装破损且附件/商品短少、211现场要求退回 |
| SIGN_ON_BEHALF代收 | 300代收点正常签收、301要求送货上门、302客户超过24小时未取、303客户拒签 |
| IN_CABINET入柜/代收点 | 401正常入柜、402超时未取 |
| OUT_CABINET出柜/代收点 | 501收件人取件、502快递员取消派件并取走、503管理员取件 |
| CLEARANCE_FINISH清关完成 | 101 BC、102 CC、103 BBC、104邮关、105一般贸易 |

二级契约值由类别加下划线和数字组成，例如FAIL_601；不可把直邮FAIL_101的“电话无人接”解释为保税FAIL_101的“检疫审核未过”。

### 12.4 接收结果

返回结果按轨迹ID与运单逐条关联success和reasonCode。保留未订阅1302、批次超限1206、频率限制1208、未授权1209、维护1210等原因，区分修正资料、取得订阅或恢复后再发的场景；自动重试间隔、去重及乱序处理尚未定义，见INT-DOC-11。

本章及保税仓契约的未决事项见[对接资料问题清单](../../analysis/PRD自洽性问题.md#integration-source-review)。
