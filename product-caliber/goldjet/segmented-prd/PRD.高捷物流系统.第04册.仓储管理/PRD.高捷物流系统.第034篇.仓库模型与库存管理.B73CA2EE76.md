...
客服维护仓库配置；仓库人员查询库存和标准单据；客户只处理自己的商品、仓库库存和店铺库存。作业订单的收货、理货、上架及出库操作见[仓库订单管理](PRD.高捷物流系统.第033篇.仓库-仓库订单管理.A77FC65070.md#doc-A77FC65070-section-fb20853cddc3)。
...
```plantuml
@startuml goldjet-warehouse-four-layer-model
hide empty members
hide circle
class "物理仓" as Physical
class "逻辑仓" as Logical
class "库存分配策略" as Strategy
class "共享仓" as Shared
class "渠道仓" as Channel
class "平台店铺" as Shop

Physical "1" -- "0..*" Logical : 关联
Logical --> Strategy : 库存来源
Strategy --> Shared : 按 SKU 范围和比例分配
Shared "1..*" -- "0..*" Channel : 按关联供货比例供货
Channel "0..1" -- "0..1" Shop : 绑定
@enduml
```

图中策略连线表示库存分配关系，不表示实物移库；渠道仓可在保存基本资料后维护共享仓关联，图中“1..*”表示已建立供货关系的渠道仓。店铺与渠道仓允许尚未绑定，绑定后为一对一。客户直接分配库存的替代路径见[客户库存分配与店铺转移](#doc-B73CA2EE76-customer-stock)，不得与策略分配重复累计。
