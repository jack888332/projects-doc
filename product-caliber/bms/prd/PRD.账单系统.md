...
3. 条款生效周期内不允许重叠。
4. 货款结算币种的选择、匹配和兜底规则见<a href="#section-15">15. 金额汇兑</a>。
...
返款账单模块用于管理 COD 包裹签收后的货款回收、货款返还、账单核销追踪和账单结清进度；币种、汇率与汇兑口径统一见<a href="#section-15">15. 金额汇兑</a>。
...
返款账单用于承载 COD 包裹签收后的货款回收与返还结算，统一管理签收返款和回款返款两种模式下的账单归集、核销追踪和账单结清；币种、汇率与汇兑规则统一见<a href="#section-15">15. 金额汇兑</a>。
...
2. 所有 COD 包裹均纳入回款管理；只有正常签收包裹进入回款、返款，并由回款管理计算汇兑损益，具体口径见<a href="#section-15">15. 金额汇兑</a>。
...
5. 若到付币种与回款 / 返款币种不一致，系统将回款 / 返款金额及快照交由回款管理计算汇兑损益，具体口径见<a href="#section-15">15. 金额汇兑</a>。
...
返款模式以回款返款为主流路径，签收返款作为少数客户的例外配置存在。系统需同时支持两种模式，但页面默认表达和业务描述应优先围绕回款返款展开；涉及币种换算、汇率快照和回款管理中的汇兑损益口径统一见<a href="#section-15">15. 金额汇兑</a>。
...
1. 系统识别到订单为正常签收。
2. 财务先向寄件人返还货款，金额口径与快照规则见<a href="#section-15">15. 金额汇兑</a>。
3. 系统按<a href="#section-15">15. 金额汇兑</a>计算应返金额，并将回款 / 返款金额供回款管理计算汇兑损益。
...
3. 财务确认回款到账后，系统按<a href="#section-15">15. 金额汇兑</a>锁定相关快照并计算应返金额，相关汇兑损益由回款管理计算。
...
返款账单详情页是账单视角页面，用于集中展示订单级返款账单的结算结果和核销轨迹。它只回答一张账单“怎么算、算到哪一步、核销到哪一步”，不负责判断包裹是否已回款、已返款；包裹级状态请到回款管理查看。涉及货款结算币种、回款 / 返款换算和汇率快照的规则统一见<a href="#section-15">15. 金额汇兑</a>。
...
| 返款信息列 | 返款时间、实返货款金额、返款模式、关联返款账单、应返货款金额 | 用于展示返款动作和结算结果，具体口径见<a href="#section-15">15. 金额汇兑</a>。 |
...
1. 回款管理中的回款金额、返款金额和未回款金额，均引用<a href="#section-15-7-1">15.7.1 返款账单换算总则</a>的换算结果。
...
1. 费项明细字段、动态费项展开、日期时间格式、金额币种显示格式和 3 位小数精度，与<a href="#section-19-6-1-4">19.6.1.4 Sheet 2：账单明细</a>和<a href="#section-19-6-1-6">19.6.1.6 金额、币种与精度</a>中的应收账单口径一致。
