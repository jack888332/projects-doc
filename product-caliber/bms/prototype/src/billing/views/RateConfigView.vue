...
function saveRate() {
  ...
  if (editorKind.value === 'MASTER') {
    ...
    const isNew = draft.value.sourceNo === '新配置'
    if (isNew) draft.value.sourceNo = `RATE-CUSTOM-${draft.value.customerCode}`
    const version = isNew ? 'V1' : `V${Number(draft.value.version.slice(1) || 0) + 1}`
    const saved = { ...draft.value, version, customerName:draft.value.customerName || customer?.name || '', store:draft.value.store || customer?.store || '', group:draft.value.group || customer?.group || '', sourceType:'CUSTOM', sourceName:'客户自定义特调', masterId:null, status:'启用' }
