...
客户级账单配置回答的是“某个客户按什么规则生成应收账单和返款账单”。所有配置采用同一种可复用配置实体，客户通过引用记录采用一个`配置编号 + 准确版本`；配置不会因当前被一个或多个客户引用而改变实体类型。店铺和客户组只用于批量选择客户、查询和核对，不参与账单生成时的配置匹配。账单结算主体仍为客户，但同一客户在不同店铺产生的费用必须按来源订单固化的`所属店铺快照`拆分为不同账单，不能跨店铺归集。客户级账单配置位于账单生成之前，是任务类型为`账单生成`的BMS任务读取配置快照的来源。
...
应收账单配置和返款账单配置分别建立客户引用。一个客户可以在两类配置中引用不同的配置编号和准确版本；同一客户、同一配置类型、同一生效时段只能存在一份有效引用，具体规则见<a href="#section-7-2-1">7.2.1 统一配置与引用标签</a>。

#### 7.1.1 页面原型概述

页面先按应收账单配置和返款账单配置切换，再分为客户引用、配置库和配置编辑三个业务区域：

| 区域 | 业务字段 | 业务操作 | 关键业务约束 |
| --- | --- | --- | --- |
| 客户引用 | 客户编码、客户名称、所属店铺、所属客户组、关联会员、配置编号、准确版本、生效周期、引用状态；多项归属按多值展示；应收配置另含方案概览；返款配置另含返款模式和账期类型 | 查询、展开应收分支方案、切换配置或版本、查看配置、按客户生成账单 | 运行时只按客户读取当前生效引用；店铺、客户组和会员关系仅用于筛选与核对；同一客户、同一配置类型、同一生效时段只能有一份引用；应收方案概览按<a href="#section-7-4-1">7.4.1 应收配置列表怎样展示方案概览？</a>展示 |
| 配置库 | 配置编号、配置名称、当前显示准确版本、引用标签、当前有效引用客户数、当前显示准确版本引用客户数、状态；应收配置另含方案概览；返款配置另含返款模式和账期类型 | 新建配置、发布新版本、展开应收分支方案、分配客户、查看引用客户、按当前显示的准确版本批量生成账单、停用 | 引用标签由当前有效客户引用数派生且不可编辑；发布新版本不自动升级客户；分配客户须通过向导确认具体客户清单；批量生成只读取当前引用该准确版本的客户；应收方案概览按<a href="#section-7-4-1">7.4.1 应收配置列表怎样展示方案概览？</a>展示 |
| 应收配置编辑 | 配置基本信息、配置编号与准确版本、默认方案、分支方案、费项结算币种、履约节点、账期类型、账单发出时间、方案生效周期、信用评级、信用期限、逾期滞纳金、垫资额度上限、合同编号、合同文件 | 新增或移除分支方案、新增或移除费项币种规则、引用费项结算币种模板、另存为新配置、发布新版本 | 已发布版本不得就地修改；客户需要差异化时，须另存为新配置并仅切换该客户；其它方案和币种规则采用本章约束 |
| 返款配置编辑 | 配置基本信息、配置编号与准确版本、启用状态、返款模式、账期类型、半周起始日、账单发出时间、必要归集金额、直接扣减费项、货款原始币种、货款结算币种、客户收款账户、负数金额处理方式、条款生效周期 | 新增或移除币种账户规则、另存为新配置、发布新版本 | 账期仅支持周和半周；半周必须选择两个起始日且两个账期均不少于 3 天；币种账户规则必须保留一条末行兜底规则；已发布版本不得由客户局部覆盖 |
...
1. `配置版本`保存一个配置编号在某一版本的完整内容；`客户引用记录`保存某个客户在某一时段实际采用的配置编号和准确版本。配置实体、配置版本与客户引用记录不得混为一条可变记录。
2. 客户引用唯一性按`供应链 + 客户主数据编号 + 配置类型 + 生效时段`校验。同一客户在同一应收或返款配置类型、同一时段只能引用一个配置编号的一个准确版本。店铺不是配置唯一键；同一客户在不同店铺产生的订单仍使用该客户的同一引用。账单结算主体取客户，但一张账单只能包含同一客户且同一`所属店铺快照`的费用；店铺快照不同必须拆成不同客户账单。
3. 应收账单配置和返款账单配置分别独立建立引用。客户可以分别引用不同的配置编号和准确版本。
4. 配置内容版本在`草稿`状态可以编辑，且不得被客户引用或被任务、账单读取；版本一经发布即成为不可修改的准确版本，无论是否已经生效或被引用都不得就地编辑，后续变更只能创建下一版本草稿并再次发布。版本至少记录版本状态、生效开始日期、生效结束日期、启用状态、操作人、操作时间、变更原因和内容校验值。
5. 发布新版本只创建新的配置内容版本，不自动升级任何客户。客户只有在分配向导中确认切换并形成新的客户引用记录后，才从确认的切换时点`T`开始采用该准确版本。
6. 每个任务和每张账单必须记录客户实际采用的配置编号、准确版本和客户引用记录编号。后续配置版本和引用变化不得改写历史快照。
7. 配置编号或准确版本变化只影响后续新任务；尚未发出的`待审核`账单可由财务确认影响后发起`账单生成`并选择新的配置编号和准确版本，由系统判定是否替换生成。已经发出、待结清或已结清的账单继续使用原快照，只能通过重算、调账、冲正或后续账单承接修正，不得作废或替换生成。
8. 客户邮箱属于客户主数据，不属于配置实体或配置版本；财务必须在客户主数据中维护客户邮箱，用于系统自动向客户发送账单和客户级账单配置变更通知。未维护客户邮箱时，系统不得自动发送客户邮件。
9. 保存、发布或分配配置前，系统必须校验必填项、账期、履约节点、生效周期、限定情形、方案互斥关系、费项结算币种、客户引用唯一性及其它规则是否合法且完整。任一校验不通过时，不得使该客户的新引用生效，并应定位到具体客户或配置项说明原因。
10. 普通账单任务只允许读取已经通过校验并生效的客户引用记录及其准确配置版本。唯一例外是与`替换生成`任务绑定的`待生效`客户引用：该引用只允许用于生成本次替换的候选账单，不得被其它任务读取，并须在候选账单成功切换时同步生效。不得把配置阶段能够识别的问题延迟到账单生成或重算阶段处理。
...
1. 应收配置和返款配置分别采用统一的可复用配置实体。客户引用一个准确的`配置编号 + 准确版本`，不得引用“自动跟随新版本”的动态版本。
2. 系统按配置编号统计查询时点正在生效的客户引用，并自动派生引用标签：引用客户数为 `0` 时显示`未引用配置`，为 `1` 时显示`独享配置`，大于或等于 `2` 时显示`共享配置（N）`，其中 `N` 为去重后的当前有效引用客户数。引用标签只表达引用结果，不是配置类型，不允许人工新增、修改或锁定。
3. 引用标签按配置编号跨版本统计；同一配置编号下引用不同准确版本的客户均计入该配置的当前有效引用客户数。配置库切换查看某一准确版本时，还须独立显示该准确版本在查询时点的有效引用客户数，不得用版本引用数替代配置引用标签。
4. 客户不得在引用记录上覆盖配置版本中的单个字段。某个客户需要差异化时，财务须从该客户当前有效引用记录锁定的准确版本执行`另存为新配置`，不得改取该配置编号下的其它版本。系统把该准确版本的内容复制为新配置的`V1`草稿；`V1`发布后，再通过分配向导为目标客户建立锁定`新配置编号 + V1`的引用。原配置及其它客户引用不得因此改变。
5. 店铺、客户组和会员关系遵循 OIS 主数据，仅作为分配向导的批量选客条件和客户配置、账单、任务列表的筛选与核对信息。向导按打开时的主数据把所选店铺、客户组和指定客户条件展开为具体客户，以客户主数据编号去重；同一客户存在多条当前有效归属关系时，只要任一条关系链命中筛选条件即可成为候选。店铺、客户组、会员关系和本次选客条件均不写入配置版本，也不成为运行时配置层级。
6. 向导确认后，以最终确认的具体客户清单形成逐客户引用记录。后续客户改组、会员改挂或店铺关系变化，不得自动新增、删除或切换客户引用；财务需要调整时须重新运行分配向导。
7. 任务和账单中的`配置编号`与`准确版本`是两个独立快照字段。按配置编号或准确版本筛选历史任务或账单时，必须读取任务或账单快照，不得按客户当前引用重新展开。
8. 财务在配置的准确版本上发起批量生成时，系统只展开任务创建时仍引用该准确版本的客户，并按<a href="#section-9-2-1">9.2.1 系统自动跑，还是财务手动跑？</a>形成账单生成批次和独立客户任务。配置编号和准确版本只用于批量发起、快照追溯及汇总筛选，不作为一个整体执行任务。
9. 财务按生成批次编号、任务编号、配置编号或准确版本筛选出账单后，可以对筛选结果批量下载；每张账单仍按各自客户生成独立对账文件，批量下载规则见<a href="#section-19-6-3-1">19.6.3.1 发起导出</a>。
...
配置分配、版本升级和强制切换客户均通过同一向导完成：

1. 财务选择目标配置编号、准确版本和配置切换时点`T`后，可以组合使用`店铺`、`客户组`和`指定客户`条件实时筛选候选客户；每个条件均可选择多个对象，也可以只使用其中部分条件。同一条件内任一值命中即可，不同条件之间按交集筛选；店铺和客户组同时使用时，必须由同一条当前有效的`客户—店铺—会员—客户组`关系链同时命中，不能把客户在不同关系链上的店铺和客户组交叉拼成一次命中。筛选条件只用于本次向导选客，不写入配置实体或配置版本，也不形成可复用的候选客户范围。
2. 候选客户必须按`未配置`、`已引用当前配置的其它版本`、`已引用其它配置`、`已引用当前准确版本`和`不可切换`分类展示，并以客户主数据编号去重为唯一客户行。每行分别展示`全部当前归属`和`本次命中归属`：前者列出该客户全部当前有效的所属店铺、所属客户组和关联会员，后者只列出满足本次筛选条件的关系链；同一维度存在多个值时须完整多值展示，不得只显示第一项。每行同时展示当前配置编号、当前准确版本、冲突任务或账单及可切换结论。
3. 常规打开分配向导时，系统不得默认勾选全部`未配置`客户或引用其它配置的客户。发布新版本后进入版本升级选择时，可以默认勾选`已引用当前配置的其它版本`客户，财务可以逐个或批量取消；从客户行进入配置切换时，只默认勾选该目标客户。版本升级可以选择任意数量客户，包括仅选择一个客户作为通用规则的首批推广对象；仅选择一个客户不等同于客户专属差异，也不要求另存为新配置。已引用当前准确版本和不可切换客户不得勾选或重复提交。
4. 勾选引用其它配置的客户属于强制切换。只有具备配置强制切换权限的财务人员可以执行，且必须勾选二次确认、填写变更原因；系统须展示将失效的配置编号、准确版本和受影响客户数。
5. 系统按客户逐一执行引用唯一性、目标版本完整性、切换时点`T`及通知资料校验。只有与`T`或新引用生效时段重叠的`待执行`、`执行中`、尚未删除的`执行失败`任务，或尚未完成的替换事务可以阻断切换；已完成任务和已生成历史账单只展示影响提示，由快照隔离，不得作为阻断条件。批量操作允许部分成功，但每个客户的引用切换必须原子完成，不得出现旧引用已失效而新引用未建立的中间结果。
6. 成功客户逐一结束旧引用、建立新引用和配置切换记录，并从`T`开始采用所选配置准确版本；失败客户保持原引用不变，向导结果必须返回客户、失败原因和建议处理方式，可仅重试失败客户。
7. 向导提交时固化本次实时筛选条件、候选客户清单、每个候选客户的全部当前归属与本次命中归属、最终勾选客户清单、配置编号及准确版本、`T`、强制切换标记、变更原因、操作人和逐客户结果，用于本次提交审计，不得把筛选条件或候选客户清单回写为配置适用范围。向导执行期间主数据变化不得扩大本次客户清单。
8. 只有实际改变客户配置编号或准确版本的成功客户才触发客户级账单配置变更通知；单纯发布新版本、未选择客户或客户已引用同一准确版本时不发送通知。

<div id="section-7-2-3"></div>

#### 7.2.3 配置版本、停用与引用约束

1. 新建配置先形成`V1`草稿，已有配置的变更先形成下一版本草稿；只有草稿可以编辑。版本一经发布即不可修改，旧版本继续供现有客户引用和历史快照追溯，后续变更只能再创建下一版本草稿。
2. 任何配置发布新版本前，无论当前为`未引用配置`、`独享配置`还是`共享配置（N）`，系统都必须展示按配置编号统计的全部当前有效引用客户、各客户正在使用的准确版本和计划变更摘要；没有有效引用时明确显示引用客户数为`0`。配置为`共享配置（N）`时，还必须突出提示本次发布可能影响多个客户及准确受影响人数，防止财务把多人影响误判为单客户变更。
3. 发布任何配置的新版本后，所有客户均保持原准确版本；发布流程随后进入版本升级选择，财务必须在分配向导中明确选择全部客户、部分客户或不升级客户，只有选中的客户才从确认的`T`开始改用新版本。部分升级属于通用规则的分批推广，即使本批只选择一个客户也继续使用同一配置编号的新版本，不要求另存为新配置。
4. 只有目标客户需要采用不同于通用版本的专属配置内容时，财务才执行`另存为新配置`。系统须从该客户引用记录锁定的准确版本复制内容，形成新配置的`V1`草稿；`V1`发布后通过向导把该客户切换为准确引用`新配置编号 + V1`，不得为客户在原配置版本上保存覆盖字段，也不得改变原配置的其它客户引用。
5. 被任何客户引用或被任务、账单快照引用的配置版本不得物理删除。配置停用只禁止新分配和发布新版本，不自动解除现有客户引用；停用前页面必须提示当前有效引用客户数、各准确版本引用数和待处理客户清单。
6. 配置内容、状态、发布原因和版本关系必须进入版本快照；客户引用关系、强制切换原因和逐客户执行结果单独进入引用及切换记录。
7. 同一配置编号允许不同客户在同一时点引用不同版本；系统必须始终按客户引用记录中的准确版本执行，不得因为配置存在更新版本而隐式升级。
...
应收账单配置由配置版本信息、默认方案和分支方案组成。客户侧只保存对配置编号和准确版本的引用记录，不复制或覆盖配置版本中的方案字段。
...
应收客户引用列表和应收配置库分别以客户引用和配置版本为管理对象。默认方案和分支方案是同一版本内的出账规则，不得拆成独立列表行。

1. 应收客户引用列表按客户及其当前生效引用显示一条主行；应收配置库按配置编号及列表当前显示的准确版本显示一条主行。默认方案和每个分支方案不得重复客户、配置编号或版本信息形成多行。
...
4. 财务展开主行后，系统按分支方案逐行展示分支方案序号、订单类型、目的国、集运仓、账期类型、账单发出时间、应收账单履约节点，以及该分支费项结算币种末行兜底规则的结算币种。展开行只用于查看方案概览，不改变客户引用或配置库的主行粒度，也不提供独立发布、分配或生成账单操作。
5. 客户引用列表的方案概览和展开内容读取该客户引用记录中的准确配置版本。即使同一配置编号已经发布更新版本，客户主行也不得改为展示更新版本的方案内容。
6. 配置库主行的方案概览和展开内容读取该行明确展示的准确版本。财务切换查看其它历史版本时，概览和展开内容同步切换到所选版本，不得混用不同版本的方案内容。
7. 客户尚未建立应收配置引用时，`方案概览`显示`-`，展开主行时不展示方案内容。
...
返款账单配置不包含默认方案和分支方案。返款客户引用列表和返款配置库不得展示应收配置的`方案概览`或方案展开内容，继续按返款模式、账期类型和返款条款字段展示配置内容。
...
1. 返款配置采用统一配置实体和版本化管理；客户必须保存配置编号、准确版本和客户引用记录。
...
客户级汇率配置解决的是“账单生成前，某个客户、某个货币对默认采用什么汇率”的问题。它位于账单生成之前，是账单特调汇率快照的前置配置；账单生成之后，采用的汇率会进入账单侧快照，由<a href="#section-15">15. 金额汇兑</a>继续定义换算、汇总和核销规则。特调汇率采用与账单配置相同的统一可复用配置模型，客户引用一个`配置编号 + 准确版本`；店铺和客户组仅用于批量选客和筛选，不参与运行时汇率命中。
...
客户级汇率配置页面用于让财务先维护基准汇率，再维护特调配置及其客户引用。页面划分为左右并列的两个业务区域：左侧为基准汇率表区，右侧为特调配置区，并在右侧通过页签切换`客户引用`和`配置库`；窄屏仅调整为上下排列，不改变区域字段与业务操作。
...
| 客户引用区 | 展示每个客户当前采用的特调配置准确版本 | `客户`、`所属店铺`、`所属客户组`、`关联会员`、`配置编号`、`准确版本`、`货币对`、`调整方式`、`特调后默认汇率`、`引用状态`；同一客户的多项归属按多值展示 | 按客户、店铺、客户组、配置编号和状态筛选；切换配置或版本；查看所引用配置。 |
| 配置库区 | 维护可供一个或多个客户复用的特调规则集合 | `配置编号`、`配置名称`、`当前显示准确版本`、`引用标签`、`当前有效引用客户数`、`当前显示准确版本引用客户数`、`规则数量`、`状态` | 新建配置、另存为新配置、发布新版本、分配客户、查看引用客户、停用；客户分配与版本升级使用统一向导。 |
...
3. 财务维护特调配置后，通过客户分配与版本升级向导建立客户对配置准确版本的引用；同一客户、特调汇率配置类型和生效时段只能有一份引用。
4. 客户当前特调配置中没有所需货币对或对应规则不可用时，系统先使用已确认的基准汇率兜底；若基准汇率也不可用，再使用店铺汇率表兜底；若仍找不到兜底汇率，该货币对汇率固定按 `1` 处理。
5. 特调配置的引用标签、店铺、客户组与指定客户的组合选客、版本升级、强制切换、停用和引用留痕采用<a href="#section-7-2-1">7.2.1 统一配置与引用标签</a>至<a href="#section-7-2-3">7.2.3 配置版本、停用与引用约束</a>。应收、返款和特调汇率三类配置分别建立引用，一个客户可分别引用不同配置。特调汇率引用切换本期只进入内部审计，不触发 7.2.2 第 8 条的客户账单配置变更邮件。
6. 页面所有操作均由财务执行。页面及编辑弹窗不展示操作人和操作时间；基准汇率以及特调配置、配置版本、客户分配的新增、导入、修改、发布、移除、确认和停用统一进入内部审计，规则见<a href="#section-8-7">8.7 内部审计</a>。
...
特调汇率采用统一配置实体并实行版本控制。只有草稿版本可以编辑；版本一经发布即不得就地修改，无论是否已经生效或被任务、账单引用。变更必须创建新版本草稿并重新发布，再按客户确认的切换时点生效。新版本不自动升级已有客户引用，历史账单快照不得被回写。
...
| :--- | :--- |
| 配置版本 | 客户引用的配置编号和准确版本。 |
| 客户引用 | 记录客户引用记录编号、生效时点、配置编号和准确版本。 |
...
9. 原型表格中展示的特调汇率，应理解为特调后用于账单生成的默认汇率结果；系统仍需保留其背后的配置编号、准确版本、客户引用、调整方式、调整方向和调整值。
10. 新建特调配置时必须先填写配置名称，再配置货币对、调整方式和调整值。配置发布后通过客户分配与版本升级向导选择具体客户，不在规则编辑弹窗中直接保存店铺、客户组或多个适用对象。
...
13. 特调配置的引用标签、分配向导、版本升级、强制切换和停用规则采用<a href="#section-7-2-1">7.2.1 统一配置与引用标签</a>至<a href="#section-7-2-3">7.2.3 配置版本、停用与引用约束</a>；三类配置的引用关系相互独立。特调汇率切换不发送客户账单配置变更邮件，只记录内部审计。
...
7. 特调汇率与应收、返款账单配置的客户引用关系相互独立：三类配置分别校验客户引用唯一性，一个客户可以分别引用不同的配置编号和准确版本。
...
| `账单生成批次` | 一次配置批量发起包含哪些客户任务，整体结果怎样查看？ | 财务按配置编号和准确版本批量发起时形成的非执行分组记录。批次冻结配置编号、准确版本、客户引用、客户在确认时的全部当前归属快照（所属店铺、所属客户组和关联会员）、方案标识、方案名称、方案类型、实际账期、数据截止点和创建结果，并关联本次创建的独立BMS任务；批次本身没有任务类型、执行阶段或占用范围。归属快照用于还原批量发起时的客户关系，不代替来源订单的所属店铺快照，也不得把任务范围扩大为配置级或店铺级任务。 |
...
配置准确版本批量生成只改变任务发起方式，不改变单条任务的执行流程。系统先按配置编号和准确版本冻结本批次客户、客户全部当前归属和方案范围，再按`客户 + 账单类型 + 方案标识 + 实际账期`为每个可创建范围生成独立任务；每条任务冻结所用准确版本和客户归属快照，并分别形成任务状态、快照和结果。任务仍以客户为执行主体；执行中识别出的费用按来源订单的所属店铺快照分别形成账单。
...
| 配置批量生成 | 按`客户 + 账单类型 + 方案标识（返款固定为 REFUND） + 实际账期`拆分任务范围；每条任务同时冻结配置编号、准确版本，以及客户在批次确认时的全部当前所属店铺、所属客户组和关联会员快照。一个应收客户有多个方案在本次达到生成条件时，可以在同一批次内形成多条任务。任务同时保存方案名称和方案类型用于展示；返款方案名称为`返款配置`、方案类型为`不适用`。客户归属快照用于审计和筛选，不参与配置命中；账单仍按来源订单所属店铺快照拆分。 |
...
| 配置批量生成 | 财务在配置库选择配置编号和当前显示的准确版本，并确认本次方案范围 | 系统先冻结提交参数、客户全部当前归属快照并预校验，只展开确认时仍引用该准确版本的客户；至少一个范围通过时生成唯一批次编号，并按客户、账单类型、方案标识和实际账期创建一条或多条手动`账单生成`任务。返款配置按客户和实际账期创建任务。全部范围跳过时不生成批次或任务。 |
...
用户不能独立触发`费项入池`任务，只能从客户、配置库或账单业务入口手动创建`账单生成`和`账单重算`任务。账单生成任务在同一条任务中先完成必要的来源筛选和费项入池，再计算账单；该内置阶段不得在页面上拆成可单独点击的`同步数据`操作。
...
3. 系统必须防止同一范围重复创建任务。任务身份必须记录`客户 + 账单类型 + 方案标识（返款固定为 REFUND） + 实际账期 + 配置编号及准确版本 + 任务类型`，并另存方案名称、方案类型和客户归属快照用于展示与审计。来源处理范围按`客户 + 账单类型 + 方案标识 + 配置准确版本 + 来源数据集 + 处理时间范围`判定；账单计算范围按`客户 + 账单类型 + 实际账期 + 方案标识 + 目标账单或替换批次 + 任务类型`判定，不得仅因配置编号、版本、方案名称或方案类型不同而放行同一账单结果的并发计算。任务在客户粒度占用处理范围，一个客户任务可在执行后形成多个所属店铺快照下的账单；所属店铺快照用于账单分组、唯一识别和结果追溯，不把任务拆成店铺任务，也不缩小任务创建时的客户级并发保护。`费项入池`任务占用来源处理范围；`账单生成`任务同时占用来源处理范围和账单计算范围；`账单重算`任务只占用账单计算范围。同一范围已有`待执行`、`执行中`或尚未删除的`执行失败`任务时，不得再次创建。失败任务重新执行成功或被财务删除后，才释放其占用范围。
...
8. 配置批量生成确认时，系统先按配置编号和准确版本冻结财务最终选择的客户引用、每个客户的全部当前所属店铺、所属客户组和关联会员快照、方案标识、方案名称、方案类型、实际账期和数据截止点，再逐范围执行配置生效、防重和权限预校验。至少一个范围通过时，系统在同一事务中创建批次和所有通过校验的任务，并把未通过范围记录为`跳过`；没有任何范围可创建时，不创建批次和任务，只保存本次提交校验审计并在页面保留客户、方案、账期和失败原因。任务执行时实际进入账单的费用仍按来源订单自身的所属店铺快照归集，批次确认时的客户归属快照不得覆盖订单快照。
...
| :--- | :--- | :--- |
| 费项入池 | 当前客户生效的配置编号、准确版本及客户引用记录所对应的账单配置快照；来源数据与场景取值规则快照；费项入池规则快照 | 客户、任务创建时的客户全部当前归属快照、账单类型、方案标识、方案名称、方案类型、账期、来源处理范围、数据截止点，以及各来源数据集采用的扫描方式和读取的增量同步水位。任务执行后，来源扫描结果另行记录实际命中来源订单的所属店铺快照集合。 |
| 账单生成 | 保存既有待审核账单引用的原业务快照，以及本次客户采用的配置编号、准确版本、客户引用记录、来源规则、费项规则、费项结算币种和默认汇率快照；没有既有账单时保存任务创建时生效的对应业务快照 | 是否需要收口、任务创建时的客户全部当前归属快照、来源处理范围、账单计算范围、数据截止点、已有待审核账单、本次采用的客户配置编号、准确版本及切换时点、方案标识、方案名称、方案类型，以及生成批次编号（如有），并保存各来源数据集采用的扫描方式和读取的增量同步水位；取得执行权后在执行结果中补记账单生成方式、收口结果和实际命中来源订单的所属店铺快照集合。判定为替换生成时，再形成替换批次和待替换账单集合。 |
...
提交客户分配与版本升级向导时，系统先将该客户的新引用置为`待生效`，再按以下步骤确定并校验`T`：
...
6. 配置批量分配或版本升级时，系统必须为每个客户独立计算和校验`T`。部分客户失败不回滚其它已经原子切换成功的客户，但失败客户必须保持原引用不变并返回失败原因。
...
4. 每个`账单生成`任务必须在创建时记录配置编号、准确版本、客户引用记录、客户全部当前归属快照、方案标识、方案名称、方案类型、`T`、生成批次编号（如有）、任务执行快照、计划账期和数据截止点；执行后再在任务结果中记录实际命中的来源订单所属店铺快照集合和实际处理范围。执行结果不得反向改写创建时冻结的客户归属快照。
...
1. 首次生成按任务执行快照引用的业务规则、处理范围和数据截止点筛选费项池记录，创建账单、账单编号、首版明细和首个结果版本。系统按`客户 + 所属店铺快照 + 账单类型 + 方案标识 + 配置编号 + 准确版本 + 实际账期 + 账单类型的其它分组维度`唯一识别同一张账单，不得重复创建。应收账单的其它分组维度为`业务板块 + 运抵国`，具体拆分规则以<a href="#section-10-5">10.5 自动拆单</a>为准；返款账单没有店铺之外的其它分组维度。
...
| 客户级账单配置 | 财务从客户行按客户发起账单生成，或从配置库的当前准确版本批量发起 | 创建前逐客户、逐方案和逐实际账期确认配置编号及准确版本已生效，且不存在未结束的同范围任务。配置批量发起须生成批次编号并返回创建任务与跳过范围；单客户任务和批次内任务均由系统在取得执行权后判定首次生成、补充生成或替换生成。 |
...
任务页面使用一个统一任务列表，通过`任务类型`筛选`费项入池`、`账单生成`或`账单重算`，不建立父子任务关系。配置准确版本批量发起形成的任务继续平铺展示，并通过生成批次编号关联；批次查看区只汇总关联任务，不成为父任务。只有账单生成任务展示`账单生成方式`：取得执行权前显示`待判定`，完成判定后显示`首次生成`、`补充生成`或`替换生成`；其它任务类型统一显示`不适用`。
...
| 任务列表区 | 生成批次编号、任务编号、任务状态、任务创建时间、执行耗时、任务类型<br>账单生成方式、触发方式、配置编号、准确版本、方案标识、方案名称、方案类型<br>客户名称、客户编码、关联会员、所属店铺、所属客户组、账期<br>数据截止点 | `费项入池`任务只提供详情；`账单生成`和`账单重算`任务按状态提供详情、重新执行或删除。一个客户任务关联多项归属时，关联会员、所属店铺和所属客户组按任务快照多值展示。任务执行阶段、内部收口结果、来源扫描方式和错误摘要不在列表展示。 |
| 生成批次查看区 | 生成批次编号、配置编号及准确版本、冻结客户数、客户全部当前归属快照、初始创建任务数、当前有效任务数、已删除任务数、跳过数量<br>待执行、执行中、执行成功和执行失败数量，以及包含删除审计行的关联任务清单 | 生成批次只提供范围与结果汇总，不提供整体重新执行或整体删除；失败任务从关联任务清单单独处理，删除任务保留只读审计行。 |
| 任务详情区 | 生成批次编号、任务编号、任务状态、任务创建时间、执行耗时、任务类型<br>账单生成方式、触发方式、配置编号、准确版本、客户引用记录、方案标识、方案名称、方案类型、客户、账期<br>客户全部当前归属快照、实际命中的来源订单所属店铺快照、数据截止点、原账单清单、新账单清单、结果版本、错误摘要<br>处理建议、任务执行快照、业务快照引用、替换影响、重算范围<br>来源扫描记录、来源查询语句（SQL） | 任务执行阶段、阶段记录和内部收口结果不展示；错误摘要和处理建议仅在任务详情展示。费项入池和账单生成按来源数据集展示扫描方式、扫描范围、上次成功水位、本次数据截止点、拉取数量、命中数量、扫描结果和失败原因；同一任务的不同来源数据集可以采用不同扫描方式。账单重算不读取来源数据，来源扫描记录显示`不适用`。 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 应收账单 | ARB | 客户编号 | yyyymmdd | 店铺编号+业务板块+运抵国 MD5 截前 4 位 | `ARB-OG4155-20260101-fse3` |
...
| 客户组 | 店铺下的客户归集分组，即集运客门户中的子账号分组；一个店铺对应一个集运客门户二级域名和主账号，可建立多个子账号分组，一个子账号分组可下挂多个客户，一个会员（集运客门户账号）只能同时从属一个子账号分组。同一客户主体可通过多个会员账号在一个或多个店铺开展业务。 | 客户组用于查询、报表归集和配置分配向导选客。BMS 只读取订单及客户主数据已沉淀的客户组，不重新划分客户组，也不按客户组动态决定配置引用。 |
...
7. 一个店铺可有多个客户组，一个客户组可下挂多个客户；同一客户主体可关联多个会员账号，也可通过不同会员关联一个或多个店铺。一个会员在同一时点只能从属一个客户组。客户组是 OIS 已形成的客户归集主数据，订单已沉淀所属店铺、客户组、会员和客户关系，BMS 只读取该关系。
...
1. 客户级账单配置和客户特调配置都以`供应链 + 客户主数据编号`确定客户配置主体；客户在每种配置类型和同一生效时段只能引用一个配置编号的一个准确版本。店铺、客户组和会员关系仅用于配置分配向导选客、列表筛选和结果核对，不参与运行时分级命中。账单结算主体为客户；账单生成必须保留供应链、店铺、客户组、会员和客户维度，并把来源订单的所属店铺快照纳入账单分组键与唯一识别，防止跨店铺归集。
2. 账单生成必须按订单已沉淀的主体关系归集费用，不能跨供应链、跨店铺或跨客户合并。同一客户在多个店铺产生费用时，使用同一客户配置引用，但按所属店铺快照形成不同客户账单。
...
7. 客户组作为 OIS 已形成的主数据，BMS 只读取并快照订单侧已沉淀的店铺、客户组和客户关系，不反向修改客户组及客户挂靠关系。客户组关系变化不得自动新增、删除或切换客户的配置引用。
8. 客户组即集运客门户中的子账号分组，会员所属子账号分组用于分配向导展开和账单追溯。向导必须以客户主数据编号去重为唯一客户行；同一客户关联多个店铺、客户组或会员时，`全部当前归属`完整多值展示全部当前有效关系，`本次命中归属`只展示命中本次筛选条件的关系链。任一当前有效关系链命中即可把客户纳入候选，但不得因多条关系生成重复客户行，也不得因后续改组动态改变已经确认的引用。
9. OIS 提供给分配向导的数据契约必须包含供应链编号、客户主数据编号、店铺编号及名称、客户组编号及名称、会员编号及名称、客户—店铺—会员—客户组关系、关系生效状态和更新时间。产品与 OIS 负责人须在技术设计评审前确认接口或数据视图、字段映射和刷新时效；任一必需字段缺失时不得进入分配向导联调或上线验收。
...
| 财务日常 / 核心配置 | 账单配置 | 维护统一的应收和返款配置、配置版本及客户引用，包括账期、履约节点、限定情形和币种 | <a href="#section-7">7. 客户级账单配置</a> |
| 财务日常 / 核心配置 | 汇率配置 | 维护基准汇率、统一的特调配置、配置版本及客户引用，查看客户最终默认汇率结果 | <a href="#section-8">8. 客户级汇率配置</a> |
...
10. 客户级账单配置变更通知只按本次实际成功改变配置编号或准确版本的客户清单逐客户发送。仅发布新版本、未勾选客户或客户已引用同一准确版本时不发送；配置分配、版本升级、强制切换和另存为新配置后切换客户成功时均按成功客户发送。未配置邮箱的客户记录`未配置邮箱`，不阻断其它客户发送。
...
| 当客户引用的账单配置编号或准确版本变更确认后 | `{{客户名称}} 账单配置变更通知 {{配置生效周期}}` | 尊敬的客户，您好：<br/>贵司账单配置已发生变更，配置编号：`{{配置编号}}`，配置版本：`{{配置版本}}`，生效周期：`{{配置生效周期}}`。本次变更内容：`{{配置变更摘要}}`。<br/>该变更不会自动改写已有账单；尚未向贵司发出的待审核账单如需采用新配置版本，将由我司财务确认后重新生成；已经向贵司发出的账单仍以原账单内容为准。如对配置变更内容有疑问，请及时与我司对接人员联系。 | 无 |
...
| `{{配置编号}}` | 客户本次引用的账单配置编号 | 客户级账单配置变更通知 |
| `{{配置版本}}` | 客户本次引用的准确版本号 | 客户级账单配置变更通知 |
