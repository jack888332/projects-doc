# 2026-08-11 BMS 对账报表导出增加尾程包裹数据行技术调整方案与计划（V2 已确认口径）

**版本：** V2 已确认口径（已对齐最新 PRD，并完成产品 Q1-Q10 答复确认）
**日期：** 2026-08-11
...
**文档状态：** V2 已按最新 PRD 调整并完成产品疑问确认（见 §8），待技术评审
...
5. 返款：
   - `RefundBillServiceImpl.exportData` 当前按 `businessOrderNo` 聚合成一行，但产品口径为“返款账单数据行本来就是包裹行”；
   - 返款 `fee_detail` 行按子运单落行，样例 `PCB-OG0347-20260526` 的每行 `last_mile_waybill_no` 均为单值；
   - `fee_detail.source_extra_json` 保存来源快照；`RefundBillFeeDetailDTO` 当前也没有携带 `source_id` / `source_extra_json`。
...
3. 存在“只有 `bill_waybill_no`、`sub_bill_waybill_no` 为空”的历史或部分来源行；产品已确认不存在此情况，代码仍保留防御处理。
4. 历史账单可能没有 `bill_order_waybill_snapshot` 回填；新方案需支持历史兼容或明确不做包裹行。
5. 返款样例 `PCB-OG0347-20260526`：8 行 `fee_detail` 对应 8 个不同业务订单，每行只有一个 `last_mile_waybill_no`，说明返款现有明细行已是包裹粒度，不需要再按 AR 方式做包裹拆分。
...
| G-07 | 历史快照无 `bill_order_waybill_snapshot` 时无法生成包裹行。 | 历史任务兼容需明确。 | 已确认不回退 CXMS；历史账单无快照时只输出订单行。 |
| G-08 | 返款导出按 `businessOrderNo` 聚合为一行。 | 返款数据行本来就是包裹行，客户 Sheet 2 需两级结构。 | 返款按 `businessOrderNo + 子运单号` 分组生成包裹行，并补充订单父行。 |
| G-09 | Excel 无行分组、无`>`层级标志。 | 父子分组 + 浅蓝 + `>`。 | Excel 渲染增加分组和表现规则。 |
| G-10 | CERD 新增`导出明细行快照`。 | 概念实体及行类型。 | 已确认走映射方式：`rows` JSON 落地，不新建物理表。 |
...
packageNo               包裹号（可选保留字段；包裹标识已确认使用子单号，不新增列）
...
| `lastTrajectoryStatus/Desc/Time` | 订单内最新轨迹 | 包裹自身末条轨迹（已确认） |

订单行的`子运单号`仍保存完整逗号清单；包裹行只保存当前包裹子运单号。`rows` 列表顺序即 `导出明细行快照.行顺序`。

返款行结构说明：返款现有 `fee_detail` 行即包裹行来源，按 `businessOrderNo + 子运单号` 分组；不需要像应收那样再按 `bill_order_waybill_snapshot` 拆分。
...
2. 已确认历史账单无 `bill_order_waybill_snapshot` 时**不回退 CXMS 当前关系**；该订单只输出订单行，不生成包裹行。
3. 只有 1 个子运单号的订单不生成包裹行；多于 1 个时按包裹 ID / 子运单号稳定排序生成包裹行。

新增 `BillOrderWaybillSnapshotMapper.selectByBillNo`，返回 `bill_no + business_order_no` 维度列表，应收使用；返款不依赖该表展开包裹行。
...
| 导出含义 | CXMS 候选字段 | 状态 |
| --- | --- | --- |
| 包裹标识 | `sub_waybill_no`（已确认以子单号为包裹标识，不新增列） | 已确认 |
| 子运单号 | `sub_waybill_no` | 已有 |
| 承运信息 | `logistics_name` / `carrier_product_name` / `tail_express_name` | 无异议，开发前以 CXMS DDL 终核 |
| 重量 | `gross_weight` / `settle_weight` / `pickup_weight` / `fee_weight` | 无异议，开发前以 CXMS DDL 终核 |
| 履约时间 | `measure_time` / `delivery_time` / `signed_time` 等 | 无异议，开发前以 CXMS DDL 终核 |
...
4. 产品确认不存在“只有 `bill_waybill_no`、没有 `sub_bill_waybill_no`”的无法映射情况；代码仍保留防御：万一出现，只留在订单行汇总并告警，不阻止导出。
5. 包裹行金额使用与订单行相同的 `mergeFinanceFeeCell` 逻辑，但只汇总映射到该包裹的费项；`feeCells`、`feeAmountMap`、`amount*Currency` 等字段只放包裹级结果。

返款包级费项（`sale_order_package_fee` 来源）：产品已确认返款数据行本来就是包裹行，不需要再按 AR 方式做跨库映射；直接以现有 `fee_detail` 行按 `businessOrderNo + 子运单号` 分组，同组费项汇总为该包裹行，再按业务订单生成父订单行。

### 4.5 导出数据组装流程

```mermaid
flowchart LR
    A["selectFeeDetailsByBillNo<br/>补 source_id / source_extra_json"] --> B{账单类型}
    B -- 应收 --> C["读 bill_order_waybill_snapshot<br/>无快照不回退 CXMS"]
    C --> D["按 cxms_package_id 批量查包裹详情"]
    D --> E["生成 ORDER_LINE 行<br/>保留完整汇总"]
    E --> F["子运单 > 1 时生成 PACKAGE_LINE 行<br/>按子运单映射包级费项"]
    F --> G["汇总只累加 ORDER_LINE"]
    B -- 返款 --> H["现有 fee_detail 行<br/>按 businessOrderNo + 子运单号分组"]
    H --> I["生成 ORDER_LINE + PACKAGE_LINE<br/>包裹行使用自身字段与费项"]
    I --> G
```
...
`RefundBillServiceImpl.exportData` 不再按 AR 方式做包裹拆分：直接以现有返款明细行为包裹行来源，按 `businessOrderNo + 子运单号` 分组生成 `PACKAGE_LINE`，并补充业务订单 `ORDER_LINE`。
...
- `PACKAGE_LINE`：浅蓝底样式，`业务订单号`输出`父业务订单号 >`（`>`前带一个空格），其余列只输出包裹行自身值；
...
- 客户版 Sheet 2 包裹行使用浅蓝底，与财务版一致（已确认）。
...
4. 历史账单没有 `bill_order_waybill_snapshot` 时，不回退 CXMS，只输出订单行（已确认）。

### 4.8 存储方案：已确认不新建物理表

已确认继续沿用 `bill_export_task_item.data_snapshot_json`，将 `rows[]` 作为 `导出明细行快照` 的 JSON 落地，不新建 `bill_export_task_item_line` 表：

1. 现有异步导出、重试、文件替换链路都只消费 `data_snapshot_json`，不建表改动最小；
2. 每行数据已含行类型、父订单、包裹引用和行数据，可完整还原；
3. 包裹行不会增加新金额事实，不需要独立物理表做统计或对账。

后续若出现“逐行明细独立检索”需求，再按以下结构评估扩展（当前不实施）：
...
2. `ArBillServiceImpl`：
   ...
   - 新增包级费项映射方法；
   - 汇总只累加订单行。
3. `RefundBillServiceImpl`：
   - 按 `businessOrderNo + 子运单号` 将现有返款明细行分组为 `PACKAGE_LINE`；
   - 按业务订单生成 `ORDER_LINE`，汇总只累加订单行；
   - 包裹行末条轨迹按各自子运单号查询。
...
### 阶段 0：开发前置确认（已确认业务口径，剩余 CXMS DDL 终核）

1. §8 疑问点已由产品确认，开发按确认口径执行。
2. 与数据/CXMS 负责人完成 `ob_shipping_package_header` 包裹字段映射终核。
3. 输出最终字段映射表；无法映射费项按“留在订单行并告警”的防御规则处理。
...
1. 改造 `RefundBillServiceImpl.exportData`：现有明细行按 `businessOrderNo + 子运单号` 分组为包裹行，并补充订单父行。
2. 用返款样例账单 `PCB-OG0347-20260526` 验证包级/订单级费项归属与汇总不重复。
...
1. 应收客户导出：
   ...
   - 子运单 > 1：订单行后连续输出包裹行；
   - 订单行`子运单号`为完整逗号清单；
   - 包裹行`子运单号`为单值，订单号单元格为`父订单号 >`（`>`前带一个空格）；
   - 包裹行浅蓝底，只显示自身字段与费项；
   - Sheet 1 汇总金额与当前有效账单一致，不因包裹行翻倍。
2. 返款客户导出：
   - 现有明细行按子运单号生成包裹行；
   - 同一业务订单生成订单父行，汇总只按订单父行计算；
   - 单子运单订单只输出订单行；
   - 两级结构、字段归属、汇总一致。
...
## 8. 已确认口径（产品 2026-08-11 答复）

### Q-01 包裹字段精确来源

确认：尾程包裹以【子单号】为标识，其它 CXMS 候选字段无异议。实现时以 `ob_shipping_package_header.sub_waybill_no` 作为包裹行唯一标识；重量、时间、承运字段在开发前与 CXMS DDL 做终核。

### Q-02 是否新增“包裹标识”列

确认：不需要新增列，复用现有`子运单号`列展示当前包裹。

### Q-03 订单行金额口径

确认：正确。订单行金额包含包级费项的订单汇总值，包裹行展示该包裹自身金额，两层不求和。

### Q-04 包级费项无法精确映射

确认：不存在此情况。代码仍保留防御：万一出现无法映射，只留在订单行汇总并告警，不阻止导出。

### Q-05 `>` 格式与默认折叠

确认：默认展开；`业务订单号`单元格展示`业务订单号 >`（`>`前带一个空格）。展开/收起包裹行不影响金额。

### Q-06 客户版包裹行底色

确认：客户版 Sheet 2 的包裹行使用浅蓝底，与财务版一致。

### Q-07 历史账单兼容

确认：不接受回退 CXMS。历史账单无 `bill_order_waybill_snapshot` 时只输出订单行，不生成包裹行；任务可记录“历史账单未展开包裹行”的诊断信息。

### Q-08 是否新建导出明细行物理表

确认：走映射方式。`导出明细行快照`映射为 `bill_export_task_item.data_snapshot_json.rows[]`，不新建物理表。

### Q-09 返款是否全量展开

确认：返款账单数据行本来就是包裹行。返款按 `businessOrderNo + 子运单号` 将现有明细行分组为 `PACKAGE_LINE`，并补充 `ORDER_LINE`；单子运单订单只输出订单行。

### Q-10 包裹行末条轨迹

确认：按推荐方案。包裹行按各自子运单号查询 LFC 末条轨迹，不复用订单级最新轨迹；轨迹查询失败仍降级为空，不影响导出。
