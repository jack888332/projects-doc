...
3. 账单不得存在核销、返款登记、收款等账单侧资金事实；来源侧资金事实（如返款账单的包裹回款记录）允许存在，但必须保持映射、只读，不复制、不删除、不修改（分类见[资金事实分为哪两类？](#doc-4CA3C606DB-billing-calculation-funds-fact-classification)）；冲正记录必须全部审核通过。
