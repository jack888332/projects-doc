...
返款相关金额同时存在于 OIS 输出的订单级机算字段和订单费用报表导入形成的包裹级金额中。OIS 订单侧机算形成并直接挂靠业务订单的`cod_price`（到付金额）与`dest_country_surcharge_amount`（到付附加费总额），是 BMS 计算订单级`应付返款（即代收货款）= cod_price - dest_country_surcharge_amount`的输入；`collection_price`等订单侧字段保留用于核对，不作为应付返款计算输入。`ofp_ofdb1.sale_order_package_fee`中的所有金额均为尾程包裹级导入值，不是 OIS 机算值。包裹侧只导入新的实收回款、回款汇率和包裹级手续费等事实；历史实付返款和返款汇率只保留存量原记录。
...
| `ofp_ofdb1.sale_order_header.cod_price` | 收件人侧 COD 到付总额 | 作为订单级到付金额计算输入，与`dest_country_surcharge_amount`相减得到`应付返款（即代收货款）`；该金额可能同时包含代收货款本金和到付附加费，不等于`应付返款（即代收货款）`。 |
| `ofp_ofdb1.sale_order_header.cod_amount` | 历史字段中文名为`货到付款手续费` | 不得仅凭字段名直接认定为手续费；必须结合具体订单和包裹侧事实判断。 |
| `ofp_ofdb1.sale_order_header.dest_country_surcharge_amount` | 订单侧到付附加费总额 | 作为订单级到付附加费计算输入，从`cod_price`（到付金额）中扣除，确定`应付返款（即代收货款）`；不作为客户应收费用进入应收账单。 |
...
6. 返款账单详情须保留业务订单号、尾程运单号、当前采用的返款计算来源字段、来源金额、来源币种和最终账单计算金额，便于财务追溯返款计算结果。包裹级回款记录作为订单费用报表导入的核对事实，在返款账单详情的订单返款数据行展开内容中按包裹逐行展示，展示规则见[COD 订单回款记录与汇兑损益怎样展示？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-remittance-712a4d3e)；本节只定义来源字段含义，不重复定义展示区域字段。
7. 订单级`应付返款`一律按`cod_price - dest_country_surcharge_amount`计算；`collection_price`只保留用于核对，不得作为应付返款计算输入或覆盖公式结果。系统不对`collection_price`与公式结果的取值差异进行异常提示或自动处置，默认各来源字段本身为正确的订单侧业务事实，差异仅保留用于核对。
8. COD 业务订单的返款计算以业务订单为单位：`cod_price`、`dest_country_surcharge_amount` 等订单级金额不按尾程包裹拆分；一票多件中部分包裹异常签收时整票仍按订单级金额计算，业务订单全部尾程包裹均异常签收时整单不进入返款计算，口径见[核心概念](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-e15cb291)。任一计算输入缺失或不可计算时，该订单不进入本期返款账单，按[返款金额从哪里取？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-3b9f382c)处理。
