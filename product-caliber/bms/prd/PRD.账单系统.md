...
| 区域 | 业务字段 | 业务操作 | 关键业务约束 |
| --- | --- | --- | --- |
| 客户配置 | 客户编码、客户名称、店铺、客户组、配置来源、自定义配置编号或母版编号、准确版本、生效周期、状态 | 查询、新建或编辑自定义配置、基于当前母版复制为自定义配置、查看所引用母版、按客户生成账单 | 运行时只按客户读取配置；店铺和客户组仅用于筛选；同一客户、同一配置类型、同一生效时段只能有一个来源 |
| 共享配置母版 | 母版编号、母版名称、版本、可分配客户范围、当前引用客户数、默认结算币种、账期、状态 | 新建母版、发布新版本、分配客户、查看引用客户、停用 | 母版必须版本化；发布新版本不自动升级已有客户；分配客户须通过向导展开并确认具体客户清单 |
...
6. 母版范围变化不自动解除既有引用。已经引用但不再处于可分配范围内的客户继续使用原准确版本，并在母版引用列表标记`范围外引用`；该客户不得升级到母版新版本，直至财务扩展范围或把客户切换为其它来源。
7. 店铺和客户组仍可用于客户配置、账单和任务列表筛选，但筛选不改变客户配置来源、账单归属客户、账单编号或客户对账口径。
8. 任务和账单中的`配置编号`按已固化快照解释：自定义来源记录客户自定义配置编号，母版来源记录母版编号及版本。按配置编号筛选历史任务或账单时，必须读取任务或账单快照，不得按客户当前引用重新展开。
9. 财务按任务编号或配置编号筛选出账单后，可以对筛选结果批量下载；每张账单仍按各自客户生成独立对账文件，批量下载规则见<a href="#section-19-6-3-1">19.6.3.1 发起导出</a>。
10. 存量客户独享配置迁移为该客户的自定义配置。存量店铺级或客户组级共享配置迁移为共享配置母版，并仅在迁移预览中按原规则计算建议客户清单；财务确认后逐客户生成引用记录。迁移完成后，系统不得继续使用店铺级、客户组级或客户级的运行时优先级匹配。
...
2. 候选客户必须按`未配置`、`已引用当前母版的其它版本`、`已引用其它母版`、`使用自定义配置`、`已引用当前准确版本`和`不可切换`分类展示；每行展示客户、店铺、客户组、当前来源、当前准确版本、冲突任务或账单及可切换结论。
...
5. 系统按客户逐一执行来源唯一性、可分配范围、版本完整性、旧任务和旧账单占用、切换时点`T`及通知资料校验。批量操作允许部分成功，但每个客户的来源切换必须原子完成，不得出现旧来源已失效而新引用未建立的中间结果。
...
| 客户特调配置区 | 展示每个客户当前采用的特调来源和准确版本 | `客户`、`店铺`、`客户组`、`配置来源`、`自定义配置编号或母版编号`、`版本`、`货币对`、`调整方式`、`特调后默认汇率`、`状态` | 按客户、店铺、客户组、来源和状态筛选；新增或编辑客户自定义配置；基于母版复制为自定义配置；查看所引用母版。 |
...
5. 母版可分配客户范围和分配向导沿用<a href="#section-7-2-1">7.2.1 客户配置来源与共享配置母版</a>及<a href="#section-7-2-2">7.2.2 母版分配客户向导</a>的通用规则。应收、返款和特调汇率三类配置分别建立引用，一个客户可分别引用不同母版。
...
12. 存量客户特调汇率迁移为原所属客户的自定义特调配置；存量店铺级或客户组级特调规则迁移为共享特调汇率母版，并由财务在迁移预览中确认具体客户引用清单。
13. 共享特调汇率母版的可分配范围、分配向导、版本升级、强制覆盖、停用和范围外引用规则沿用<a href="#section-7-2-1">7.2.1 客户配置来源与共享配置母版</a>至<a href="#section-7-2-3">7.2.3 母版版本、停用与引用约束</a>；三类配置的引用关系相互独立。
...
账单生成成功后，系统必须把客户、配置来源类型、客户自定义配置编号及版本或母版编号及母版版本、客户引用记录编号、命中的货币对规则、基准汇率、店铺汇率、汇率来源、调整方式、调整方向、调整值、推导关系和计算后的默认汇率一起固化到账单特调汇率快照中。若命中的是固定汇率值，基准汇率和店铺汇率可记录为`不适用`，但固定汇率值和准确配置来源必须进入快照。后续客户级汇率配置变化，只影响后续新账单和仍允许账单侧重算的账单；店铺汇率表变化不触发 BMS 任何动作；二者均不得回写历史账单。
...
| `账单分组键` | 在同一客户、账单类型、实际采用的配置来源及准确版本、配置方案和实际账期内，用于继续区分具体账单的业务维度组合。 | 应收账单使用`业务板块 + 运抵国`；分组键不同必须形成不同账单。 |
...
| :--- | :--- | :--- |
| 费项入池 | 当前客户生效的账单配置来源快照，包括自定义配置版本或母版准确版本及客户引用记录；来源数据与场景取值规则快照；费项入池规则快照 | 客户、账单类型、配置方案、账期、来源处理范围、数据截止点，以及各来源数据集采用的扫描方式和读取的增量同步水位。 |
| 账单生成 | 保存既有待审核账单引用的原业务快照，以及本次客户采用的来源类型、配置或母版准确版本、客户引用记录、来源规则、费项规则、费项结算币种和默认汇率快照；没有既有账单时保存任务创建时生效的对应业务快照 | 是否需要收口、来源处理范围、账单计算范围、数据截止点、已有待审核账单、本次采用的客户配置来源及切换时点，以及各来源数据集采用的扫描方式和读取的增量同步水位；取得执行权后补记账单生成方式和收口结果。判定为替换生成时，再形成替换批次和待替换账单集合。 |
...
3. 每张账单须记录配置来源、准确版本、客户引用记录、实际账期、账期类型、是否为截断账期和对应的配置切换记录。
4. 每个`账单生成`任务必须记录配置来源、准确版本、客户引用记录、`T`、任务执行快照、计划账期、数据截止点和实际处理范围。
...
| 客户级账单配置 | 财务发起账单生成，且目标客户范围尚无有效账单 | 创建前逐客户确认最终配置来源及准确版本已生效，且同一客户、账单类型、实际账期和配置方案不存在未结束的同范围任务。批量发起时允许剔除失败客户继续创建其它客户任务，但必须返回逐客户结果。 |
...
| 任务筛选区 | 任务编号、任务类型、配置来源、配置编号或母版编号、准确版本、客户名称、客户编码<br>会员编码、店铺、任务状态、账单生成方式、触发方式<br>账期开始日期、账期结束日期 | 配置编号筛选读取任务执行快照，自定义来源匹配自定义配置编号，母版来源匹配母版编号及版本，不按客户当前引用展开。组合筛选同时作用于统计区和任务列表。 |
| 任务列表区 | 任务编号、任务状态、任务创建时间、执行耗时、任务类型<br>账单生成方式、触发方式、配置来源、配置编号或母版编号、准确版本、配置类型<br>客户名称、客户编码、会员编码、店铺、账期<br>数据截止点 | `费项入池`任务只提供详情；`账单生成`和`账单重算`任务按状态提供详情、重新执行或删除。任务执行阶段、内部收口结果、来源扫描方式和错误摘要不在列表展示。 |
| 任务详情区 | 任务编号、任务状态、任务创建时间、执行耗时、任务类型<br>账单生成方式、触发方式、配置来源、配置编号或母版编号、准确版本、客户引用记录、客户、账期<br>数据截止点、原账单清单、新账单清单、结果版本、错误摘要<br>处理建议、任务执行快照、业务快照引用、替换影响、重算范围<br>来源扫描记录、来源查询语句（SQL） | 任务执行阶段、阶段记录和内部收口结果不展示；错误摘要和处理建议仅在任务详情展示。费项入池和账单生成按来源数据集展示扫描方式、扫描范围、上次成功水位、本次数据截止点、拉取数量、命中数量、扫描结果和失败原因；同一任务的不同来源数据集可以采用不同扫描方式。账单重算不读取来源数据，来源扫描记录显示`不适用`。 |
...
5. 应收账单列表支持按店铺、客户组、任务编号和配置编号筛选客户级账单；配置编号按账单快照中的客户自定义配置编号或母版编号及版本匹配，不读取客户当前引用。财务按任务编号或配置编号筛选出账单后，可以对筛选结果批量下载，规则见<a href="#section-19-6-3-1">19.6.3.1 发起导出</a>。
...
5. 财务按任务编号或账单快照中的配置编号筛选出应收账单后，可以对筛选结果批量下载：`导出给客户`逐客户生成对账文件并打包下载，`导出给内部`按筛选结果全部账单生成内部明细文件，打包规则见<a href="#section-19-6-3-1">19.6.3.1 发起导出</a>。
...
5. 返款账单列表支持按店铺、客户组、任务编号和配置编号筛选客户级账单；配置编号按账单快照中的客户自定义配置编号或母版编号及版本匹配，不读取客户当前引用。财务按任务编号或配置编号筛选出返款账单后，可以对筛选结果批量下载，每张账单仍按客户生成独立对账文件，规则与应收账单一致，见<a href="#section-19-6-3-1">19.6.3.1 发起导出</a>。
...
2. 同一客户、同一账期、同一账单类型和同一配置方案的账单生成流程不得并发执行；客户是否引用同一母版不扩大并发锁范围。已有任务执行中时，后续任务应提示被占用或排队处理。
...
1. 客户自定义账单配置、共享账单配置母版及版本发布、客户分配与强制覆盖、客户切换为自定义配置、基准汇率表、客户自定义特调配置、共享特调汇率母版及版本发布、客户分配与强制覆盖、账单内锁定汇率、客户侧费项索引、成本费项索引、间接成本分摊规则、导入设置快照中的费项映射、场景取值规则、数据源规则和供应商财务档案变更。
...
12. 按任务编号或配置编号筛选后的批量下载：财务在应收账单或返款账单列表按`任务编号`或`配置编号`筛选出账单后，可以对筛选结果批量下载，无需逐张勾选；配置编号基于账单已固化快照匹配，自定义来源使用客户自定义配置编号，母版来源使用母版编号并可继续限定版本，客户当前引用变化不得改变结果。应收账单仍可选择`导出给客户`或`导出给内部`，返款账单固定为`导出给客户`。系统在导出任务中固化本次筛选条件和账单清单（例如`任务编号 / BMS-20260731-0001`或`母版编号 / ARB-MASTER-20260701-01 / V2`），执行导出时不得按客户当前引用重新展开；压缩包内各客户账单仍分别生成独立 Excel，文件名以客户编号或账单编号区分，不得重复。
