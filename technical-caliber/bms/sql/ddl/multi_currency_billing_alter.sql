...
CREATE TABLE IF NOT EXISTS `bill_config_fee_currency_rule` (
  ...
  `charge_currency_mode` varchar(32) NOT NULL DEFAULT 'CONFIG_DEFAULT' COMMENT '结算币种模式：CONFIG_DEFAULT账单默认币种，SOURCE来源币种，FIXED固定币种',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='账单配置费项结算币种规则';
...
ALTER TABLE `payment_writeoff_detail`
  ADD COLUMN `currency_summary_id` bigint(20) unsigned DEFAULT NULL COMMENT '核销的结算币种汇总ID' AFTER `bill_no`,
