...
2. 先处理费项：确认本期账单已生成后，财务先形成补录、调账或冲正记录，再按规则补充生成或重算账单，之后分别处理应收账单和返款账单。
...
上述顺序只规定财务日常推进方式，各环节的完整规则分别见[账单生成/重算机制](PRD.账单系统.第007篇.BMS任务生命周期与操作.D94FE560DB.md#doc-D94FE560DB-task-lifecycle-48b1e1bb)、[应收账单模块](PRD.账单系统.第011篇.应收账单.88A604AED3.md#doc-88A604AED3-receivable-bill-b1671b99)、[返款账单模块](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-09993f61)和[调账中心](PRD.账单系统.第015篇.调账中心.17F89514C9.md#doc-17F89514C9-adjustment-center-98536490)。
...
| 费项处理 | 形成补录、调账或冲正记录，再按规则补充生成或重算账单。 | 费项、金额、币种、汇率、扣减项和冲正结果是否完整、准确。 | 可供导出核对的账单 |
...
主流程以“配置确认 → 账单生成确认 → 费项处理 → 分线导出、审核、客户对账和核销”为固定顺序。流程图将应收账单和返款账单表示为两个并行分支，两条账单线可以同时推进，但无需同时完成。子流程只在下表所列的配置变更、账单有误、客户异议或资金差异等场景下触发。

<a id="doc-C108D6E5CA-daily-sop-ae13abbf"></a>
#### 1.4.1 配置确认至账单核销的财务主流程
...
子流程只在下表所列场景下触发，不属于主流程的必经节点。子流程处理完成后，应回到配置确认、账单确认、费项处理、账单导出、客户对账或账单核销中的对应节点继续主流程。
...
| 通知失败 / 未配置邮箱 | 账单审核通过后，`通知状态`为`通知失败`或`未配置邮箱`。 | 确认客户邮箱配置并按[账单邮件](PRD.账单系统.第017篇.账单通知.F82B342382.md#doc-F82B342382-bill-email)重新发送；不得因通知失败重新生成账单。 | 客户对账 |
...
| 导出字段定制 | 客户要求在默认对账报表外追加辅助核对字段。 | 按[对账报表导出格式](PRD.账单系统.第016篇.账单导出.29F2A66D22.md#doc-29F2A66D22-notification-export-c1ad8a3c)处理，追加字段不得改变账单金额口径。 | 账单导出 |

财务 SOP 确定了配置确认、费项处理、账单导出、审核、客户对账和核销的操作顺序。客户出账和返款口径分别由[客户级账单配置](PRD.账单系统.第005篇.客户级账单配置.B6349C60A4.md#doc-B6349C60A4-billing-config-d161171a)和[客户级汇率配置](PRD.账单系统.第006篇.客户级汇率配置.72D3AF6045.md#doc-72D3AF6045-rate-config-809a91c4)定义。
