...
**概念模型：** [数据字典 · 基础主数据实体](../PRD.高捷物流系统.第8册.平台共用与运营/PRD.高捷物流系统.第063篇.数据字典.7A3D9E1F50.md#doc-7A3D9E1F50-domain-master-data)、[数据字典 · 财务结算实体](../PRD.高捷物流系统.第8册.平台共用与运营/PRD.高捷物流系统.第063篇.数据字典.7A3D9E1F50.md#doc-7A3D9E1F50-domain-finance)
...
角色的操作授权见[财务结算操作权限](../PRD.高捷物流系统.第8册.平台共用与运营/PRD.高捷物流系统.第064篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-finance-actions)。本篇保留结算流程中的场景分工：
...
币种数据词典见 [数据字典 · 币种数据词典](../PRD.高捷物流系统.第8册.平台共用与运营/PRD.高捷物流系统.第063篇.数据字典.7A3D9E1F50.md#doc-7A3D9E1F50-values-enum-currency)。
...
- **区域字段或业务元素**：部门编码、部门名称、一级成本项目、二级成本项目、明细成本项目、顺序、创建人、创建日期。
- **区域级通用规则**：字段均只读；按查询条件展示匹配记录；未设置查询条件时展示全部记录。
...
- **区域字段或业务元素**：部门编码、一级成本项目、二级成本项目、明细成本项目、顺序。
- **区域级通用规则**：除所列特例外，字段均必填、无预设值、可编辑。

| 字段或控件特例 | 界面特例或可见反馈 |
| --- | --- |
| 部门编码 | 候选列表展示2列（编码、名称）；候选范围见[财务数据访问范围](../PRD.高捷物流系统.第8册.平台共用与运营/PRD.高捷物流系统.第064篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-finance-scope)。 |
...
- **区域级通用规则**：字段均只读；在[财务数据访问范围](../PRD.高捷物流系统.第8册.平台共用与运营/PRD.高捷物流系统.第064篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-finance-scope)内按查询条件筛选，未设置查询条件时展示范围内全部记录。
...
- 保存校验：必填字段未填写，或该组织下不存在基本户标识为“Y”的银行账户时，无法保存。
- 基本户变更：保存成功后，若新增账户的基本户标识为“Y”，则实时将该组织原基本户的标识改为“N”，保持一个组织仅一个基本户。
- 保存结果：根据当前账户记录“所属的公司/部门”“更新人”和“更新日期”，新增银行账户的状态为“已生效”。
- 界面反馈：新增完成后，自动返回“银行账户列表”界面。
