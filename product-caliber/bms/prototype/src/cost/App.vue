...
import ConditionFilter from '../billing/components/ConditionFilter.vue'
import HoverActionMenu from '../billing/components/HoverActionMenu.vue'
import MetricGrid from '../billing/components/MetricGrid.vue'
import PageHeader from '../billing/components/PageHeader.vue'
import SegmentedControl from '../billing/components/SegmentedControl.vue'
import StackedCell from '../billing/components/StackedCell.vue'
import StatusTag from '../billing/components/StatusTag.vue'
import TablePagination from '../billing/components/TablePagination.vue'
...
const numeric = (value) => Number(String(value ?? 0).replaceAll(',', '').replace(/[A-Z]/g, '').trim()) || 0
const formatAmount = (value, decimals = 3) => Number(value || 0).toLocaleString('en-US', { minimumFractionDigits: decimals, maximumFractionDigits: decimals })
...
function contains(row, fields = []) {
  ...
  return !keyword || fields.some((field) => String(row[field] || '').toLowerCase().includes(keyword))
}
function matchesCommon(row) {
  const board = row.board?.endsWith('成本') ? row.board : row.board ? `${row.board}成本` : ''
...
const overviewKpis = computed(() => {
  const byCurrency = (currency) => bills.value.filter((row) => row.currency === currency).reduce((sum, row) => sum + numeric(row.amount), 0)
  return [
...
const billKpis = computed(() => [
  ...
  { label: '账单金额', value: money(numeric(selectedBill.value.amount), selectedBill.value.currency), extra: selectedBill.value.period, tone: 'amber' },
  { label: '已结清金额', value: money(numeric(selectedBill.value.settled), selectedBill.value.currency), extra: selectedBill.value.state, tone: 'green' },
])
