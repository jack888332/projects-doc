...
客户级账单配置用于集中维护客户出账和返款的前置规则。应收账单模块和返款账单模块不再各自重复定义配置口径，统一从本章读取客户最终采用的配置来源、准确版本、账期规则、币种规则、履约节点、返款模式和生效周期。

<div id="section-7-1"></div>

### 7.1 章节概述

客户级账单配置回答的是“某个客户按什么规则生成应收账单和返款账单”。运行时配置只下挂客户，不再按店铺、客户组和客户设置多级命中关系。客户可以维护独立的自定义配置，也可以引用一份共享配置母版；店铺和客户组只用于批量选客、查询和核对，不参与账单生成时的配置匹配。它位于账单生成之前，是任务类型为`账单生成`的BMS任务读取配置快照的来源。
...
应收账单配置和返款账单配置分别维护来源，一个客户可以在两类配置中采用不同母版；同一客户在同一配置类型下只能采用`自定义配置`或`共享配置母版`中的一种来源，具体规则见<a href="#section-7-2-1">7.2.1 客户配置来源与共享配置母版</a>。

#### 7.1.1 页面原型概述

页面先按应收账单配置和返款账单配置切换，再分为客户配置、共享配置母版和配置编辑三个业务区域：

| 区域 | 业务字段 | 业务操作 | 关键业务约束 |
| --- | --- | --- | --- |
| 客户配置 | 客户编码、客户名称、所属店铺、所属客户组、配置来源、自定义配置编号或母版编号、准确版本、生效周期、状态；应收配置另含方案概览；返款配置另含返款模式和账期类型 | 查询、展开应收分支方案、新建或编辑自定义配置、基于当前母版复制为自定义配置、查看所引用母版、按客户生成账单 | 运行时只按客户读取配置；店铺和客户组仅用于筛选；同一客户、同一配置类型、同一生效时段只能有一个来源；应收方案概览按<a href="#section-7-4-1">7.4.1 应收配置列表怎样展示方案概览？</a>展示 |
| 共享配置母版 | 母版编号、母版名称、版本、可分配客户范围、当前引用客户数、状态；应收母版另含方案概览；返款母版另含返款模式和账期类型 | 新建母版、发布新版本、展开应收分支方案、分配客户、查看引用客户、按当前显示的准确版本批量生成账单、停用 | 母版必须版本化；发布新版本不自动升级已有客户；分配客户须通过向导展开并确认具体客户清单；批量生成只读取当前引用该准确版本的客户，不按母版可分配范围重新匹配；应收方案概览按<a href="#section-7-4-1">7.4.1 应收配置列表怎样展示方案概览？</a>展示 |
| 应收配置编辑 | 配置来源、客户或母版基本信息、配置编号与版本、默认方案、分支方案、费项结算币种、履约节点、账期类型、账单发出时间、方案生效周期、信用评级、信用期限、逾期滞纳金、垫资额度上限、合同编号、合同文件 | 新增或移除分支方案、新增或移除费项币种规则、引用费项结算币种模板、保存自定义配置或发布母版新版本 | 引用母版的客户不得在客户侧覆盖单个字段；需要差异化时，须先复制当前母版内容形成客户自定义配置并解除引用；其它方案和币种规则沿用本章既有约束 |
| 返款配置编辑 | 配置来源、客户或母版基本信息、配置编号与版本、启用状态、返款模式、账期类型、半周起始日、账单发出时间、必要归集金额、直接扣减费项、货款原始币种、货款结算币种、客户收款账户、负数金额处理方式、条款生效周期 | 新增或移除币种账户规则、保存自定义配置或发布母版新版本 | 账期仅支持周和半周；半周必须选择两个起始日且两个账期均不少于 3 天；币种账户规则必须保留一条末行兜底规则；客户引用母版时不得局部覆盖母版字段 |
...
客户级账单配置采用“配置内容版本 + 客户引用记录 + 任务与账单快照”的分层版本模型。

1. `客户自定义配置版本`保存一个客户独立维护的配置内容；`共享配置母版版本`保存可供多个客户复用的配置内容；`客户引用记录`保存某个客户在某一时段实际采用的来源类型、准确配置编号和准确版本。三者不得混为一条可变记录。
2. 唯一性按`供应链 + 客户主数据编号 + 配置类型 + 生效时段`校验。同一客户在同一应收或返款配置类型、同一时段只能采用一个来源，不允许同时引用多个母版，也不允许在引用母版的同时保留生效中的自定义配置。店铺不是配置唯一键；同一客户在不同店铺产生的订单仍使用该客户的同一配置来源，但账单继续按既有主体和店铺规则分别归集。
3. 应收账单配置和返款账单配置分别独立。客户可以在应收配置引用母版、在返款配置使用自定义配置，反之亦可。
4. 配置内容版本一旦生效并被任务或账单引用，不允许就地编辑，只能创建新版本。版本至少记录生效开始日期、生效结束日期、启用状态、操作人、操作时间、变更原因和内容校验值。
5. 发布母版新版本只创建新的母版内容版本，不自动升级任何客户。客户只有在分配向导中确认切换并形成新的客户引用记录后，才从确认的切换时点`T`开始采用该母版新版本。
6. 每个任务和每张账单必须记录客户实际采用的来源类型、客户自定义配置编号及版本，或母版编号及母版版本，以及对应客户引用记录编号。后续配置和引用变化不得改写历史快照。
7. 配置来源或准确版本变化只影响后续新任务；尚未发出的`待审核`账单可由财务确认影响后发起`账单生成`并选择新来源版本，由系统判定是否替换生成。已经发出、待结清或已结清的账单继续使用原快照，只能通过重算、调账、冲正或后续账单承接修正，不得作废或替换生成。
8. 客户邮箱属于客户主数据，不属于客户自定义配置或共享配置母版；财务必须在客户主数据中维护客户邮箱，用于系统自动向客户发送账单和客户级账单配置变更通知。未维护客户邮箱时，系统不得自动发送客户邮件。
9. 保存、发布或分配配置前，系统必须校验必填项、账期、履约节点、生效周期、限定情形、方案互斥关系、费项结算币种、客户来源唯一性及其它规则是否合法且完整。任一校验不通过时，不得使该客户的新来源生效，并应定位到具体客户或配置项说明原因。
10. 普通账单任务只允许读取已经通过校验并生效的客户引用记录及其准确配置版本。唯一例外是与`替换生成`任务绑定的`待生效`客户来源：该来源只允许用于生成本次替换的候选账单，不得被其它任务读取，并须在候选账单成功切换时同步生效。不得把配置阶段能够识别的问题延迟到账单生成或重算阶段处理。
...
1. 客户在每个配置类型下只有`自定义配置`和`共享配置母版`两种来源。引用母版时，客户引用的是一个准确母版版本，不是“自动跟随最新版本”的动态别名。
2. 引用母版的客户不得在客户侧覆盖母版中的单个字段。客户确需差异化时，系统提供`基于当前母版复制为自定义配置`，复制完成后从确认的切换时点解除母版引用，并以复制内容创建客户自定义配置新版本。
3. 每份母版必须设置`可分配客户范围`。范围维度一次只能选择`店铺`、`客户组`或`指定客户`中的一种，同一维度可选择多个对象；范围对象必须真实存在且在当前供应链内。该范围只是分配向导的候选约束，不是账单生成时的运行时匹配层级。
4. 店铺和客户组关系遵循 OIS 主数据。分配向导按打开向导时的主数据把范围展开为具体客户，以客户主数据编号去重；同一客户因多个会员或多个选中对象重复出现时，只能保留一条候选记录。
5. 向导确认后，以本次确认的具体客户清单形成逐客户引用记录。后续客户改组、会员改挂或店铺关系变化，不得自动新增、删除或切换客户引用；财务需要调整时须重新运行分配向导。
6. 母版可分配范围属于母版版本快照，范围变化必须发布新版本，不得改写旧版本。既有客户是否属于`范围外引用`，按其所引用准确版本的范围与当前 OIS 归属关系判断；客户因改组、会员改挂或店铺关系变化而离开该版本范围时，继续使用原准确版本并标记`范围外引用`。发布范围不同的新版本不得把旧版本引用重新判为范围外；客户升级时必须按目标版本范围重新校验，不符合目标版本范围的客户标记为`不可升级`。
7. 店铺和客户组仍可用于客户配置、账单和任务列表筛选，但筛选不改变客户配置来源、账单归属客户、账单编号或客户对账口径。
8. 任务和账单中的`配置编号`与`准确版本`是两个独立快照字段：自定义来源的配置编号记录客户自定义配置编号，母版来源的配置编号记录母版编号，准确版本记录对应来源版本。按配置编号或准确版本筛选历史任务或账单时，必须读取任务或账单快照，不得按客户当前引用重新展开。
9. 财务在共享配置母版的准确版本上发起批量生成时，系统只展开任务创建时仍引用该准确版本的客户，并按<a href="#section-9-2-1">9.2.1 系统自动跑，还是财务手动跑？</a>形成账单生成批次和独立客户任务。母版编号和版本只用于批量发起、快照追溯及汇总筛选，不作为一个整体执行任务。
10. 财务按生成批次编号、任务编号、配置编号或准确版本筛选出账单后，可以对筛选结果批量下载；每张账单仍按各自客户生成独立对账文件，批量下载规则见<a href="#section-19-6-3-1">19.6.3.1 发起导出</a>。
11. 存量迁移须在固定迁移时点按旧优先级`客户级 > 客户组级 > 店铺级`逐客户计算唯一最终生效配置：命中客户级配置的迁移为该客户自定义配置；最终命中客户组级或店铺级共享配置的迁移为共享配置母版，并形成该客户对准确母版版本的唯一引用。同一优先级命中多份配置、主数据缺失或无法确定唯一结果时，必须进入冲突清单由财务确认，不得直接建立多份引用。迁移完成后，系统不得继续使用任何层级的运行时优先级匹配。

<div id="section-7-2-2"></div>

#### 7.2.2 母版分配客户向导

母版分配、母版版本升级和强制切换客户均通过同一向导完成：

1. 财务选择母版准确版本和配置切换时点`T`，系统按母版可分配范围展开候选客户，并展示展开依据和客户总数。
2. 候选客户必须按`未配置`、`已引用当前母版的其它版本`、`已引用其它母版`、`使用自定义配置`、`已引用当前准确版本`和`不可切换`分类展示；每行展示客户、所属店铺、所属客户组、当前来源、当前准确版本、冲突任务或账单及可切换结论。
3. 系统默认勾选`未配置`客户和可升级的`当前母版旧版本`客户；使用其它母版或自定义配置的客户默认不勾选，已引用当前准确版本和不可切换客户不得重复提交。
4. 勾选使用其它母版或自定义配置的客户属于强制覆盖。只有具备配置强制切换权限的财务人员可以执行，且必须勾选二次确认、填写变更原因；系统须展示将失效的旧来源和受影响客户数。
5. 系统按客户逐一执行来源唯一性、目标版本可分配范围、版本完整性、切换时点`T`及通知资料校验。只有与`T`或新来源生效时段重叠的`待执行`、`执行中`、尚未删除的`执行失败`任务，或尚未完成的替换事务可以阻断切换；已完成任务和已生成历史账单只展示影响提示，由快照隔离，不得作为阻断条件。批量操作允许部分成功，但每个客户的来源切换必须原子完成，不得出现旧来源已失效而新引用未建立的中间结果。
6. 成功客户逐一结束旧引用、建立新引用和配置切换记录，并从`T`开始采用所选母版准确版本；失败客户保持原来源不变，向导结果必须返回客户、失败原因和建议处理方式，可仅重试失败客户。
7. 向导提交时固化候选客户清单、最终勾选客户清单、母版编号及版本、`T`、强制覆盖标记、变更原因、操作人和结果。向导执行期间主数据变化不得扩大本次客户范围。
8. 只有实际改变客户生效来源或准确版本的成功客户才触发客户级账单配置变更通知；单纯发布母版新版本、未选择客户或客户已引用同一准确版本时不发送通知。

<div id="section-7-2-3"></div>

#### 7.2.3 母版版本、停用与引用约束

1. 母版内容编辑必须发布为新版本；旧版本继续供现有客户引用和历史快照追溯，不得就地修改。
2. 母版新版本发布后，当前引用客户保持原版本。财务可通过分配向导选择全部或部分客户升级，并为每个客户分别确认`T`和切换结果。
3. 被任何客户引用或被任务、账单快照引用的母版版本不得物理删除。母版停用只禁止新分配和新版本升级，不自动解除现有客户引用；停用前页面必须提示当前引用客户数和待处理客户清单。
4. 母版的可分配客户范围、配置内容、状态、发布原因和版本关系必须进入版本快照；客户引用关系、强制覆盖原因和执行结果单独进入引用及切换记录。
5. 同一母版允许不同客户在同一时点引用不同版本；系统必须始终按客户引用记录中的准确版本执行，不得因为母版存在更新版本而隐式升级。
...
应收账单配置由来源信息、配置版本信息、默认方案和分支方案组成。来源信息必须明确该内容属于客户自定义配置还是共享配置母版；客户引用母版时，客户侧只保存引用记录，不复制或覆盖母版方案字段。

配置版本信息至少包括：

- 来源类型
- 客户自定义配置编号，或母版编号及母版版本
- 客户引用记录编号（母版来源时）
- 生效周期
- 启用状态
- 操作人
...
#### 7.4.1 应收配置列表怎样展示方案概览？

应收客户配置列表和应收共享配置母版列表以配置或母版版本为管理对象。默认方案和分支方案是同一版本内的出账规则，不得拆成独立列表行。

1. 应收客户配置列表按客户及其当前生效来源显示一条主行；应收共享配置母版列表按母版及列表当前显示的版本显示一条主行。默认方案和每个分支方案不得重复客户、配置编号、母版编号或版本信息形成多行。
2. 主行的`方案概览`先显示默认方案的账期类型，以及默认方案费项结算币种末行兜底规则的结算币种；显示内容由`默认：`、账期类型、` / `和兜底结算币种依次组成，例如`默认：月账单 / TWD`。兜底结算币种为随原始币种时，直接显示`随原始币种`。
3. 当前准确版本存在已启用分支方案时，主行同时显示`N 个分支`，其中`N`为该版本内已启用分支方案数量；没有已启用分支方案时显示`仅默认方案`，展开主行时只展示默认方案，不展示分支明细。
4. 财务展开主行后，系统按分支方案逐行展示分支方案序号、订单类型、目的国、集运仓、账期类型、账单发出时间、应收账单履约节点，以及该分支费项结算币种末行兜底规则的结算币种。展开行只用于查看方案概览，不改变客户配置或母版列表的主行粒度，也不提供独立发布、分配或生成账单操作。
5. 客户采用自定义配置时，方案概览和展开内容读取该客户当前生效的准确自定义配置版本；客户引用共享配置母版时，读取客户引用记录中的准确母版版本。即使该母版已经发布更新版本，客户主行也不得改为展示母版最新版本的方案内容。
6. 共享配置母版主行的方案概览和展开内容读取该行明确展示的母版版本。财务切换查看其它历史版本时，概览和展开内容同步切换到所选版本，不得混用不同版本的方案内容。
7. 客户尚未配置应收来源时，`方案概览`显示`-`，展开主行时不展示方案内容。
...
返款账单配置不包含默认方案和分支方案。返款客户配置列表和返款共享配置母版列表不得展示应收配置的`方案概览`或方案展开内容，继续按返款模式、账期类型和返款条款字段展示配置内容。
...
9. 每份应收配置中，默认方案的方案标识固定为`DEFAULT`；分支方案首次创建时取得该配置内唯一且不可变的方案标识。发布新版本时，继续存在的分支必须沿用原方案标识；新增分支取得新标识，已删除分支的标识不得复用。方案显示顺序、显示名称或配置版本变化不得改写方案标识。任务和账单快照必须同时记录方案标识、方案名称和方案类型；方案类型只取`默认方案`或`分支方案`，不参与唯一性识别。
...
客户级汇率配置解决的是“账单生成前，某个客户、某个货币对默认采用什么汇率”的问题。它位于账单生成之前，是账单特调汇率快照的前置配置；账单生成之后，采用的汇率会进入账单侧快照，由<a href="#section-15">15. 金额汇兑</a>继续定义换算、汇总和核销规则。特调配置只下挂客户，客户可以使用自定义特调配置或引用共享特调汇率母版；店铺和客户组仅用于母版分配向导选客，不参与运行时汇率命中。

本章的业务链路如下：

1. 财务先维护基准汇率表。基准汇率表支持手动导入，也支持财务在页面中添加、修改、移除基准汇率；`抓取`功能暂且预留，本期不开放使用。
2. 账单默认汇率按`账单级锁定汇率 > 客户当前特调配置（自定义配置或母版准确版本） > 基准汇率表 > 店铺汇率表 > 1`取值；若这些来源都找不到可用汇率，该货币对汇率固定按 `1` 处理。
3. 财务按客户维护自定义特调配置，或维护共享特调汇率母版并通过向导分配给具体客户；特调配置不再按应收 / 返款等账单类型拆分专属规则。
4. 生成账单时，系统按客户和货币对命中默认汇率。本章不设置`账单特调汇率日期`概念，也不支持按账期开始日、账期结束日或指定日期切换汇率口径。
5. 账单生成成功后，系统把客户实际采用的来源类型、客户自定义配置编号及版本或母版编号及版本、客户引用记录、货币对规则、基准汇率、店铺汇率、推导关系和最终默认汇率固化到账单特调汇率快照。
...
基准汇率和特调汇率在导入、添加、修改、发布、分配、确认或启用前，必须校验币种、货币对、汇兑方向、汇率值、调整方式、调整方向、调整值、客户来源唯一性、母版可分配范围及推导结果是否合法。任一校验不通过时，不得保存、发布、分配、确认或启用，并应定位到具体客户或字段说明原因。账单任务只读取已经通过校验且可以生效的汇率配置；缺少兜底汇率时按本章规定使用`1`，不得把可在配置阶段识别的问题转化为任务执行异常。

<div id="section-8-2"></div>

### 8.2 页面原型概述

客户级汇率配置页面用于让财务先维护基准汇率，再维护客户特调配置和共享特调汇率母版。页面划分为左右并列的两个业务区域：左侧为基准汇率表区，右侧为客户特调配置区，并在右侧通过页签切换`客户配置`和`共享配置母版`；窄屏仅调整为上下排列，不改变区域字段与业务操作。

| 区域 | 区域用途 | 业务字段 | 支持操作 |
| :--- | :--- | :--- | :--- |
| 基准汇率表区 | 展示当前已确认或待确认的基准货币对汇率 | `货币对`、`汇兑方向`、`基准汇率`、`汇率来源`、`来源时间`、`确认状态`、`当前生效` | 查看、确认、添加、修改、移除基准汇率；支持手动导入基准汇率；`抓取`功能入口暂且预留，本期不开放使用；确认后的基准汇率作为客户级汇率配置的兜底来源。 |
| 客户特调配置区 | 展示每个客户当前采用的特调来源和准确版本 | `客户`、`所属店铺`、`所属客户组`、`配置来源`、`自定义配置编号或母版编号`、`版本`、`货币对`、`调整方式`、`特调后默认汇率`、`状态` | 按客户、店铺、客户组、来源和状态筛选；新增或编辑客户自定义配置；基于母版复制为自定义配置；查看所引用母版。 |
| 共享配置母版区 | 维护可供多个客户复用的特调规则集合 | `母版编号`、`母版名称`、`版本`、`可分配客户范围`、`当前引用客户数`、`规则数量`、`状态` | 新建母版、发布新版本、分配客户、查看引用客户、停用；母版分配与版本升级使用客户向导。 |
...
3. 财务为客户保存自定义特调配置，或通过母版分配向导建立客户对准确母版版本的引用；同一客户只能采用其中一种来源。
4. 客户当前特调配置中没有所需货币对或对应规则不可用时，系统先使用已确认的基准汇率兜底；若基准汇率也不可用，再使用店铺汇率表兜底；若仍找不到兜底汇率，该货币对汇率固定按 `1` 处理。
5. 母版可分配客户范围和分配向导采用<a href="#section-7-2-1">7.2.1 客户配置来源与共享配置母版</a>及<a href="#section-7-2-2">7.2.2 母版分配客户向导</a>的范围、版本、覆盖、阻断和留痕规则。应收、返款和特调汇率三类配置分别建立引用，一个客户可分别引用不同母版。特调汇率来源切换本期只进入内部审计，不触发 7.2.2 第 8 条的客户账单配置变更邮件。
6. 页面所有操作均由财务执行。页面及编辑弹窗不展示操作人和操作时间；基准汇率以及客户自定义特调配置、特调汇率母版、客户分配的新增、导入、修改、发布、移除、确认和停用统一进入内部审计，规则见<a href="#section-8-7">8.7 内部审计</a>。
...
| 内部审计记录 | 导入、添加、修改、移除、确认、停用等操作统一记录操作人、操作时间和操作原因，不在页面展示；规则见<a href="#section-8-7">8.7 内部审计</a>。 |
...
客户自定义特调配置和共享特调汇率母版都实行版本控制。已经生效或被任务、账单引用的内容不得就地编辑；变更必须创建新版本，并按客户确认的切换时点生效。母版新版本不自动升级已有客户引用，历史账单快照不得被回写。

规则维度如下：

| 规则维度 | 说明 |
| :--- | :--- |
| 配置来源 | 客户自定义配置，或共享特调汇率母版的准确版本。 |
| 客户引用 | 母版来源时记录客户引用记录编号、生效时点和母版准确版本；自定义来源时记录客户及自定义配置准确版本。 |
...
| 启用状态 | 启用后参与对应客户的账单生成；停用或客户未配置所需货币对时，回到基准汇率表、店铺汇率表和`1`的兜底链路。 |
| 内部审计 | 新增、发布版本、分配客户、强制覆盖、解除引用和停用统一进入内部审计记录；页面不展示操作人和操作时间。 |
...
1. 同一客户在同一生效时段只有一个特调配置来源；该来源的同一准确版本内，同一货币对只允许存在一条启用中的特调规则。特调汇率不按账单类型拆分。
...
9. 原型表格中展示的特调汇率，应理解为特调后用于账单生成的默认汇率结果；系统仍需保留其背后的配置来源、准确版本、客户引用、调整方式、调整方向和调整值。
10. 新建客户自定义特调配置时必须先选择一个明确客户；新建共享特调汇率母版时必须先填写母版名称和可分配客户范围，再配置货币对、调整方式和调整值。母版保存后通过向导分配客户，不在规则编辑弹窗中直接选择多个适用对象。
11. `编辑`或`停用`特调汇率时，只影响后续新账单；任何既有账单的普通重算均沿用该账单锁定的汇率快照，不得隐式读取客户当前特调配置。财务显式提交账单级汇率调整时，系统按本次输入或所选准确版本另建账单级汇率快照，不回写旧快照。
12. 存量特调汇率须在固定迁移时点按旧优先级`客户级 > 客户组级 > 店铺级`逐客户计算唯一最终生效规则：客户级结果迁移为该客户自定义特调配置，客户组级或店铺级结果迁移为共享特调汇率母版及唯一客户引用。同一优先级命中多份规则或无法确定唯一结果时进入冲突清单，由财务确认后再迁移。
13. 共享特调汇率母版的可分配范围、分配向导、版本升级、强制覆盖、停用和范围外引用规则采用<a href="#section-7-2-1">7.2.1 客户配置来源与共享配置母版</a>至<a href="#section-7-2-3">7.2.3 母版版本、停用与引用约束</a>；三类配置的引用关系相互独立。特调汇率切换不发送客户账单配置变更邮件，只记录内部审计。
...
1. 新账单生成时，系统先读取该客户在任务执行快照中锁定的唯一特调配置来源：客户自定义配置准确版本，或客户所引用的共享特调汇率母版准确版本；再按账单所需货币对查找启用规则。规则方向与账单所需方向相反时，按倒数规则推导默认汇率。
...
账单生成成功后，系统必须把客户、配置来源类型、客户自定义配置编号及版本或母版编号及母版版本、客户引用记录编号、命中的货币对规则、基准汇率、店铺汇率、汇率来源、调整方式、调整方向、调整值、推导关系和计算后的默认汇率一起固化到账单特调汇率快照中。若命中的是固定汇率值，基准汇率和店铺汇率可记录为`不适用`，但固定汇率值和准确配置来源必须进入快照。后续客户级汇率配置或店铺汇率表变化只影响后续新账单，不触发 BMS 任务，也不得回写历史账单；普通账单重算继续使用目标账单快照，显式账单级汇率调整另建新快照。
...
4. 推导后的汇率必须保留推导来源，例如`由 USD -> CNY 倒数推导 CNY -> USD`，或`由 USD -> CNY 与 TWD -> CNY 相除推导 USD -> TWD`；若直接命中特调规则，则记录客户实际采用的自定义配置或母版准确版本，不再额外要求经 `CNY` 推导。
...
5. 账单费项明细中的汇率默认值取值优先级为：`账单级锁定汇率 > 客户当前特调配置（自定义配置或母版准确版本） > 基准汇率表 > 店铺汇率表 > 1`。
6. 历史账单始终以账单快照为准，不再读取最新客户级汇率配置。
7. 特调汇率与应收、返款账单配置的客户引用关系相互独立：三类配置分别校验客户唯一来源，一个客户可以分别引用不同母版。

客户级账单配置提供出账范围和账期口径，客户级汇率配置提供生成前的默认汇率。两类前置配置都具备后，账单生成机制才能按稳定快照归集来源数据并形成账单结果。

<div id="section-8-7"></div>

### 8.7 内部审计

基准汇率和特调汇率的新增、导入、修改、移除、确认、停用等操作必须形成内部审计记录，统一见<a href="#section-18">18. 内部审计</a>。

1. 汇率配置页面及编辑弹窗不展示操作人、操作时间；操作人、操作时间、操作原因、变更前值和变更后值只在内部审计中查询。
2. 每次变更必须记录客户或母版、来源类型、准确版本、货币对、变更前后汇率设置、母版分配与强制覆盖范围、操作原因、操作人、操作时间和逐客户执行结果。
...
系统统一使用`BMS任务`承载费项入池、账单生成和账单重算，不建立父子任务。每条任务只有一个`任务类型`，用于直接说明本次任务要完成`费项入池`、`账单生成`还是`账单重算`。财务从共享配置母版批量发起账单生成时，系统另建一条非执行型`账单生成批次`用于冻结范围和汇总结果；批次不进入任务队列，也不是这些任务的父任务。
...
2. `账单生成`：财务可在账期结束前或结束后按客户点击`账单生成`，也可从共享配置母版准确版本批量发起。母版批量发起先冻结提交参数并预校验；至少一个范围通过时，系统原子创建非执行批次及对应的客户独立任务，全部范围跳过时只保存提交校验审计。目标范围没有有效账单时首次生成，已有可沿用的从未发出`待审核`账单时补充生成，本次配置改变账期范围、分组结果或费项归属且满足替换条件时替换生成。
3. `账单重算`：只读取目标账单及其有效调整记录，不重新筛选业务来源数据。同时存在新增费项和重算需求时，先通过账单生成任务补充生成，再按补充后的结果重算。

`期末收口`不是第四种任务类型。账期结束后的次日凌晨，系统按客户、账单类型、到期方案和实际账期分别创建本范围最后一个`账单生成`任务，补齐期末费用并首次生成或补充生成；校验通过后把对应账单的账期收口状态更新为`已收口`，不得按母版编号合并为一个执行任务，也不得重复创建账单。
...
| `账单生成批次` | 一次母版批量发起包含哪些客户任务，整体结果怎样查看？ | 财务按共享配置母版准确版本批量发起时形成的非执行分组记录。批次冻结所选准确版本、客户引用、方案标识、方案名称、方案类型、实际账期、数据截止点和创建结果，并关联本次创建的独立BMS任务；批次本身没有任务类型、执行阶段或占用范围。 |
...
| `业务快照` | 客户自定义配置或共享配置母版、客户引用、来源规则、费项规则和汇率等业务内容在某一版本下的不可变记录。 | 回答“业务规则具体是什么、客户当时采用哪个来源”；任务只引用所需业务快照，不重复复制其完整内容。 |
...
| `账单配置快照` | 客户最终采用的自定义配置版本，或共享配置母版准确版本连同客户引用记录的完整、不可变记录。 | 任务执行快照和账单结果都引用它；后续配置或母版引用变化不会改写该快照或已有账单口径。 |
...
| `账单分组键` | 在同一客户、账单类型、实际采用的配置来源及准确版本、方案标识和实际账期内，用于继续区分具体账单的业务维度组合。 | 应收账单使用`业务板块 + 运抵国`；分组键不同必须形成不同账单。 |
...
共享配置母版批量生成只改变任务发起方式，不改变单条任务的执行流程。系统先按母版准确版本冻结本批次客户和方案范围，再为每个可创建范围生成独立任务；每条任务从上述第一步开始执行，并分别形成任务状态、快照和结果。
...
系统先根据触发场景确定任务类型。单客户操作和系统定时触发直接创建一条`BMS任务`；共享配置母版批量生成先冻结提交参数并逐范围预校验，至少一个范围通过时再原子创建一条`账单生成批次`和一条或多条独立`BMS任务`，全部范围跳过时不创建批次或任务。每条任务进入队列后即视为创建成功，任务状态为`待执行`；此时任务正在轮候执行，尚未开始处理。
...
| 共享母版批量生成 | 按`客户 + 账单类型 + 方案标识（返款固定为 REFUND） + 实际账期 + 母版准确版本`拆分任务范围；一个应收客户有多个方案在本次达到生成条件时，可以在同一批次内形成多条任务。任务同时保存方案名称和方案类型用于展示；返款方案名称为`返款配置`、方案类型为`不适用`。 |
...
| 系统期末生成 | 账期结束后的次日凌晨，到达某客户、账单类型、方案标识和实际账期的最后一次定时执行时间 | 为该客户和到期方案创建任务类型为`账单生成`的定时任务；取得执行权后判定首次生成、补充生成或替换生成，没有新增结果时记为`无需补充`，并在成功后完成期末收口。共享母版编号不扩大定时任务粒度。 |
...
| 共享母版批量生成 | 财务在共享配置母版列表选择当前显示的准确版本并确认本次客户和方案范围 | 系统先冻结提交参数并预校验，只展开确认时仍引用该准确版本的客户；至少一个范围通过时生成唯一批次编号，并按客户、方案标识和实际账期创建一条或多条手动`账单生成`任务。返款配置按客户和实际账期创建任务。全部范围跳过时不生成批次或任务。母版可分配客户范围不参与本次展开。 |
...
用户不能独立触发`费项入池`任务，只能从客户、共享配置母版或账单业务入口手动创建`账单生成`和`账单重算`任务。账单生成任务在同一条任务中先完成必要的来源筛选和费项入池，再计算账单；该内置阶段不得在页面上拆成可单独点击的`同步数据`操作。
...
3. 系统必须防止同一范围重复创建任务。任务身份必须记录`客户 + 账单类型 + 方案标识（返款固定为 REFUND） + 实际账期 + 配置来源及准确版本 + 任务类型`，并另存方案名称和方案类型用于展示。来源处理范围按`客户 + 账单类型 + 方案标识 + 配置准确版本 + 来源数据集 + 处理时间范围`判定；账单计算范围按`客户 + 账单类型 + 实际账期 + 方案标识 + 目标账单或替换批次 + 任务类型`判定，不得仅因配置编号、版本、方案名称或方案类型不同而放行同一账单结果的并发计算。`费项入池`任务占用来源处理范围；`账单生成`任务同时占用来源处理范围和账单计算范围；`账单重算`任务只占用账单计算范围。同一范围已有`待执行`、`执行中`或尚未删除的`执行失败`任务时，不得再次创建。失败任务重新执行成功或被财务删除后，才释放其占用范围。
...
7. 单客户操作和系统定时触发生成一个唯一任务编号。共享配置母版批量生成一个唯一批次编号，并为每个通过校验的任务范围分别生成唯一任务编号；批次编号只能用于关联、筛选和汇总，不得代替任务编号进入队列或占用处理范围。任务必须记录任务类型、账单生成方式、内部收口结果、数据截止点、当前执行阶段，以及各阶段的开始时间、结束时间、执行结果和检查点。任务类型不是`账单生成`时，账单生成方式显示为`不适用`；账单生成任务尚未取得执行权时显示为`待判定`。内部收口结果不向用户展示。
8. 母版批量生成确认时，系统先按母版编号和准确版本冻结财务最终选择的客户引用、方案标识、方案名称、方案类型、实际账期和数据截止点，再逐范围执行配置生效、防重和权限预校验。至少一个范围通过时，系统在同一事务中创建批次和所有通过校验的任务，并把未通过范围记录为`跳过`；没有任何范围可创建时，不创建批次和任务，只保存本次提交校验审计并在页面保留客户、方案、账期和失败原因。
...
| :--- | :--- | :--- |
| 费项入池 | 当前客户生效的账单配置来源快照，包括自定义配置版本或母版准确版本及客户引用记录；来源数据与场景取值规则快照；费项入池规则快照 | 客户、账单类型、方案标识、方案名称、方案类型、账期、来源处理范围、数据截止点，以及各来源数据集采用的扫描方式和读取的增量同步水位。 |
| 账单生成 | 保存既有待审核账单引用的原业务快照，以及本次客户采用的来源类型、配置或母版准确版本、客户引用记录、来源规则、费项规则、费项结算币种和默认汇率快照；没有既有账单时保存任务创建时生效的对应业务快照 | 是否需要收口、来源处理范围、账单计算范围、数据截止点、已有待审核账单、本次采用的客户配置来源及切换时点、方案标识、方案名称、方案类型，以及生成批次编号（如有），并保存各来源数据集采用的扫描方式和读取的增量同步水位；取得执行权后补记账单生成方式和收口结果。判定为替换生成时，再形成替换批次和待替换账单集合。 |
...
1. 客户自定义配置或共享配置母版、客户引用、来源规则、费项规则和汇率快照保存业务规则本身；任务执行快照只保存其快照编号、准确版本和校验值，不重复保存完整内容。
...
客户的自定义配置版本、所引用母版或母版准确版本发生变更后，旧来源形成的账单和任务可能尚未结束，新来源也可能已经保存。本节规定每个客户新旧来源版本的切换边界、账期衔接和费项归属，确保同一费项不重复出账，也不因任务执行先后改变归属。批量分配母版时仍逐客户应用本节规则。
...
1. 已按旧来源版本生成的账单继续使用原配置快照。新来源版本不得自动覆盖、作废、重建或改变已有账单；只有尚未发出的`待审核`账单，才允许财务发起`账单生成`并选择新来源版本，由系统判定是否替换生成。
2. 同一客户的新旧来源以唯一的配置切换时点`T`划分承接范围：`T`之前由旧来源版本承接，`T`及之后由新来源版本承接。
3. 系统先确定费项归属该客户的哪个配置来源和准确版本，再按该版本校验账期、履约节点、限定情形、费项范围和结算币种。不得先尝试新来源，再把不符合新来源的费项交给旧来源处理。
...
| 概念 | 定义 |
| :--- | :--- |
| 配置切换时点`T` | 同一客户的旧配置来源版本停止承接、新配置来源版本开始承接的唯一时间边界。 |
...
保存客户自定义配置新版本或提交母版分配向导时，系统先将该客户的新来源置为`待生效`，再按以下步骤确定并校验`T`：
...
5. 新来源版本可提前保存并指定未来的`T`。到达`T`前，系统继续使用旧来源版本；到达`T`后，不得再为普通新增费项创建跨越`T`的旧版账期。
6. 母版批量分配或升级时，系统必须为每个客户独立计算和校验`T`。部分客户失败不回滚其它已经原子切换成功的客户，但失败客户必须保持原来源不变并返回失败原因。
...
| 区域 | 展示内容 | 交互与约束 |
| :--- | :--- | :--- |
| 版本信息 | 客户、账单类型、旧来源类型与准确配置编号/版本、新来源类型与准确配置编号/版本、客户引用记录、变更原因 | 财务必须确认本次逐客户切换对象无误。 |
...
1. 配置切换记录至少包括客户、账单类型、旧来源类型和准确配置编号/版本、新来源类型和准确配置编号/版本、旧新客户引用记录、切换方式、`T`、是否强制覆盖、变更原因、操作人和确认时间。
2. 配置切换记录须包括旧版最后一个实际账期、新版第一个实际账期、新版首个完整账期及是否形成截断账期。
3. 每张账单须记录配置来源、准确版本、客户引用记录、方案标识、方案名称、方案类型、实际账期、账期类型、是否为截断账期、生成任务编号、生成批次编号（如有）和对应的配置切换记录。
4. 每个`账单生成`任务必须记录配置来源、准确版本、客户引用记录、方案标识、方案名称、方案类型、`T`、生成批次编号（如有）、任务执行快照、计划账期、数据截止点和实际处理范围。
5. 每条进入账单的费项必须能够追溯其归属配置来源及准确版本、原业务时间、首次达到归集条件的时间、归属账单及是否属于延迟归集。
...
任务锁定规则后，系统先按<a href="#section-9-3-1">9.3.1 配置改了，旧账单怎么办，新费项归谁？</a>确定费项归属配置版本，再按该版本和各来源适用的账期时间口径、履约门槛、限定情形筛选本次可处理的数据；附加费的专用口径见<a href="#section-9-5-4">9.5.4 费用达到什么条件才能入池？</a>。各类费项对应的来源表、金额字段、业务类型和首次产生时机见<a href="#section-19-2-3">19.2.3 客户侧费项的 OIS 数据源</a>。`BMS检阅标记`只表示来源数据是否已被BMS处理，不表示账单、支付或核销状态。
...
2. 除附加事项外，订单类来源按本次任务的账期、履约节点和限定情形筛选。`附加费用报表导入`形成的附加费挂靠真实业务订单下的尾程包裹，以`ofp_ofdb1.sale_order_additional_matter.create_time`确定归属账期，并校验该包裹所属真实业务订单是否已达到配置要求的履约节点；履约节点可以早于本账期开始日，不要求与附加费创建时间落在同一账期。`记账单导入`形成的附加费挂靠虚拟业务订单，同样按`create_time`筛选，但不校验订单履约节点。两类入口的来源事实见<a href="#section-19-2-3-5">19.2.3.5 同一张附加事项表，为什么要区分两种导入？</a>。
...
| 时间口径 | 跟随账单配置或来源创建时间 | 除附加事项外，订单类费项跟随履约节点。`附加费用报表导入`和`记账单导入`形成的附加费均使用`ofp_ofdb1.sale_order_additional_matter.create_time`作为费用发生时间并确定归属账期；其中挂靠真实尾程包裹的附加费还须校验所属真实业务订单已经达到配置要求的履约节点，但履约时间不替代附加费创建时间。各来源使用的非金额字段见<a href="#section-19-2-3-3">19.2.3.3 BMS 归集费项时使用哪些非金额字段？</a>。 |
...
1. 除附加事项外，订单类费项的履约节点按<a href="#section-7-4">7.4 应收账单配置</a>执行；`订单完结`的判断按<a href="#section-19-2-5">19.2.5 广义签收</a>执行。
2. 费项金额的稳定边界按<a href="#section-19-2-4">19.2.4 费项重算窗口</a>执行。尚处于重算窗口内的横表来源不得入池。
3. 返款模式和账期范围按<a href="#section-7-5">7.5 返款账单配置</a>执行；附加费按本节下方的导入入口、挂靠对象、创建时间和履约门槛判断能否入池。
...
8. 两个入口形成的附加费进入费项池后，如对应账期尚无账单，则参与首次生成；如对应账单尚未发出且允许补充，则参与补充生成；其它情形按<a href="#section-9-8">9.8 第七步：账单生成后又有变化，应该走哪条路？</a>承接，不得重复形成应收费项。
...
1. 首次生成按任务执行快照引用的业务规则、处理范围和数据截止点筛选费项池记录，创建账单、账单编号、首版明细和首个结果版本。系统按`客户 + 账单类型 + 方案标识 + 配置版本 + 实际账期 + 账单分组键`识别同一张账单，不得重复创建。应收账单的分组键包含业务板块和运抵国，具体拆分规则以<a href="#section-10-5">10.5 自动拆单</a>为准；不适用自动拆单的账单类型使用其对应模块规定的默认分组键。
...
| 客户级账单配置 | 财务从客户行按客户发起账单生成，或从共享配置母版的当前准确版本批量发起 | 创建前逐客户、逐方案和逐实际账期确认最终配置来源及准确版本已生效，且不存在未结束的同范围任务。母版批量发起须生成批次编号并返回创建任务与跳过范围；单客户任务和批次内任务均由系统在取得执行权后判定首次生成、补充生成或替换生成。 |
...
任务页面使用一个统一任务列表，通过`任务类型`筛选`费项入池`、`账单生成`或`账单重算`，不建立父子任务关系。共享配置母版批量发起形成的任务继续平铺展示，并通过生成批次编号关联；批次查看区只汇总关联任务，不成为父任务。只有账单生成任务展示`账单生成方式`：取得执行权前显示`待判定`，完成判定后显示`首次生成`、`补充生成`或`替换生成`；其它任务类型统一显示`不适用`。

账期内每日预定任务的任务类型为`费项入池`；系统在账期结束后的次日凌晨创建本账期最后一个定时任务时，任务类型为`账单生成`。财务可在自动生成前后按客户创建`账单生成`或`账单重算`任务，也可从共享配置母版批量创建多条独立`账单生成`任务。账单生成方式由每条任务取得执行权后分别判断。`账单发出时间`只决定账单是否已经对客户生效，不参与任务创建判断。
...
| 任务筛选区 | 生成批次编号、任务编号、任务类型、配置来源、配置编号或母版编号、准确版本、客户名称、客户编码<br>会员编码、所属店铺、所属客户组、任务状态、账单生成方式、触发方式、方案类型<br>账期开始日期、账期结束日期 | 生成批次编号匹配任务创建时固化的批次关联；配置编号和准确版本分别读取任务执行快照，自定义来源匹配自定义配置编号，母版来源匹配母版编号，不按客户当前引用展开。方案类型只用于筛选`默认方案`、`分支方案`或`不适用`，不代替方案标识。组合筛选同时作用于统计区和任务列表。 |
| 任务列表区 | 生成批次编号、任务编号、任务状态、任务创建时间、执行耗时、任务类型<br>账单生成方式、触发方式、配置来源、配置编号或母版编号、准确版本、方案标识、方案名称、方案类型<br>客户名称、客户编码、会员编码、所属店铺、所属客户组、账期<br>数据截止点 | `费项入池`任务只提供详情；`账单生成`和`账单重算`任务按状态提供详情、重新执行或删除。任务执行阶段、内部收口结果、来源扫描方式和错误摘要不在列表展示。 |
| 生成批次查看区 | 生成批次编号、母版编号及准确版本、冻结客户数、初始创建任务数、当前有效任务数、已删除任务数、跳过数量<br>待执行、执行中、执行成功和执行失败数量，以及包含删除审计行的关联任务清单 | 生成批次只提供范围与结果汇总，不提供整体重新执行或整体删除；失败任务从关联任务清单单独处理，删除任务保留只读审计行。 |
| 任务详情区 | 生成批次编号、任务编号、任务状态、任务创建时间、执行耗时、任务类型<br>账单生成方式、触发方式、配置来源、配置编号或母版编号、准确版本、客户引用记录、方案标识、方案名称、方案类型、客户、账期<br>数据截止点、原账单清单、新账单清单、结果版本、错误摘要<br>处理建议、任务执行快照、业务快照引用、替换影响、重算范围<br>来源扫描记录、来源查询语句（SQL） | 任务执行阶段、阶段记录和内部收口结果不展示；错误摘要和处理建议仅在任务详情展示。费项入池和账单生成按来源数据集展示扫描方式、扫描范围、上次成功水位、本次数据截止点、拉取数量、命中数量、扫描结果和失败原因；同一任务的不同来源数据集可以采用不同扫描方式。账单重算不读取来源数据，来源扫描记录显示`不适用`。 |
...
本规则生效前已经创建的任务不补写生成批次编号，任务列表和详情显示`-`。这些任务继续按原任务编号、执行快照和状态规则处理；本规则生效后从共享配置母版批量发起的新任务必须记录生成批次编号。
...
5. 应收账单列表支持按客户名称或编码、所属店铺、所属客户组、生成批次编号、任务编号、配置编号和准确版本筛选客户级账单；生成批次编号和任务编号读取账单关联任务快照，配置编号与准确版本分别匹配账单快照中的独立字段，均不读取客户当前引用。财务按生成批次编号、任务编号、配置编号或准确版本筛选出账单后，可以对筛选结果批量下载，规则见<a href="#section-19-6-3-1">19.6.3.1 发起导出</a>。
...
5. 财务按生成批次编号、任务编号、账单快照中的配置编号或准确版本筛选出应收账单后，可以对筛选结果批量下载：`导出给客户`逐客户生成对账文件并打包下载，`导出给内部`按筛选结果全部账单生成内部明细文件，打包规则见<a href="#section-19-6-3-1">19.6.3.1 发起导出</a>。
...
返款账单详情页用于核对一张账单“返款金额从哪里来、怎样计算、已经处理多少、还剩多少”，并向财务展示汇兑损益。详情页不展示或计算应收回款、回款汇率、实收回款、已回款、待回款等回款字段；相关事实和状态统一在<a href="#section-12">12. 回款管理</a>查看。
...
| 包裹返款明细 | 尾程运单号、业务订单号、签收时间、来源金额与币种、到付附加费、应付返款、指定扣减金额、实付返款（准）、返款汇率、实付返款、已返金额、待返金额、核销状态 | 每笔明细必须保留业务订单和包裹关联，并能从来源金额追溯至最终返款计算结果；不展示包裹级回款字段。 |
...
3. 财务本位币汇总、货款结算币种汇总和包裹明细必须能相互追溯；展示精度差异按<a href="#section-15">15. 金额汇兑</a>处理。
4. `实付返款（准）`小于零时，详情页应展示负数金额及其处理去向，不计算实付返款，也不生成负数待返金额或负数返款核销记录。
5. 汇兑损益按<a href="#section-15-7-3">15.7.3 返款账单怎样计算汇兑损益？</a>计算，不参与实付返款、已返金额、待返金额或核销金额。
...
5. 返款账单列表支持按客户名称或编码、所属店铺、所属客户组、生成批次编号、任务编号、配置编号和准确版本筛选客户级账单；生成批次编号和任务编号读取账单关联任务快照，配置编号与准确版本分别匹配账单快照中的独立字段，均不读取客户当前引用。财务按生成批次编号、任务编号、配置编号或准确版本筛选出返款账单后，可以对筛选结果批量下载，每张账单仍按客户生成独立对账文件，规则与应收账单一致，见<a href="#section-19-6-3-1">19.6.3.1 发起导出</a>。
...
2. 客户当前特调配置：读取任务执行快照锁定的客户自定义配置准确版本，或客户引用的共享特调汇率母版准确版本；同一货币对存在启用规则时使用，方向相反时按倒数规则推导。
3. 基准汇率表：客户当前特调配置没有可用规则时，使用已确认生效的基准汇率。
4. 店铺汇率表：客户特调配置和基准汇率表均不可用时，使用账单归属店铺对应的店铺汇率。
5. 固定汇率 `1`：以上来源均不可用，或同币种换算时使用。

客户当前特调规则因基准汇率不可用而无法产出默认汇率时，视为客户特调未命中并继续使用基准汇率表、店铺汇率表或`1`。账单金额换算优先级为：`账单级锁定汇率 > 客户当前特调配置（自定义配置或母版准确版本） > 基准汇率表 > 店铺汇率表 > 1`。同币种换算汇率固定为 `1`；异币种换算在客户特调配置、基准汇率表和店铺汇率表均找不到兜底汇率时，该货币对汇率也固定按 `1` 处理，并写入账单特调汇率快照。费项级明确汇率目前仅作为后续扩展概念预留，本期不纳入开发范围，也不参与账单生成和账单重算优先级。
...
12. 账单特调汇率列表只决定费项结算币种级锁定汇率与回显口径，不改变 `账单级锁定汇率 > 客户当前特调配置（自定义配置或母版准确版本） > 基准汇率表 > 店铺汇率表 > 1` 的换算优先级。
...
2. 同一客户、同一账期、同一账单类型和同一方案标识的账单生成流程不得并发执行；客户是否引用同一母版不扩大并发锁范围。已有任务执行中时，后续任务应提示被占用或排队处理。
...
3. 汇率一旦在账单侧锁定，后续市场汇率、基准汇率表、客户自定义特调配置或共享特调汇率母版及其引用变化不得自动影响历史账单金额。
...
6. 表格中的客户、账单、任务、回款或报价归属关系字段统一显示为`所属店铺`和`所属客户组`，避免与可编辑对象或配置层级混淆；查询筛选条件、表单对象类型和母版范围维度仍可简写为`店铺`和`客户组`。
...
1. 客户自定义账单配置、共享账单配置母版及版本发布、客户分配与强制覆盖、客户切换为自定义配置、共享母版账单生成批次提交、基准汇率表、客户自定义特调配置、共享特调汇率母版及版本发布、客户分配与强制覆盖、账单内锁定汇率、客户侧费项索引、成本费项索引、间接成本分摊规则、导入设置快照中的费项映射、场景取值规则、数据源规则和供应商财务档案变更。
...
2. 涉及账单生成批次、账单、任务、订单、尾程包裹、成本明细、间接成本分摊集或调账时，应保存相应关联编号；批次审计还须保存母版编号及准确版本、冻结客户与方案范围、创建任务、跳过范围和原因，使财务能够沿业务链路追溯。
...
| 二级标题章节 | 术语链接 |
| :--- | :--- |
| 5. 总体方案 | <a href="#section-5-2">应收账单</a>、<a href="#section-5-2">返款账单</a>、<a href="#section-5-2">成本账单</a> |
| 9. 账单生成/重算机制 | <a href="#section-9-5-1">费项</a>、<a href="#section-9-1-2">提前收口</a>、<a href="#section-9-3-1">截断账期</a>、<a href="#section-9-5-1">仅展示不核销</a> |
| 10. 应收账单模块 | <a href="#section-10-4">待审核</a>、<a href="#section-10-4">待结清</a>、<a href="#section-10-4">已结清</a>、<a href="#section-10-4">已作废</a>、<a href="#section-10-4">账期收口状态</a>、<a href="#section-10-4-1">账单发出时间</a>、<a href="#section-10-4-1">通知状态</a>、<a href="#section-10-4">逾期未结清</a> |
| 15. 金额汇兑 | <a href="#section-15-3-2">锁定汇率</a> |
...
| 店铺 | 面向客户经营的服务入口，可配置独立域名、服务客群、启用仓库、物流产品和差异化汇率。 | 店铺决定订单从哪个经营入口产生，用于账单归集与筛选、共享配置母版分配向导选客、店铺汇率兜底和对账报表归属；不参与账单配置或特调配置的运行时分级命中。 |
| 客户组 | 店铺下的客户归集分组，即集运客门户中的子账号分组；一个店铺对应一个集运客门户二级域名和主账号，可建立多个子账号分组，一个子账号分组可下挂多个客户，一个会员（集运客门户账号）只能同时从属一个子账号分组。 | 客户组用于查询、报表归集和共享配置母版分配向导选客。BMS 只读取订单及客户主数据已沉淀的客户组，不重新划分客户组，也不按客户组动态决定配置来源。 |
...
4. 店铺可启用特定集运仓、物流产品、线路报价和汇率口径，因此店铺是订单与费用归属、账单拆分、共享配置母版选客和店铺汇率兜底的重要维度，但不是客户账单配置或客户特调配置的运行时唯一键。
...
8. 一个店铺对应一个集运客门户二级域名和主账号；一个会员对应一个集运客门户账号；一个店铺可有多个子账号分组，一个会员只能同时从属一个子账号分组。典型业务模式：供应链（如天马运通）开设多个店铺（视为子公司），店铺作为独立接单渠道和独立运营端；子公司可建立多个子账号供客户在店铺上执行业务，也可设立子账号分组；一个子账号分组有时可视为子公司的一个重点客户（如代购商），子账号分组中的会员视为该重点客户的下辖伙伴（如代购员）。

<div id="section-19-2-1-5"></div>

##### 19.2.1.5 BMS 使用口径

1. 客户级账单配置和客户特调配置都以`供应链 + 客户主数据编号`确定客户配置主体；客户在每种配置类型下只能使用自定义配置或引用一个共享配置母版准确版本。店铺和客户组仅用于母版分配向导选客、列表筛选和结果核对，不参与运行时分级命中。账单生成仍保留供应链、店铺、客户组、会员和客户维度，并继续按订单主体关系防止跨店铺归集。
...
7. 客户组作为 OIS 已形成的主数据，BMS 只读取并快照订单侧已沉淀的店铺、客户组和客户关系，不反向修改客户组及客户挂靠关系。客户组关系变化不得自动新增、删除或切换客户的配置母版引用。
8. 客户组即集运客门户中的子账号分组，会员所属子账号分组用于分配向导展开和账单追溯。向导必须以客户主数据编号去重并固化本次客户清单，不得因同一客户关联多个会员或后续改组而动态改变已经确认的引用。
9. OIS 提供给分配向导的数据契约必须包含供应链编号、客户主数据编号、店铺编号及名称、客户组编号及名称、会员关系、关系生效状态和更新时间。产品与 OIS 负责人须在技术设计评审前确认接口或数据视图、字段映射和刷新时效；任一必需字段缺失时不得进入分配向导联调或上线验收。
...
| 财务日常 / 核心配置 | 账单配置 | 维护客户应收和返款自定义配置、共享配置母版、母版版本及客户分配，包括账期、履约节点、限定情形和币种 | <a href="#section-7">7. 客户级账单配置</a> |
| 财务日常 / 核心配置 | 汇率配置 | 维护基准汇率、客户自定义特调配置、共享特调汇率母版及客户分配，查看客户最终默认汇率结果 | <a href="#section-8">8. 客户级汇率配置</a> |
...
6. 账单类邮件发送成功或失败都必须记录发送时间、收件人、抄送人、模板版本、账单编号、附件名称和失败原因；客户级账单配置变更通知发送成功或失败必须记录发送时间、收件人、抄送人、模板版本、配置来源、准确配置或母版版本、生效周期和失败原因。发送失败只进入对应页面的状态提示，不另行触发面向内部人员的邮件。
7. 客户邮箱来自客户主数据；客户自定义配置和共享配置母版都不保存客户邮箱。未配置客户邮箱时，系统不得自动发送客户邮件。账单类邮件应在对应账单列表的`通知状态`展示`未配置邮箱`标签；客户级账单配置变更通知不挂靠具体账单，应在配置变更通知记录中记录`未配置邮箱`，不回写应收账单列表或返款账单列表。
...
10. 客户级账单配置变更通知只按本次实际成功改变来源或准确版本的客户清单逐客户发送。仅发布母版新版本、修改母版可分配范围、未勾选客户或客户已引用同一准确版本时不发送；母版分配、母版版本升级、强制覆盖和切换为自定义配置成功后均按成功客户发送。未配置邮箱的客户记录`未配置邮箱`，不阻断其它客户发送。
...
| 当客户的账单配置来源或准确版本变更确认后 | `{{客户名称}} 账单配置变更通知 {{配置生效周期}}` | 尊敬的客户，您好：<br/>贵司账单配置已发生变更，配置来源：`{{配置来源}}`，配置版本：`{{配置版本}}`，生效周期：`{{配置生效周期}}`。本次变更内容：`{{配置变更摘要}}`。<br/>该变更不会自动改写已有账单；尚未向贵司发出的待审核账单如需采用新来源版本，将由我司财务确认后重新生成；已经向贵司发出的账单仍以原账单内容为准。如对配置变更内容有疑问，请及时与我司对接人员联系。 | 无 |
...
| `{{配置来源}}` | `客户自定义配置`或`共享配置母版`；母版来源可附母版名称或编号 | 客户级账单配置变更通知 |
| `{{配置版本}}` | 客户自定义配置本次生效的版本号，或客户本次引用的母版准确版本号 | 客户级账单配置变更通知 |
...
12. 按生成批次编号、任务编号、配置编号或准确版本筛选后的批量下载：财务在应收账单或返款账单列表按任一上述快照字段筛选出账单后，可以对筛选结果批量下载，无需逐张勾选；生成批次编号和任务编号基于账单关联的任务快照匹配，配置编号和准确版本基于账单已固化的两个独立字段匹配。自定义来源使用客户自定义配置编号，母版来源使用母版编号，客户当前引用变化不得改变结果。应收账单仍可选择`导出给客户`或`导出给内部`，返款账单固定为`导出给客户`。系统在导出任务中固化本次筛选条件和账单清单（例如`生成批次编号 / BMSB-20260731-0006`、`任务编号 / BMS-20260731-0001`或`母版编号 / ARB-MASTER-20260701-01 + 准确版本 / V2`），执行导出时不得按客户当前引用重新展开；压缩包内各客户账单仍分别生成独立 Excel，文件名以客户编号或账单编号区分，不得重复。
