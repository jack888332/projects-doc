...
const exportRows = useDemoDataset('billingExports', [
  { no: 'EXP-20260802-0018', billType: '应收账单', purpose: '导出给客户', format: '不适用', fileType: '压缩包', bills: 8, processed: 8, success: 7, failed: 1, status: '部分成功', progress: 100, creator: '谭清辉', createdAt: '2026-08-02 10:02', finishedAt: '2026-08-02 10:05', expiresAt: '2026-08-03 10:05' },
  { no: 'EXP-20260802-0017', billType: '应收账单', purpose: '导出给内部', format: '合并式', fileType: '表格文件', bills: 12, processed: 12, success: 12, failed: 0, status: '导出成功', progress: 100, creator: '郑雅雯', createdAt: '2026-08-02 09:46', finishedAt: '2026-08-02 09:48', expiresAt: '2026-08-03 09:48' },
  { no: 'EXP-20260802-0016', billType: '返款账单', purpose: '导出给客户', format: '不适用', fileType: '表格文件', bills: 10, processed: 6, success: 6, failed: 0, status: '导出中', progress: 62, creator: '谭清辉', createdAt: '2026-08-02 09:31', finishedAt: '-', expiresAt: '-' },
  { no: 'EXP-20260802-0015', billType: '返款账单', purpose: '导出给客户', format: '不适用', fileType: '压缩包', bills: 5, processed: 0, success: 0, failed: 0, status: '待执行', progress: 0, creator: '谭清辉', createdAt: '2026-08-02 09:26', finishedAt: '-', expiresAt: '-' },
  { no: 'EXP-20260801-0041', billType: '应收账单', purpose: '导出给内部', format: '拆分式', fileType: '表格文件', bills: 6, processed: 6, success: 0, failed: 6, status: '导出失败', progress: 100, creator: '郑雅雯', createdAt: '2026-08-01 17:10', finishedAt: '2026-08-01 17:12', expiresAt: '-' },
], 3)
...
<template>
  <div class="module-page">
    <PageHeader v-if="!embedded && mode === 'audit'">
      <template #export><el-button :icon="Download" @click="action('审计导出任务')">导出审计结果</el-button></template>
    </PageHeader>
    <template v-if="mode === 'base'">
      ...
      <section class="module-panel filter-table-panel"><div class="module-toolbar"><div class="condition-filter-bar"><ConditionFilter v-model="query.keyword" label="关键词" type="text" search-placeholder="编码 / 名称 / 场景 / 数据源" /></div><span class="module-result-count">{{ filtered.length }} 条配置</span></div>
...
<el-table v-else :data="filtered" class="clean-table"><el-table-column prop="no" label="模板编号" /><el-table-column prop="name" label="模板名称" /><el-table-column prop="default" label="默认结算币种" /><el-table-column prop="mapping" label="费项币种映射" min-width="320" /><el-table-column prop="status" label="状态" /><el-table-column prop="operator" label="操作人" /><el-table-column prop="updatedAt" label="操作时间" /><el-table-column label="操作" width="64"><template #default><HoverActionMenu><el-dropdown-item :icon="EditPen" @click="action('编辑结算币种模板')">编辑</el-dropdown-item></HoverActionMenu></template></el-table-column></el-table>
        <TablePagination :total="filtered.length" :page-size="20" />
      </section>
    </template>
    ...
    <template v-else-if="mode === 'exports'">
      <section class="module-panel filter-table-panel"><div class="module-toolbar"><div class="condition-filter-bar"><ConditionFilter v-model="query.keyword" label="任务编号" type="text" /><ConditionFilter v-model="query.type" label="账单类型" :options="['应收账单','返款账单']" /><ConditionFilter v-model="query.purpose" label="导出用途" :options="['导出给客户','导出给内部']" /><ConditionFilter v-model="query.format" label="内部格式" :options="['拆分式','合并式']" /><ConditionFilter v-model="query.status" label="任务状态" :options="['待执行','导出中','导出成功','部分成功','导出失败','已取消']" /><ConditionFilter v-model="query.creator" label="创建人" :options="['谭清辉','郑雅雯']" /><ConditionFilter v-model="query.dateRange" label="创建时间" type="date-range" /><div class="condition-filter-actions"><el-button type="primary" @click="applyQuery">查询</el-button><el-button @click="resetQuery">重置</el-button></div></div></div>
        <div class="table-reference-toolbar"><TableFieldSortButton /><div class="table-reference-actions"><el-button :icon="Setting" @click="exportConfigVisible = true">对账报表配置</el-button><el-button type="primary" :icon="Plus" @click="action('新建导出任务')">新建导出</el-button></div></div>
        ...
        <TablePagination :total="filtered.length" :page-size="20" :summary="`共 ${filtered.length} 行，已选 0 行`" />
      </section>
     <el-dialog v-model="exportDetailVisible" title="导出任务详情" width="820px" align-center><template v-if="selectedExport"><dl class="detail-grid"><div><dt>任务编号</dt><dd>{{ selectedExport.no }}</dd></div><div><dt>任务状态</dt><dd>{{ selectedExport.status }}</dd></div><div><dt>账单类型 / 用途</dt><dd>{{ selectedExport.billType }} / {{ selectedExport.purpose }}</dd></div><div><dt>内部导出格式</dt><dd>{{ selectedExport.format }}</dd></div><div><dt>处理进度</dt><dd>{{ selectedExport.processed }} / {{ selectedExport.bills }}（{{ selectedExport.progress }}%）</dd></div></dl><div class="table-reference-toolbar"><TableFieldSortButton /></div>
<el-table :data="selectedExportItems" border><el-table-column prop="billNo" label="账单编号" width="230" /><el-table-column prop="result" label="导出结果" width="100" /><el-table-column prop="output" label="Sheet / 文件" width="180" /><el-table-column prop="reason" label="失败原因" min-width="220" /></el-table><TablePagination :total="selectedExportItems.length" :page-size="20" /></template></el-dialog>
