...
> 本篇定位：BMS任务生命周期与操作；主要内容：账单生成/重算机制、任务触发、任务状态与操作留痕；文档角色：模块档；文档ID：D94FE560DB

<a id="doc-D94FE560DB-task-lifecycle-48b1e1bb"></a>
## 1. 账单生成/重算机制
...
账单处理依次经过任务触发、配置取用、来源筛选、费项入池、账单计算和结果保存；账单形成后的差异处理、页面操作与记录要求在本章末尾统一规定。

<a id="doc-D94FE560DB-task-lifecycle-527a0a10"></a>
### 1.1 一张账单如何由BMS任务形成

<a id="doc-D94FE560DB-task-lifecycle-897fe491"></a>
#### 1.1.1 费项入池、账单生成与重算的执行路径
...
同一套任务流程按任务目的执行到不同阶段：费项入池任务在费用写入`BMS费项池`后结束；账单生成任务从来源筛选执行到账单结果保存；账单重算任务跳过来源同步和费项入池，直接读取已有账单。账单形成后又出现变化时，进入第七步处理差异。
...
2. `账单生成`按以下方式执行：
    - 财务可在账期结束前或结束后按客户点击`账单生成`，也可从配置准确版本批量发起。
    - 配置批量发起先冻结提交参数并预校验；至少一个范围通过时，系统原子创建非执行批次及对应的客户独立任务，全部范围跳过时只保存提交校验审计。
    - 目标范围没有有效账单时首次生成，已有可沿用的从未发出`待审核`账单时补充生成，本次配置改变账期范围、分组结果或费项归属且满足替换条件时替换生成。
3. `账单重算`：只读取目标账单及其有效调账记录，不重新筛选业务来源数据。同时存在新增费项和重算需求时，先通过账单生成任务补充生成，再按补充后的结果重算。
...
账期结束前生成的账单为`待审核 + 未收口`，默认只在财务内部处理。财务可以继续等待期末收口，也可以按[账单生成、重算与变更分流](PRD.账单系统.第010篇.账单生成重算与变更分流.4CA3C606DB.md#doc-4CA3C606DB-billing-calculation-6868a655)中的[账期还没结束，怎样提前收口并审核？](PRD.账单系统.第010篇.账单生成重算与变更分流.4CA3C606DB.md#doc-4CA3C606DB-billing-calculation-7e705a70)确认截断范围。系统必须先把截断后的实际账期更新为`已收口`，再完成审核和发出，不允许直接形成`待结清 + 未收口`。

账单审核通过后，系统才按客户级账单配置向客户发出账单；`账单发出时间`不触发任务。

任务必须记录`当前执行阶段`并为已经成功的阶段保存检查点。任务因技术异常中断后，重新执行从失败阶段继续，不重复执行已完成的来源筛选或费项入池；需要采用新配置、新范围或新数据截止点时，必须创建新任务。

<a id="doc-D94FE560DB-task-lifecycle-984e810a"></a>
#### 1.1.2 任务、处理范围、快照与账单结果定义

本章术语分为任务执行、任务处理范围和账单结果三个层次。同一术语在任务、账单和页面中保持相同含义。
...
| `账期结束` | 一个完整账期或截断后的实际账期到达其结束时点。完整账期的结束时点由客户级账单配置确定；提前收口形成的截断账期，以数据截止点所在日期作为实际账期结束日，并以数据截止点作为新旧范围的精确边界。 | 它是时间和范围事实，只表示该账期范围已经走到末端，不自动表示期末数据已经处理完整，也不自动改变账期收口状态。 |
| `账期收口` | 系统针对一张账单的实际账期完成期末费项入池或返款明细归集、任务冲突检查、费项归属检查、币种分桶和金额一致性校验，并把账单的账期收口状态更新为`已收口`。 | 它是业务处理结果，不是账期结束的同义词，也不是任务类型或账单生成方式。账期已经结束但校验未通过时仍为`未收口`；原账期尚未结束时，只有先截断实际账期并通过该截断范围的收口校验，才能提前收口。 |
...
`账期结束`与`账期收口`必须分别判断。常规期末场景先到达账期结束时点，再由系统执行本账期最后一个`账单生成`任务；任务通过全部收口条件后，账单才从`未收口`更新为`已收口`。任务失败、来源数据不完整、存在冲突任务或金额校验未通过时，账期即使已经结束，账单仍保持`未收口`。提前收口场景则先把原完整账期截断为新的实际账期，再对该实际账期完成收口；截断点之后的剩余范围由后续账单承接。账单尚未发出且又出现影响范围或金额的数据变化时，原`已收口`结果可以按规则恢复为`未收口`并重新校验。因此，`已收口`只表示当前实际账期通过收口校验，不改变原完整账期的配置结束时点，也不表示账单已经结清。
...
| `账单分组键` | 在同一客户、账单类型、实际采用的配置编号及准确版本、方案标识和实际账期内，用于继续区分具体账单的业务维度组合。 | 所有客户账单都必须包含来源订单的`所属店铺快照`；应收账单在此基础上再使用`业务板块 + 运抵国`，返款账单不再增加其他拆分维度。分组键不同必须形成不同账单。 |
...
| `配置移出待处理` | 原账单费项因新版配置不再适用，且暂时无法归入其他候选新账单时的费项处置标记。 | 它不是账单状态；被标记费项不得因释放占用而被后续任务自动归入其他账期。 |
...
4. 计算成功后，首次生成为新账单保存首个结果版本，补充生成在目标账单下保存后续结果版本。替换生成先形成候选新账单集合，并在整体成功后切换新旧账单；所有旧结果继续保留供追溯。
5. 补充生成没有发现新增或更正结果时，任务以`无需补充`成功结束，不创建空的结果版本。

配置准确版本批量生成只改变任务发起方式，不改变单条任务的执行流程。系统先按配置编号和准确版本冻结本批次客户 / 会员当前的会员编码、所属店铺、所属客户组和方案范围，再按`客户 + 账单类型 + 方案标识 + 实际账期`为每个可创建范围生成独立任务。每条任务冻结所用准确版本和客户 / 会员归属快照，并分别形成任务状态、快照和结果。

任务仍以客户 / 会员为执行主体；执行中识别出的费用按来源订单的所属店铺快照分别形成账单。

<a id="doc-D94FE560DB-task-lifecycle-cdc718d2"></a>
#### 1.1.3 账单生成与账单重算的职责边界

`账单生成`决定哪些费项进入账单：系统先同步来源数据，再从`BMS费项池`中选出符合账期和配置范围、尚未出账的费项，用于创建、补充或替换账单。

`账单重算`决定已有账单怎样重新得出金额：系统锁定一张已经存在的目标账单，在该账单已有明细范围内，结合已经审核生效且明确归属于该账单的调账记录，重新计算允许变化的明细、汇率、币种分桶和汇总金额。重算不重新发现来源数据，也不把最新账单配置套用到历史账单。

这里的`账单已有明细范围`，是指已经归属于目标账单的费项池记录，以及审核通过后作为账单级结果计入的`金额冲正记录`（金额冲正不进入费项池）；`计算结果`是基于这些记录得到的明细金额、结算币种金额、汇率换算结果、币种分桶、账单总额和未结余额。
...
| 能否加入调账记录 | 只纳入已经形成费项池记录且符合本期范围的普通来源费项与补录费项；金额冲正记录不进入费项池，不作为账单生成输入。 | 只读取已经审核生效且明确指向目标账单的金额冲正记录；金额冲正不进入费项池，审核通过后直接计入账单汇总分桶。 |
...
<a id="doc-D94FE560DB-task-lifecycle-63d493d9"></a>
#### 1.1.4 任务执行的数据唯一性与历史保护约束

1. 创建任务时必须立即锁定本任务规则：
    - `费项入池`和账单生成锁定任务创建时生效的来源规则、处理范围、数据截止点及本次采用的客户级账单配置；账单重算锁定目标账单已经使用的配置版本。
    - 账单生成方式在任务取得执行权并完成重复检查、既有账单检查和配置影响检查后，确定为`首次生成`、`补充生成`或`替换生成`。
    - 补充生成没有新增结果时，任务以`无需补充`的执行结论成功结束，不新增结果版本。
    - 排队和执行期间发生的其他配置变化不得影响已创建任务。
...
<a id="doc-D94FE560DB-task-lifecycle-64869658"></a>
### 1.2 BMS任务创建条件与成功结果
...
<a id="doc-D94FE560DB-task-lifecycle-c2537763"></a>
#### 1.2.1 定时任务、手动任务与批量生成入口
...
3. 系统必须防止同一范围重复创建任务，并按以下口径判定任务身份、范围占用和冲突：
    - 任务身份必须记录`客户 + 账单类型 + 方案标识（返款固定为 REFUND） + 实际账期 + 配置编号及准确版本 + 任务类型`，并另存方案名称、方案类型、会员编码、所属店铺和所属客户组快照用于展示与审计；每项当前归属快照均为单值。
    - 来源处理范围按`客户 + 账单类型 + 方案标识 + 配置准确版本 + 来源数据集 + 处理时间范围`判定。
    - 账单计算范围按`客户 + 账单类型 + 实际账期 + 方案标识 + 目标账单或替换批次 + 任务类型`判定，不得仅因配置编号、版本、方案名称或方案类型不同而放行同一账单结果的并发计算。
    - 任务在客户粒度占用处理范围，一个客户任务可在执行后形成多个来源订单所属店铺快照下的账单；来源订单所属店铺快照用于账单分组、唯一识别和结果追溯，不把任务拆成店铺任务，也不缩小任务创建时的客户级并发保护。
    - `费项入池`任务占用来源处理范围；`账单生成`任务同时占用来源处理范围和账单计算范围；`账单重算`任务只占用账单计算范围。
    - 同一范围已有`待执行`、`执行中`或尚未删除的`执行失败`任务时，不得再次创建。失败任务重新执行成功或被财务删除后，才释放其占用范围。
...
6. 任务执行快照不得重复保存账单配置、来源规则、费项规则或汇率快照的完整内容，只保存所引用业务快照的编号、版本和校验值，以及本次执行特有的范围、时点、目标对象和操作参数。不同任务类型需要保存的具体内容见[第二步：任务按哪套规则执行，又需保存什么？](PRD.账单系统.第008篇.任务快照与配置衔接.A82E89D72C.md#doc-A82E89D72C-task-snapshot-7cd5d599)。
7. 任务编号、批次编号和任务记录按以下规则生成：
    - 单客户操作和系统定时触发生成一个唯一任务编号。
    - 配置准确版本批量生成一个唯一批次编号，并为每个通过校验的任务范围分别生成唯一任务编号；批次编号只能用于关联、筛选和汇总，不得代替任务编号进入队列或占用处理范围。
    - 任务必须记录任务类型、账单生成方式、内部收口结果、数据截止点、当前执行阶段，以及各阶段的开始时间、结束时间、执行结果和检查点。
    - 任务类型不是`账单生成`时，账单生成方式显示为`不适用`；账单生成任务尚未取得执行权时显示为`待判定`。内部收口结果不向用户展示。
8. 配置批量生成按以下规则确认和创建：
    - 系统先按配置编号和准确版本冻结财务最终选择的客户引用、每个客户 / 会员的会员编码、所属店铺和所属客户组快照、方案标识、方案名称、方案类型、实际账期和数据截止点，再逐范围执行配置生效、防重和权限预校验。
    - 至少一个范围通过时，系统在同一事务中创建批次和所有通过校验的任务，并把未通过范围记录为`跳过`。
    - 没有任何范围可创建时，不创建批次和任务，只保存本次提交校验审计并在页面保留客户、方案、账期和失败原因。
    - 任务执行时实际进入账单的费用仍按来源订单自身的所属店铺快照归集，批次确认时的客户 / 会员当前归属快照不得覆盖订单快照。
9. 已成功完成的阶段结果可以在原任务重新执行时复用，但不得跨任务复用为新的任务输入。新触发、新配置、新范围或新数据截止点必须创建新任务，从规则与范围锁定阶段重新执行。

<a id="doc-D94FE560DB-task-lifecycle-3ea18997"></a>
#### 1.2.2 任务阶段、状态与失败恢复
...
删除账单生成或账单重算任务后，任务不再出现在任务列表并释放其占用范围，但系统仍须保留任务编号、删除前状态、删除人和删除时间。重新执行不得改用新配置或扩大原范围；需要采用新配置或新范围时，财务应删除失败任务，再从对应业务入口创建新任务。

同一范围存在尚未删除的失败任务时，系统不得接受新的同范围任务。费项入池任务不开放删除和重新执行入口，其失败恢复与范围释放由系统内部处理。

账单生成批次不提供整体重新执行或整体删除。批次查看结果分别展示`初始创建任务数`、`当前有效任务数`、`已删除任务数`，并按当前有效任务汇总`待执行`、`执行中`、`执行成功`和`执行失败`数量，另单列创建时的`跳过`数量。

被删除任务不再出现在主任务列表，但在批次关联清单中保留只读审计行及删除前状态、删除人和删除时间；某条任务失败时，财务只对该任务执行重新执行或删除，其他任务状态和结果不受影响。重新执行沿用原任务编号、批次编号和任务执行快照。
...
<a id="doc-D94FE560DB-task-lifecycle-2c544758"></a>
### 1.3 财务在哪里操作，页面如何反馈？

财务通过任务页面、账单详情、调账中心和费项配置工作台执行相关操作。各页面只能开放当前账单状态允许且当前用户有权限执行的操作。

<a id="doc-D94FE560DB-task-lifecycle-e31fa38c"></a>
#### 1.3.1 各页面的任务操作职责

| 页面入口 | 允许操作 | 状态与权限约束 |
| :--- | :--- | :--- |
| 账单任务列表 | 统一查看BMS任务、错误、快照及结果，并按任务状态执行允许的后续操作 | 不提供费项入池的创建入口；操作入口按[任务阶段、状态与失败恢复](#doc-D94FE560DB-task-lifecycle-3ea18997)开放。 |
...
| 应收账单详情 | 账单生成、账单重算、提前收口并审核、汇率重算、补录、金额冲正、导出 | `待审核 + 未收口`只允许通过提前收口完成审核；账单生成的实际方式由系统判定，替换生成仅适用于尚未发出的待审核账单；已作废和已结清不提供计算入口。 |
| 返款账单详情 | 账单生成、账单重算、提前收口并审核、返款金额修正、扣减项调整、折返审核、导出 | 使用与应收账单相同的收口和替换边界；不得通过返款账单操作改写应收账单。 |
| 调账中心 | 新增、导入、审核、移除金额冲正 / 核销记录冲正和成本账单调账 | 金额冲正审核通过后影响账单金额；核销记录冲正审核通过后更新已收 / 已付与核销进度口径，不改变账单金额。 |
| 核销记录区 | 查看核销记录、发起核销记录冲正 | 核销错误跳转调账中心新增核销记录冲正；不得修改或删除原核销记录。 |
| 数据源规则 | 查看数据集、来源表、时间口径和引用范围 | 日常财务以查看为主；变更只影响后续任务。 |

页面操作权限统一遵守[权限与职责边界](PRD.账单系统.第018篇.系统级非功能需求.F88D5AB332.md#doc-F88D5AB332-nonfunctional-a7bb77b2)，并补充以下约束：
...
<a id="doc-D94FE560DB-task-lifecycle-6d78aa65"></a>
#### 1.3.2 任务列表字段、筛选与展示

任务页面使用一个统一任务列表，通过`任务类型`筛选`费项入池`、`账单生成`或`账单重算`，不建立父子任务关系。配置准确版本批量发起形成的任务平铺展示，并通过生成批次编号关联；批次查看区只汇总关联任务，不成为父任务。

任务列表默认平铺展示，同时提供`客户 × 账期`聚合视图：按客户编码和实际账期折叠为组，组行显示客户、账期、方案数、任务数、状态汇总和生成批次，展开后逐条展示组内的方案级独立任务及其状态和操作。聚合视图只改变展示层，不改变任务粒度、任务状态、范围占用和操作归属。

只有账单生成任务展示`账单生成方式`：取得执行权前显示`待判定`，完成判定后显示`首次生成`、`补充生成`或`替换生成`；其他任务类型统一显示`不适用`。
...
各任务状态允许的操作和自动重试规则见[任务阶段、状态与失败恢复](#doc-D94FE560DB-task-lifecycle-3ea18997)。页面只展示财务关注的四种任务状态；用于调度、并发控制和故障恢复的内部标记不得作为筛选项，也不得进入客户报表或通知。

<a id="doc-D94FE560DB-task-lifecycle-cdb2c686"></a>
### 1.4 操作前预览与操作后留痕

1. 财务手动发起`账单生成`或`账单重算`前，页面必须按操作类型展示以下内容：
    - 发起`账单生成`前展示客户、账单类型、实际账期、当前配置版本、数据截止点、当前收口状态和预计处理范围。
    - 允许财务确认本次采用当前配置还是新版配置，不得要求财务预先选择首次生成、补充生成或替换生成。
    - 选择新版配置时，展示新旧配置、账期、费项和金额差异及可能触发替换生成的提示。
    - 发起`账单重算`前展示目标账单、原配置版本、重算范围、当前有效结果版本和已核销 / 已返款情况。
2. 待结清账单已有部分核销时，折返或重算前必须二次确认，并明确已核销记录不会被覆盖。
3. 已作废和已结清账单不得显示生成、重算或覆盖入口。已经发出的账单不得显示作废或替换生成入口；核销记录错误通过调账中心新增核销记录冲正，收款、回款或返款错误按对应资金纠错流程处理，账单差异由金额冲正或后续账单承接。
4. 任务失败时，必须展示财务可识别的错误原因和处理建议；任务执行阶段不在页面展示。
5. 同一费项被应收和返款同时命中时，系统按返款扣减优先处理并记录分流结果；按既定规则仍无法确定唯一归属时，任务失败并向财务提示冲突原因。
6. 补录、金额冲正、核销记录冲正和批量操作提交前必须预览校验；跨客户、跨账期、跨配置或跨币种错误时不得提交。
7. 所有首次生成、补充生成、替换生成、期末收口、账单重算、金额冲正、核销记录冲正和折返审核，必须记录操作对象、原因、前值、后值、操作人、时间和结果。
