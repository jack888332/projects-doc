...
每次小调整都运行完整验证链路，不仅耗时，还会增加命令输出、文件读取和分析所消耗的 Token，不符合产品原型快速迭代的目标。因此，默认采用“聚焦修改、人工验收”的轻量流程：只读取并修改受影响文件，不主动执行生产构建、完整测试、Playwright、截图或全项目扫描。完整验证链路只能由用户明确提出后发起，不得因改动共享逻辑、类型边界或数据结构而自行升级验证范围；这是一套用于快速讨论和演示的产品原型，不按面向最终用户交付的软件执行默认质量门禁。必要时仅运行与当前改动直接相关的轻量静态检查。
...
该命令执行架构审计、类型检查、单元测试和生产构建。完整浏览器回归同样必须由用户明确要求后执行：
