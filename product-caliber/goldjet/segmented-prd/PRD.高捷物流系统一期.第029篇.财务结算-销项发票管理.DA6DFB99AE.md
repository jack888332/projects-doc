...
- **区域字段或业务元素**：复用[销项票基本信息字段（查看）](#doc-DA6DFB99AE-section-942231915cd5)定义的字段；本场景字段或差异项：全部字段。
...
- **区域字段或业务元素**：复用[票面货物及服务信息字段（查看）](#doc-DA6DFB99AE-section-942231915cd5)定义的字段；本场景字段或差异项：货物或应税劳务名称、规格型号、单位、数量、单价(含税)、金额(含税)、税率(%)、税额。
...
| 单价(含税)、税额 |  | 回显当前编辑记录；保存后自动计算；[计算规则见销项票申请编辑页面](PRD.高捷物流系统一期.第029篇.财务结算-销项发票管理.DA6DFB99AE.md#doc-DA6DFB99AE-section-f06057042524)；只读。 |
...
| 本次开票金额(人民币) |  | 保存后自动更新；[计算规则见销项票申请编辑页面](PRD.高捷物流系统一期.第029篇.财务结算-销项发票管理.DA6DFB99AE.md#doc-DA6DFB99AE-section-f06057042524)；只读。 |
...
- **区域字段或业务元素**：复用[销项票基本信息字段（查看）](#doc-DA6DFB99AE-section-942231915cd5)定义的字段；本场景字段或差异项：开票申请号、结算单位、创建日期、创建人、原币币种、申请总金额(原币)、即期汇率、需要开票。
...
| 规格型号、单位 |  | 选填；无预设值；可编辑。 |
| 单价(含税)、税额 |  | 保存后自动计算；[计算规则见销项票申请编辑页面](PRD.高捷物流系统一期.第029篇.财务结算-销项发票管理.DA6DFB99AE.md#doc-DA6DFB99AE-section-f06057042524)；只读。 |
...
| 主业务流程 | 勾选需要进行销项票申请的对账明细，进入“销项票申请维护”页面，选择“是否需要开票”、税率和购买方名称，录入开票金额(CNY)、即期汇率、备注和行本次开票金额(原币)，可保存或提交销项票申请。<br>1. 即期汇率<br>- 系统自动生成，允许修改。<br>- 未保存前，可重新录入修改。<br>1. 需要开票：<br>- 从候选项中选择（是/否）；默认为“是”，特殊业务下需要修改为“否”<br>- 特殊业务说明：已经提前开票或不需要开票数据，则可选择否。若特殊业务该销项票不修改状态，可能会造成重复开票情况。<br>- 未保存前，可重新选择修改。<br>1. 购买方名称<br>- 默认带出该客户开票单位中标记为“默认”的数据，可修改。<br>- 候选列表仅展示开票单位维护表中该结算单位下状态为“已生效”的开票明细。<br>- 未保存前，可重新选择修改。<br>1. 开户行名称：<br>- 默认带出该操作用户标记为“基本户”的银行，可修改；<br>- 候选列表仅展示该组织下所有银行账户，并显示开户行名称和银行账号。<br>- 未保存前，可重新选择修改。<br>1. 货物或服务信息<br>- 从候选项中选择。<br>- 未保存前，可重新选择修改。<br>1. 规格信息<br>- 手工录入。<br>- 未保存前，可重新选择修改。<br>1. 单位<br>- 手工录入。<br>- 未保存前，可重新录入修改。<br>1. 数量<br>- 手工录入数量，最多保留2位小数。<br>- 未保存前，可重新录入修改。<br>1. 金额(含税)<br>- 手工录入金额，最多保留2位小数。<br>- 未保存前，可重新录入修改。<br>1. 税率(%)<br>- 从候选项中选择，指定需要开票的税率。<br>- 未保存前，可重新从候选项中选择修改。<br>1. 收款人<br>- 默认当前用户，可从候选项中选择修改。<br>- 未保存前，可重新从候选项中选择修改，只能选择当前组织下的用户。<br>1. 复核人<br>- 默认当前用户，可从候选项中选择修改。<br>- 未保存前，可重新从候选项中选择修改，只能选择当前组织下的用户。<br>1. 备注<br>- 手工录入。<br>- 未保存前，可重新录入修改。<br>1. 本次开票金额(原币)<br>- 默认可开票金额(原币)。<br>- 未保存前，可手工录入修改。<br>录入完成后，点击“保存”；录入错误时可删除并重新维护。<br>点击“保存”弹出保存确认<br>- 保存校验：必填字段是否填写。若必填信息没有填写，无法保存。<br>- 保存完成后，自动生成销项开票申请号，规则“IA+6位年月日(YYMMDD)+5位流水”。<br>- 保存完成后，该开票申请状态为“新建”，可在开票申请列表界面查询和维护。<br>点击“提交开票”弹出提交确认<br>- 保存校验：必填字段是否填写。若必填信息没有填写，无法保存。<br>- 保存完成后，自动生成销项开票申请号，规则“IA+6位年月日(YYMMDD)+5位流水”。<br>- 提交完成后，该开票申请状态为“已提交”，等待开票系统返回结果。<br>- 开票成功后，向提交人发送消息。 |
| 业务分支 | 销项票支持发送给对应客户。 |
...
- **区域字段或业务元素**：复用[票面发票信息字段（查看）](#doc-DA6DFB99AE-section-942231915cd5)定义的字段；本场景字段或差异项：发票号、发票类型、是否作废。
- **区域级通用规则**：只有匹配到状态为“未作废”，且没有被开票申请关联的该结算单位下所有开票信息中的税务发票。
...
| 主业务流程 | 选择销项发票补录的开票申请号，进入“销项发票补录维护”页面，填写或选择发票号，保存税务发票手工补录，并提交该销项票申请。<br>1. 发票号：<br>- 可手工填写或从候选项中选择；候选列表仅展示与当前开票申请的购买方、纳税识别号相同且尚未关联的税务发票。<br>- 保存前可重新录入。<br>录入完成后点击“保存”；录入错误时可删除并重新维护。<br>点击“提交”完成销项发票补录。<br>- 提交校验：原币币种为 CNY 时，校验税务发票行开票金额合计与头信息中的申请金额(原币)是否相同；不同时提示“‘销项发票申请金额XX’与‘税务发票合计金额XX’不一致，无法保存提交”。 |
...
| 字段或控件特例 | 适用条件 | 特例约束或业务结果 |
| --- | --- | --- |
| 不含税金额 |  | 自动计算；[计算规则见税务发票查询列表页面](PRD.高捷物流系统一期.第029篇.财务结算-销项发票管理.DA6DFB99AE.md#doc-DA6DFB99AE-section-f927b591e950)。 |
