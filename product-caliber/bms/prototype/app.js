...
const costNavigation = `
  ...
  <button class="nav-item" data-view="suppliers"><i data-lucide="building-2"></i><span>供应商管理</span></button>
  <button class="nav-item" data-view="bills"><i data-lucide="file-spreadsheet"></i><span>成本账单</span><span class="nav-count">8</span></button>
  <button class="nav-item" data-view="pool"><i data-lucide="database"></i><span>成本池</span><span class="nav-count warn">23</span></button>
  <div class="nav-group-label">分析与配置</div>
  <button class="nav-item" data-view="profit"><i data-lucide="chart-no-axes-combined"></i><span>利润分析</span></button>
  <button class="nav-item" data-view="rules"><i data-lucide="list-checks"></i><span>分摊规则</span></button>
...
const billingNavigation = `
  ...
  <button class="nav-item" data-billing-view="adjustments"><i data-lucide="sliders-horizontal"></i><span>调账中心</span></button>
  <div class="nav-group-label">核心配置</div>
  <button class="nav-item" data-billing-view="config"><i data-lucide="settings"></i><span>账单配置</span></button>
...
function renderDomainNavigation() {
  ...
  document.querySelector(".side-title").textContent = billing ? "账单系统" : "成本中心";
  document.querySelector(".side-nav").innerHTML = billing ? billingNavigation : costNavigation;
  document.querySelector(".sidebar-foot span").textContent = billing ? "账单系统操作指引" : "成本中心操作指引";
...
const detailGrid = (items) => `<div class="detail-grid">${items.filter(Boolean).map(([label, value, valueClass = "", itemClass = ""]) => `<div class="detail-item${itemClass ? ` ${itemClass}` : ""}"><span class="detail-label">${label}</span><span class="detail-value${valueClass ? ` ${valueClass}` : ""}">${value}</span></div>`).join("")}</div>`;
...
function rowMoreActions(items) {
  return `<span class="row-more-actions"><button class="row-more-trigger" type="button" title="更多操作" aria-label="更多操作"><span class="custom-more-icon" aria-hidden="true"></span></button><span class="row-more-menu" role="menu">${items}</span></span>`;
}

function rowMenuItem(label, action, id, className = "") {
  return `<button class="row-menu-item ${className}" type="button" role="menuitem" data-action="${action}" data-id="${id}">${label}</button>`;
}

function tableDetailButton(action, id) {
  return `<button class="table-detail-button" type="button" title="详情" aria-label="详情" data-action="${action}" data-id="${id}">${icon("eye")}</button>`;
}
...
function supplierDrawer(id) {
  ...
  showDrawer(`供应商财务档案 · ${s.name}`, `<div class="header-actions" style="justify-content:flex-start;margin-bottom:var(--space-3)"><button class="btn primary" data-action="edit-supplier" data-id="${s.code}">${icon("pencil")}编辑档案</button><button class="btn" data-view="bills">查看成本账单</button></div>
    ${detailGrid([
      ["供应商编码", s.code], ["供应商状态", status(s.state)], ["供应商名称", s.name],
      ["默认币种", s.currency], ["适用成本板块", renderSupplierBoardTags(boards)], ["最近更新时间", s.updated],
    ])}
    <div class="drawer-section"><h3>成本板块账期配置</h3>${renderSupplierPeriodConfigTable(s)}</div>
    <div class="drawer-section"><h3>金额统计</h3><div class="kpi-grid" style="grid-template-columns:repeat(3,1fr)"><div class="kpi-card"><div class="kpi-label">成本账单</div><div class="kpi-value">${s.bills}</div></div><div class="kpi-card warning"><div class="kpi-label">待结清金额</div><div class="kpi-value" style="font-size: var(--font-size-lg)">${s.pending}</div></div><div class="kpi-card success"><div class="kpi-label">已结清金额</div><div class="kpi-value" style="font-size: var(--font-size-lg)">${s.settled}</div></div></div></div>
...
function renderBillDetail(){
  ...
  return `<div class="bill-detail-page">${pageHeader("成本账单详情", `${b.id} · ${b.supplier}`, actions)}
    <section class="panel"><div class="panel-head"><span class="panel-title">账单基本信息</span></div><div class="panel-body">${detailGrid([
      ["成本账单编号", b.id, "mono"], ["结清状态", status(b.state)], ["供应商", b.supplier], ["成本板块", b.board],
      ["实际成本账期", b.period], ["原始文件", b.file], ["账单金额", money(b.amount,b.currency), "strong"], ["已结清金额", money(b.settled,b.currency)],
    ])}</div></section>
...
function costDrawer(id){const c=costs.find(x=>x.id===id);showDrawer("成本明细详情",`<div class="header-actions" style="justify-content:flex-start;margin-bottom:var(--space-3)"><button class="btn" data-action="toggle-cost" data-id="${c.id}">${icon("repeat-2")}切换成本类型</button><button class="btn">${icon("link")}维护单号关系</button></div>${detailGrid([
  ["成本明细编号",c.id,"mono"],["处理状态",status(c.status)],["供应商",c.supplier],["成本板块",getCostBoardLabel(c)],["供应商原始费项",c.raw],
  ["标准成本费项",c.fee],["关键单号",`${c.keyType} · ${c.key}`],["原始金额",money(c.amount,c.currency),"strong"],["成本类型",tag(c.type,c.type==="直接成本"?"green":"orange")],["归属结果",c.target],
])}<div class="drawer-section"><h3>判断依据</h3><div class="inline-note">系统依据供应商侧单号关系提出建议，财务确认后形成当前成本类型。若一笔金额覆盖多个对象且未拆分，即使单号可追溯也应按间接成本处理。</div></div><div class="drawer-section"><h3>原始行快照</h3>${detailGrid([["原始表头",`追踪号 / ${c.raw} / 币别 / 备注`],["原始值",`${c.key} / ${c.amount} / ${c.currency} / -`]])}</div>`);}
...
function bucketDrawer(id){
  ...
  const treatmentDetail=treatment === "不分摊"
    ? `<div class="drawer-section"><h3>不分摊确认</h3>${detailGrid([["不分摊原因",p.noAllocationReason||"-"],["确认人",p.treatmentConfirmedBy||"-"],["确认时间",p.treatmentConfirmedAt||"-"],p.noAllocationNote&&["补充说明",escapeHtml(p.noAllocationNote),"","detail-full"]])}<div class="inline-note">该分摊集不生成订单分摊结果，金额仍计入成本中心总成本和公司总利润扣减。</div></div>`
    : `<div class="drawer-section"><h3>采用的分摊规则</h3>${rule?detailGrid([["规则编号",rule.id,"mono"],["规则层级",rule.supplier === "全部供应商" ? "基础分摊规则" : "供应商特调分摊规则"],["候选订单范围",rule.scope,"","detail-full"],["优先分摊因子",rule.factor],["兜底分摊因子",rule.fallback]]):`<div class="validation-item warn">${icon("triangle-alert")}尚未确认分摊规则，不能执行分摊。</div>`}</div><div class="drawer-section"><h3>计算边界</h3><div class="inline-note">分摊集只统一规则口径，不合并成本明细的业务范围。系统仍依据每条成本明细自己的金额与范围锚点寻找候选业务订单，再汇总形成分摊结果。</div></div>`;
  showDrawer(`间接成本分摊集 · ${p.id}`,`<div class="header-actions" style="justify-content:flex-start;margin-bottom:var(--space-3)">${treatmentAction}${allocationActions}</div>${detailGrid([
    ["所属成本账单",p.bill,"mono"],["分摊状态",status(p.status)],["处理方式",tag(treatment,treatment === "不分摊"?"gray":"blue")],["供应商",p.supplier],["成本板块",p.board],
    ["标准成本费项",p.fee],["币种",p.currency],["成本明细",`${p.detailCount} 条`],["分摊集金额",p.amount,"strong"],["当前分摊版本",p.version],
  ])}${treatmentDetail}`);
}
...
function ruleDrawer(id) {
  ...
  showDrawer(`分摊规则详情 · ${rule.id}`, `<div class="header-actions" style="justify-content:flex-start;margin-bottom:var(--space-3)"><button class="btn primary" data-action="edit-rule" data-id="${rule.id}">${icon("pencil")}编辑规则</button></div>${detailGrid([
    ["规则类型",tag(typeLabel, rule.supplier === "全部供应商" ? "blue" : "purple")],["规则状态",status(rule.status)],["成本板块",rule.board],["标准成本费项",rule.fee],
    ["适用供应商",rule.supplier],["生效期间",rule.effective || "长期有效"],["尾差处理",rule.rounding],
  ])}<div class="drawer-section"><h3>候选业务订单范围</h3><div class="inline-note">${escapeHtml(rule.scope)}</div></div><div class="drawer-section"><h3>分摊因子</h3>${detailGrid([["优先分摊因子",rule.factor],["兜底分摊因子",rule.fallback]])}</div>${rule.supplier !== "全部供应商" ? `<div class="validation-item ok">${icon("badge-check")}该规则命中时优先于同一标准成本费项的基础分摊规则。</div>` : ""}`);
}
...
function feeDrawer(id) {
  ...
  showDrawer(`成本费项详情 · ${fee.name}`, `<div class="header-actions drawer-primary-actions"><button class="btn primary" data-action="edit-fee" data-id="${fee.code}">${icon("pencil")}编辑费项</button><button class="btn ${fee.status === "启用" ? "danger" : ""}" data-action="toggle-fee" data-id="${fee.code}">${fee.status === "启用" ? icon("circle-pause") + "停用" : icon("circle-play") + "启用"}</button></div>
    ...
    <div class="drawer-section"><h3>费项定义</h3>${detailGrid([["成本费项编码",fee.code,"mono"],["成本板块",fee.board],["业务定义",escapeHtml(fee.definition),"","detail-full"],["备注",escapeHtml(fee.remark || "-"),"","detail-full"],["成本明细引用",`${fee.references || 0} 条`],["最近更新",escapeHtml(fee.updatedAt || "-")]])}</div>
