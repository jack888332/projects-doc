...
4. 接受同一订单出现在多张账单。
5. 附加费有 `bill_waybill_no` 时挂靠尾程单号（`LAST_PACKAGE`）并按具体尾程号落费，无则挂靠业务订单（`ORDER`），同时保留订单与尾程包裹的关联信息。
6. 订单头缺失时跳过并输出日志。
7. 仅存在附加费时允许直接创建应收账单。
8. 方案中补充源表索引创建 SQL。
9. 生产 8 条改单费需在新代码发布后重新生成账单验证；存量 `fee_detail` 不会自动修正，需重新生成或清理后重跑。
...
防重维度 = bill_source_collect_mark(MARKED)
```
...
3. 已确认：源 SQL 不再使用 `bms_billed_flag` / `bms_bill_no` 过滤，防重完全依赖 `bill_source_collect_mark(MARKED)`；分页使用 `a.id` 游标分页，避免放开过滤后 OFFSET 深翻页。
...
5. `fee_detail.attached_object` 有 `bill_waybill_no` 时写 `LAST_PACKAGE`，无则写 `ORDER`；尾程号取具体 `bill_waybill_no`，不共用订单聚合尾程号。必须保留 `business_order_no`、`source_order_id`、`last_mile_waybill_no`、`first_mile_waybill_no` 和 `bill_order_waybill_snapshot`，保证通过订单仍能关联尾程包裹信息。
6. 订单头缺失时跳过并打印日志。
7. 仅存在附加费、没有本期主订单时，允许直接创建应收账单。
8. 生产 8 条改单费需在新代码发布后重新生成账单验证；存量 `fee_detail` 不会自动修正，需重新生成或清理后重跑。
...
flowchart TD
    ...
    Append["创建或追加应收账单\n允许仅附加费建账"]
    Fee["写 fee_detail\n有 bill_waybill_no 挂 LAST_PACKAGE\n无则挂 ORDER，尾程号按具体单号"]
...
10. 应收附加费 `attached_object` 有 `bill_waybill_no` 时调整为 `LAST_PACKAGE`，无则保持 `ORDER`；`last_mile_waybill_no` 取具体 `bill_waybill_no`，同时保留订单号、来源订单 ID、首末程运单号及尾程快照字段。
11. 沿用返款直扣/需聚合费项过滤：独立链路继续执行 `isRefundDirectDeductFee()` / `isRefundRequiredAggregateFee()`，避免从主订单分组拆出后应收口径发生变化。
12. 调整 `buildAdditionalFeesQuerySql()`：移除 `bms_billed_flag` / `bms_bill_no` 源表过滤；独立扫描按 `a.id` 游标分页，先过滤 MARKED 轨迹再补查尾程包裹，降低已打标历史数据带来的查询与 CXMS 开销。
...
- 已确认：防重只依赖 `bill_source_collect_mark(MARKED)`，源 SQL 不再用 `bms_billed_flag` / `bms_bill_no` 过滤。
- 已确认：允许同一订单出现在多张账单。
- 已确认：`fee_detail.attached_object` 有 `bill_waybill_no` 时写 `LAST_PACKAGE`，无则写 `ORDER`；尾程号取具体 `bill_waybill_no`。
...
5. 已确认：`fee_detail.attached_object` 有 `bill_waybill_no` 时写 `LAST_PACKAGE`，无则写 `ORDER`；尾程号取具体 `bill_waybill_no`，必须保留订单与尾程包裹的关联字段和快照。
6. 已确认：订单头缺失时跳过并打印日志。
7. 已确认：完全拆成独立链路，旧 `ADDITIONAL_FEE` 随订单分组写入的方式不再作为正式采集路径；说明见下方。
8. 已确认：仅存在附加费、没有本期主订单时，允许直接创建应收账单。
9. 已确认：在方案中提供 OFP 索引创建 SQL。
10. 已确认：生产 8 条改单费需在新代码发布后重新生成账单验证；存量 `fee_detail` 不会自动修正，需重新生成或清理后重跑。
