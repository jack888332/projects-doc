...
参与角色为管理员、航线部；操作授权见[空运操作权限](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第067篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-air-actions)。

<a id="doc-6A0360EC35-section-2ab623a2d33a"></a>
#### 1.2.5 订舱前置与完成流程

订单创建、子订单合成和补录流程见[空运订单管理流程](PRD.高捷物流系统.第021篇.空运-订单管理.5029269CA8.md#doc-5029269CA8-section-0de90fb8a71c)。订舱模块从待订舱订单进入航线处理开始：航线运营确认航班后，订舱服务变为“服务中”；航线操作补充航程并完成订舱后，订舱服务变为“服务已完成”，订舱信息同步回订单。

<a id="doc-6A0360EC35-section-97c909961166"></a>
#### 1.2.6 订舱订单修改边界

订单字段、服务状态和提单号的修改边界见[空运订单异常与修改规则](PRD.高捷物流系统.第021篇.空运-订单管理.5029269CA8.md#doc-5029269CA8-section-9983c813182e)。订舱信息发生变更时同步回订单，并按[消息推送规则](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第069篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-ef4930471a81)发送结果通知。
...
- **区域字段或业务元素**：订单号、组织名称、业务类型、建单客服、订单来源；客户、业务员；中文品名、特殊货物、预报毛件体、预计送货日期；始发港、目的港、航司产品、订舱要求；运费卖价、后段卡车卖价、Minimum、分泡。
- **区域级通用规则**：只读展示当前订单信息；合成主订单的取值按[订舱聚合规则](#doc-6A0360EC35-section-af5a3d174393)处理。
...
| 提单属性 | 自营/非自营；必填。 |
| 提单号、同行名称 | 自营展示自动绑定的提单号及修改入口；非自营提供同行名称、提单号录入位置。提单号绑定和修改沿用[订单异常与修改规则](PRD.高捷物流系统.第021篇.空运-订单管理.5029269CA8.md#doc-5029269CA8-section-9983c813182e)，同行名称的来源与校验尚未明确。 |
| 预计盈利规则 | 必填；按体积（m³）、毛重（kg）和密度分别勾选约束项并填写比较关系、比较值；可选大于等于/小于等于/大于/小于，默认大于等于。密度提供上下界录入位置；多项组合关系及密度计算口径仍待确认。 |
| 二程航段 | 取自航司基础数据；选填。 |
| 二程目的地 | 取自空港基础数据；必填。 |
| 三程航段 | 取自航司基础数据；必填；初始不设值。 |
| 三程目的地 | 提供目的地选择控件；候选来源及与航线类型、三程航段关联的必填条件尚未明确。 |
| 允许亏损 | 默认不勾选；选择后的提交、审核及禁止条件见[订舱处理与盈利校验规则](#doc-6A0360EC35-section-7e8797c5c9ba)。 |
...
1. 航程信息和航程补充的角色授权见[订舱字段与操作权限](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第067篇.角色与访问权限.5E31456F30.md#doc-5E31456F30-air-actions)。空运航线运营的操作按钮为“取消”“确认航班”；空运航线操作、航晟客服的操作按钮为“取消”“订舱完成”。

2. 航线运营填写后点击“确认航班”，提交订舱信息并同步回客服订单，订舱服务状态变为“服务中”。航线操作填写后点击“订舱完成”，提交订舱信息并同步回客服订单，订舱服务状态变为“服务已完成”。对应通知按[消息推送规则](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第069篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-ef4930471a81)发送。
...
```plantuml
@startuml goldjet-booking-profit-exception
skinparam activityDiamondBackgroundColor #FFF4CC
|系统|
start
:取得本次实际毛件体;
if (实际毛件体在允许区间内？) then (是)
  :本次不触发预计盈利异常;
  stop
else (否)
  :主订单列表前置并标红\n发送异常通知;
...
实际毛件体取入仓结果；未选择入仓服务时取客服再次向客户取得并手工录入的数据。图不替代允许亏损的航线总监审核，调整规则后的重新校验、通知解除及异常标识清除时点尚未定义。

**订舱服务状态图 · 确认、完成与航班修改**

图中对象为同一张订舱服务单。正常确认、完成和航班修改按上述规则处理；无法取得状态时的客服手工补录边界见[主订单创建与服务状态规则](PRD.高捷物流系统.第021篇.空运-订单管理.5029269CA8.md#doc-5029269CA8-section-a5d48cac9f5b)。
...
服务取消沿用[服务单状态规则](PRD.高捷物流系统.第021篇.空运-订单管理.5029269CA8.md#doc-5029269CA8-section-a5d48cac9f5b)。本图不设最终结束符，因为完成后仍存在符合条件的航班修改。
...
- **区域字段或业务元素**：查询栏另含主订单类型、订单状态；结果区含订单号、提单号、主订单类型、分单号、建单日期、头程航班、始发港、中转港、目的港、预计/入仓/提单毛件体、货物尺寸、订舱要求、特殊货物、货物状态、订单状态、报关状态、业务人员、建单客服。
- **区域级通用规则**：结果区只读展示对应订单信息，点击订单号进入订舱详情；毛件体按预计、入仓、提单分别展示，不能混为一个数值。

| 字段或控件特例 | 界面特例或可见反馈 |
| --- | --- |
| 主订单类型 | 可选全部、主订单、合成主订单，默认全部。 |
| 订单状态 | 可勾选全部、待订舱、待补录、待出提单、已出提单、已交单、待审核、已作废。 |
| 分单号 | 在所属主订单行内展示对应分单。 |
...
2. 订单状态为“待补录”或“待出提单”时，航线部可修改航班信息；其他状态下只能查看。修改后按[消息推送规则](../PRD.高捷物流系统.第08册.运营支撑/PRD.高捷物流系统.第069篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-ef4930471a81)发送通知。
