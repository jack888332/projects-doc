...
<template>
  <div class="setting-row matrix-row">
    ...
    <div class="matrix-wrap">
      <DataTableFrame :total="scheme.feeRules.length" :page-size="10">
        <el-table :data="scheme.feeRules" border empty-text="未配置，默认使用费项默认结算币种">
          ...
          <TableActionColumn compact><template #default="{$index}"><HoverActionMenu><el-dropdown-item class="danger-action" :icon="Delete" @click="scheme.feeRules.splice($index, 1)">删除</el-dropdown-item></HoverActionMenu></template></TableActionColumn>
        </el-table>
      </DataTableFrame>
