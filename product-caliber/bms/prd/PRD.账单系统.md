...
| 特调汇率区 | 按店铺、客户组或客户维护某些货币对的特调规则和特调后默认汇率 | `适用层级`、`适用对象`、`货币对`、`汇兑方向`、`调整方式`、`调整方向`、`调整值`、`基准汇率`、`特调后默认汇率`、`状态` | 按适用层级和适用对象筛选、查看明细、添加、编辑、停用特调汇率；特调停用后，对应范围回到下一优先级汇率来源。 |
...
6. 页面所有操作均由财务执行。页面及编辑弹窗不展示操作人和操作时间；基准汇率和特调汇率的新增、导入、修改、移除、确认和停用统一进入内部审计，规则见<a href="#section-8-7">8.7 内部审计</a>。
...
| 内部审计 | 新增、编辑、停用特调汇率统一进入内部审计记录；页面不展示操作人和操作时间。 |
...
2. 每次变更必须记录适用对象、货币对、变更前后汇率设置、操作原因、操作人、操作时间和执行结果。
...
1. 客户级账单配置、基准汇率表、特调汇率、账单内锁定汇率、客户侧费项索引、成本费项索引、间接成本分摊规则、导入设置快照中的费项映射、场景取值规则、数据源规则和供应商财务档案变更。
