...
<template>
  <el-dialog :model-value="props.modelValue" class="module-dialog module-dialog-large" width="980px" align-center append-to-body destroy-on-close @update:model-value="emit('update:modelValue', $event)">
    <template #header><div class="drawer-title"><span>{{ props.reference?.customerCode ? '客户引用的准确版本' : '配置准确版本' }}</span><small>{{ props.config?.no || props.reference?.configNo }} / {{ props.version?.version || props.reference?.configVersion }}</small></div></template>
    ...
    <el-descriptions v-if="props.version" class="snapshot-descriptions" :column="2" border>
      ...
      <el-descriptions-item label="配置">{{ props.config?.name || props.reference?.configName }}</el-descriptions-item>
      <el-descriptions-item label="准确版本">{{ props.version.version || props.reference?.configVersion }}</el-descriptions-item>
    ...
    <DataTableFrame class="rule-table" :total="props.rules.length" :pagination="false" :column-sort="false">
