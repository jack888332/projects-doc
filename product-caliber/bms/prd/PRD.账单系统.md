...
| 返款配置编辑 | 配置编号与版本、启用状态、返款模式、账期类型、半周起始日、账单发出时间、必要归集金额、直接扣减费项、货款原始币种、货款结算币种、客户收款账户、负数金额处理方式、条款生效周期 | 新增或移除币种账户规则、保存配置 | 账期仅支持周和半周；半周必须选择两个起始日且两个账期均不少于 3 天；币种账户规则必须保留一条末行兜底规则，兜底货款原始币种为只读的“全部”或“其他”；非兜底规则指定货款原始币种前，货款结算币种为空，指定后默认取货款原始币种；每条规则必须配置明确的货款结算币种和客户收款账户，不支持“随原始币种” |
...
订单费用报表可以导入`返款汇率`和`实返货款`，但没有能够明确指定`实返货款币种`的字段。系统无法仅凭这两个导入值可靠判断实际返款币种，因此返款账单不得使用导入的`返款汇率`或`实返货款`确定结算币种和应返金额。

返款账单采用以下口径：

1. `货款原始币种`是配置的匹配条件，用于匹配`应付返款（即代收货款）`的原始币种。该币种通常为目的国币种。
2. `货款结算币种`是客户最终收款的币种，也是`实付返款（准）`和`实付返款（正）`的计算币种。
3. `货款换算汇率`是`货款原始币种 → 货款结算币种`的账单锁定汇率，按<a href="#section-15-3-1">15.3.1 汇率层级</a>取得；同币种换算时固定为`1`。
4. 订单费用报表导入的`返款汇率`和`实返货款`继续保留为来源事实，仅用于追溯和人工核对，不参与返款账单金额计算，也不得覆盖账单计算结果。
...
| 3. 锁定换算汇率 | 按真实的货款原始币种和货款结算币种取得`货款换算汇率`，并随账单保存快照。 | 按<a href="#section-15-3-1">15.3.1 汇率层级</a>处理；不得使用导入返款汇率替代。 |
| 4. 计算扣减前返款 | `实付返款（准） = 应付返款（原始币种） × 货款换算汇率`。 | 缺少应付返款本金时，受影响业务订单不得进入本次返款账单。 |
| 5. 校验收款账户 | 客户收款账户币种必须等于`货款结算币种`。 | 未配置同币种客户收款账户时，配置不得保存，受影响账单不得生成。 |

配置保存时，系统必须阻止同一配置版本内相同`货款原始币种`存在多个不同的`货款结算币种`规则；否则同一业务订单会得到多个结算结果。`其他`只负责兜底匹配未单独配置的原始币种，计算时仍须使用业务订单真实的原始币种取得汇率。例如，配置为`其他 → USD`时，TWD 业务订单应使用`TWD → USD`的账单锁定汇率，不能把`其他`当作货币，也不能使用导入的`TWD → CNY`返款汇率。

同一批业务订单中，导入`返款汇率`或`实返货款`是否有值，不影响货款币种规则的匹配和返款账单计算。来源值与账单计算值不一致时，不得因此阻止账单生成、替换或审核；系统应在账单明细中分别展示来源值和账单计算值，供财务人工核对。
...
客户级汇率配置解决的是“账单生成前，不同客户、不同货币对默认采用什么汇率”的问题。它位于账单生成之前，是账单特调汇率快照的前置配置；账单生成之后，命中的汇率会进入账单侧快照，由<a href="#section-15">15. 金额汇兑</a>继续定义换算、汇总和核销规则。
...
1. 每个作为 BMS 费项或账单计算输入的导入金额都必须在导入时确定原始币种，不得只保存无币种金额。`实回货款`、`实返货款`和汇率属于回款 / 返款来源事实，不按普通费项金额处理。
...
4. `返款汇率`和`回款汇率`不是金额字段，均表示`目的国币种 -> 店铺币种`的汇率；例如目的国币种为`TWD`、店铺币种为`CNY`时，两者的货币对均为`TWD -> CNY`。具体字段口径见<a href="#section-19-2-3-4">19.2.3.4 OIS 有哪些客户侧费用字段？</a>。
5. 订单费用报表中的`实返货款`没有对应的币种字段，因此不能仅凭`返款汇率`推断其实际币种；该字段只作为来源事实保留，不作为返款账单的结算币种、货款换算汇率或金额计算输入。
...
返款账单模块用于把 COD 业务订单的应付返款及手续费，与其尾程包裹的回款事实整理为客户可核对、财务可审核、可执行返款和可追溯结清的账单结果。模块的核心结果不是“把到付总额原样返给客户”，而是依次确定到付总额、到付附加费、应付返款本金、汇兑后的扣减前返款金额、返款账单指定扣减费和最终实付返款。

本章只定义返款账单的金额、归集、状态、审核、核销、返款和输出规则。尾程包裹的回款状态、返款状态和汇兑损益展示统一由<a href="#section-12">12. 回款管理</a>负责；OIS 来源表、字段结构和导入事实统一见<a href="#section-19-2-3">19.2.3 客户侧费项的 OIS 数据源</a>；币种和汇率快照统一见<a href="#section-15">15. 金额汇兑</a>。
...
因此，返款账单必须先在原始到付币种中拆出代收货款本金，再按<a href="#section-7-5-1">7.5.1 返款账单怎样确定结算币种和金额？</a>匹配货款原始币种与结算币种，使用账单锁定汇率计算扣减前返款金额，最后扣除返款账单明确指定的扣减费项。订单费用报表导入的`返款汇率`和`实返货款`仅作为来源事实保留，不参与返款账单金额计算。签收返款和回款返款只是“何时具备返款条件、何时锁定回款事实”的两种配置方式，不能改变上述金额顺序。
...
| `应付返款（即代收货款）` | 从到付总额中扣除到付附加费后的代收货款本金 | 直接挂靠业务订单，以货款原始币种记录，是计算`实付返款（准）`的原始金额。 |
| `代收货款手续费` | 因代收货款业务产生、可按返款配置从返款中扣减的费用 | 直接挂靠业务订单；命中直接扣减配置时进入返款账单，同一业务订单只计入一次。 |
| `实收回款` | 订单费项报表导入的包裹侧`实回货款` | 直接挂靠尾程包裹，表示尾程派送商实际返给财务的金额；当前版本直接读取导入值，不由系统按公式生成。 |
| `回款汇率` | 订单费项报表导入的回款汇率 | 表示导入记录中的`目的国币种 → 店铺币种`汇率；当前版本直接读取导入值。 |
| `货款换算汇率` | 返款账单锁定的`货款原始币种 → 货款结算币种`汇率 | 按<a href="#section-15-3-1">15.3.1 汇率层级</a>取得，用于计算扣减前返款金额。 |
| `实付返款（准）` | 按货款币种配置和账单锁定汇率计算的扣减前返款金额 | `实付返款（准） = 应付返款（原始币种） × 货款换算汇率`；以货款结算币种记录。 |
| `来源实返货款` | 订单费用报表导入的包裹侧`实返货款` | 因来源报表没有提供其币种字段，仅用于来源追溯和人工核对，不参与返款账单计算。 |
| `来源返款汇率` | 订单费用报表导入的返款汇率 | 来源口径为`目的国币种 → 店铺币种`；不等同于货款换算汇率，不参与返款账单计算。 |
| `返款账单指定扣减费项` | 在返款账单中明确指定、从扣减前返款金额中扣除的客户应承担费用 | 例如`税金（账期支付）`；只有被指定后才参与本次返款扣减。 |
| `实付返款（正）` | 扣除指定扣减费项后的最终返款金额 | 客户最终应收到的金额；实际返款完成后才形成已返金额。 |
...
| `汇兑损益` | 到付币种与回款 / 返款币种不一致时的差额 | 在回款管理中计算与展示，具体口径见<a href="#section-15">15. 金额汇兑</a>。 |

关键规则：

1. 返款账单与应收账单可独立按各自账期运行，不要求同周期对齐。
2. 返款账单按 COD 业务订单形成返款金额；尾程包裹提供签收和实收回款事实。异常签收包裹不形成自身回款事实，并由<a href="#section-12">12. 回款管理</a>保留状态追踪。
3. 到付附加费在原始到付币种中先扣除；返款账单指定扣减费项在`实付返款（准）`所在币种中后扣除。两类费用不得互相替代，也不得在应收账单和返款账单中重复核销。
...
1. 订单侧的`应付返款（即代收货款）`和`代收货款手续费`直接挂靠业务订单，是返款账单按订单归集的金额来源；到付金额用于核对该订单的 COD 业务背景。
2. `实回货款`和`回款汇率`来自订单费用报表导入，继续作为回款链路的金额和汇率事实。
3. `实返货款`和`返款汇率`也来自订单费用报表导入，但由于来源报表没有提供`实返货款币种`字段，返款账单只保留这两个来源值，不把它们作为结算币种、货款换算汇率或实付返款金额的计算输入。
4. 返款账单按应付返款本金、货款原始币种、货款结算币种和账单锁定汇率计算`实付返款（准）`；计算结果不得回写或覆盖来源表中的`实返货款`和`返款汇率`。
5. 字段名称相近不代表金额含义相同。系统不得仅凭中文字段名，把到付总额、代收货款本金、手续费、来源实返货款和账单计算的实付返款互相替代。
6. 返款账单明细必须分别保留业务订单和尾程包裹的挂靠关系，并明确区分订单级金额、包裹级导入事实和本账单计算结果。
...
2. `回款返款`模式下，只要该业务订单任一满足返款条件的尾程包裹已录入非空且非零的`实收回款`，该业务订单即满足“已回款”归集条件；不要求同一订单下所有尾程包裹都录入实收回款。
...
3. **再计算应返金额及币种**：按<a href="#section-7-5-1">7.5.1 返款账单怎样确定结算币种和金额？</a>匹配货款原始币种和货款结算币种，取得并锁定`货款换算汇率`，计算`实付返款（准） = 应付返款（原始币种） × 货款换算汇率`。订单费用报表导入的`返款汇率`和`实返货款`不参与该计算。
4. **最后处理指定扣减**：从返款账单明确指定的扣减费项中汇总扣减金额，得到客户最终应收到的`实付返款（正）`。例如，`税金（账期支付）`只有在被返款账单指定为扣减费项后，才参与本次返款扣减。

两类费用的扣减时点不同：到付附加费在原始到付币种中先扣除，决定应返货款本金；返款账单指定扣减费项在`实付返款（准）`所在币种中后扣除，决定最终返款金额。到付附加费不会因为被从 COD 总额中扣除，就自动变成返款账单的指定扣减费项；返款账单也不得把同一笔费用在应收账单和返款账单中重复核销。

下表用两个相同业务金额的案例，说明货款原始币种与结算币种相同或不同时，返款账单怎样计算实付返款。两个案例的到付金额均为`70 TWD`，到付附加费均为`10 TWD`，因此应付返款均为`60 TWD`。绿色区域中的`实收回款`和`回款汇率`仍为订单费用报表导入的回款事实；粉色区域中的`货款换算汇率`和`实付返款（准）`分别为返款账单锁定的汇率和计算结果。导入的`返款汇率`和`实返货款`不在计算链路中展示，仅保留在来源追溯信息中。
  ...
  <table class="refund-amount-table">
    <tbody>
      ...
      <tr>
        ...
        <td class="value">9 CNY</td>
        <td class="value">5 CNY</td>
        <td class="value">4 CNY</td>
      </tr>
    </tbody>
  </table>
</div>

示例一中，货款币种配置为`TWD → TWD`，同币种货款换算汇率固定为`1`，因此`实付返款（准）`为`60 TWD`。示例二中，货款币种配置为`TWD → CNY`，账单锁定的货款换算汇率为`0.15`，因此`实付返款（准）`为`9 CNY`。两个示例的结算币种均由货款币种配置决定，与订单费用报表是否导入返款汇率或实返货款无关。

配置为`TWD → USD`时，系统必须取得`TWD → USD`的账单锁定汇率；配置为`其他 → USD`时，也必须按包裹真实的原始币种取得对应的美元汇率。任何情况下都不得用来源口径为`TWD → CNY`的导入返款汇率代替货款换算汇率。

计算关系如下：

1. `应付返款（即代收货款） = 应收回款（即到付金额） - 到付附加费总额`，两个示例均为 `70 TWD - 10 TWD = 60 TWD`。
2. `实付返款（准） = 应付返款（原始币种） × 货款换算汇率`；示例一为`60 TWD × 1 = 60 TWD`，示例二为`60 TWD × 0.15 = 9 CNY`。
3. `实付返款（正） = 实付返款（准） - 返款账单指定扣减费项`；示例一为 `60 TWD - 20 TWD = 40 TWD`，示例二为 `9 CNY - 5 CNY = 4 CNY`。
4. 当前版本的`实回货款`由报表导入：示例一导入`70 TWD`；示例二导入`14 CNY`。示例二的导入值与核对关系 `70 TWD × 0.2 = 14 CNY`一致；无回款汇率时，导入的实回货款继续使用目的国币种。实收回款不等于客户最终收到的返款金额。
5. 表内所有相减和相加必须在同一币种下进行。指定扣减费项不是货款结算币种时，须先按其账单锁定汇率换算为货款结算币种，再参与最终返款计算。
6. 客户最终收到的金额以扣减后的`实付返款（正）`为准；未完成实际返款前，该金额属于待返金额，完成返款后才形成已返金额，不能仅凭计算结果把账单标记为已结清。
7. `实付返款（正）`小于零时，不得执行负数返款，也不得继续按普通待返金额核销；系统应按<a href="#section-7-5">7.5 返款账单配置</a>选择的负数金额处理方式，顺延至下期返款账单或反向计入本期应收账单。

<div id="section-11-3"></div>

### 11.3 返款账单怎样形成并完成结清？

返款账单从包裹具备返款条件开始，到客户返款和尾程派送商回款全部完成后结束。本章只描述账单负责的环节；包裹派送、回款状态、返款状态和未回款跟进统一见<a href="#section-12">12. 回款管理</a>。
...
| 环节 | 处理规则 | 业务结果 |
| :--- | :--- | :--- |
| 确认包裹范围 | 只处理正常签收的 COD 包裹；异常签收包裹不进入返款账单。`签收返款`在正常签收后具备归集条件，`回款返款`还必须满足已回款条件。 | 得到本期可进入返款账单的包裹范围。 |
| 读取金额及来源事实 | 读取到付总额、到付附加费和指定扣减费项作为账单计算输入；同时保留回款、来源实返货款、回款汇率和来源返款汇率作为来源事实，具体规则见<a href="#section-11-2-1">11.2.1 返款金额从哪里取？</a>。 | 得到每个包裹可计算且可追溯的金额链路。 |
| 计算返款金额 | 按<a href="#section-11-2-3">11.2.3 到付金额、代收货款和实际返款如何区分？</a>依次计算代收货款本金、扣减前返款金额和最终实付返款。 | 形成包裹级返款结果和账单汇总金额。 |
...
系统支持`签收返款`和`回款返款`。两种模式使用相同的金额链路、扣减规则和货款结算币种，只改变包裹何时具备返款条件，以及回款和返款的先后顺序。

| 返款模式 | 包裹进入返款范围的条件 | 资金顺序 | 适用口径 |
| :--- | :--- | :--- | :--- |
| `签收返款` | COD 包裹正常签收 | 财务先向客户返款，尾程派送商再向财务回款 | 我方需要先行垫付返款。 |
| `回款返款` | COD 包裹正常签收且已确认回款 | 尾程派送商先向财务回款，财务再向客户返款 | 我方收到到付金额后再返还代收货款；页面默认优先表达该模式。 |

<div id="section-11-5-1"></div>

#### 11.5.1 签收返款（先返后回）

正常签收后，系统即可按本期账单配置归集包裹并计算返款；财务向客户完成返款后，仍须继续跟踪尾程派送商向财务回款，回款未完成前账单保持`待结清`。

<div id="section-11-5-2"></div>

#### 11.5.2 回款返款（先回后返）

正常签收后，系统继续等待尾程派送商向财务回款；财务确认收到回款后，包裹才进入返款账单归集范围。部分回款、未回款及后续跟进由<a href="#section-12">12. 回款管理</a>记录，财务向客户返还的金额仍按<a href="#section-11-2-3">11.2.3 到付金额、代收货款和实际返款如何区分？</a>计算。
...
| 货款原始币种、货款结算币种和客户收款账户 | 决定返款明细的币种匹配、货款换算汇率、分桶汇总和实际返款账户；不读取导入返款汇率和实返货款作为账单计算输入。 |
...
返款账单详情页用于核对一张账单“金额从哪里来、怎样计算、已经处理多少、还剩多少”。包裹级回款和返款状态跳转至<a href="#section-12">12. 回款管理</a>查看，不在账单详情中重新计算。
...
| 返款金额链路 | 到付附加费总额、应付返款（即代收货款）、货款换算汇率、实付返款（准）、返款账单指定扣减费项、实付返款（正） | 字段顺序必须与<a href="#section-11-2-3">11.2.3 到付金额、代收货款和实际返款如何区分？</a>一致，并展示每一步币种；来源实返货款和来源返款汇率在来源追溯区域单独展示，不进入该计算链路。 |
| 货款结算币种汇总 | 货款结算币种、实付返款（准）、指定扣减金额、实付返款（正）、已返金额、待返金额、核销状态；核销 | 各货款结算币种分别汇总和核销；所有币种待返金额为零只是结清条件之一，还须完成应回款和扣减处理。 |
| 财务本位币汇总 | 财务本位币、实付返款（正）、已返金额、待返金额 | 仅用于财务统一查看，不替代货款结算币种的核销结果。 |
| 账单汇率 | 左侧币种、右侧币种、汇兑方向、汇率来源、锁定汇率、汇率状态 | 读取账单锁定快照，不随后续配置或市场汇率变化。 |
| 包裹返款明细 | 尾程运单号、主订单号、签收时间、来源金额与币种、到付附加费、应付返款、货款换算汇率、实付返款（准）、指定扣减金额、实付返款（正）、已返金额、待返金额、核销状态 | 每笔明细必须保留来源字段和挂靠包裹；来源实返货款、来源返款汇率与账单计算结果分列展示，不得混为同一列。 |
| 指定扣减费项 | 费用编号、费项名称、业务订单号、尾程运单号、原始币种及金额、换算汇率、货款结算币种及扣减金额、处理状态 | 只展示本账单实际占用的指定扣减费项；同一费项不得再进入应收账单核销。 |
...
3. 指定扣减费项不是货款结算币种时，先按账单锁定汇率换算，再计入指定扣减金额。
4. 财务本位币汇总、货款结算币种汇总和包裹明细必须能相互追溯；展示精度差异按<a href="#section-15">15. 金额汇兑</a>处理。
5. `实付返款（正）`小于零时，详情页应展示负数金额及其处理去向，不生成负数待返金额或负数返款核销记录。
...
3. 汇兑损益和应回未回属于回款管理结果，统一见<a href="#section-12">12. 回款管理</a>，不作为返款账单模块单独定义的报表。
...
回款管理是包裹视角列表，用于查看所有 COD 包裹的回款状态、返款状态、汇兑损益和对账状态。页面中的回款和返款均采用财务视角：尾程派送商将到付金额返给财务称为回款，财务将代收货款返还给客户称为返款，完整定义见<a href="#section-11-2">11.2 核心概念</a>。它只回答一个包裹“是否已回款、是否已返款、是否需要跟踪”，不承载返款账单本身的结算流程；返款账单只作为回款状态判定和追溯依据被关联引用。列表主粒度为尾程包裹，回款明细来源见<a href="#section-19-2-3-2">19.2.3.2 各来源表怎样保存并关联业务对象？</a>。回款管理负责回款 / 返款金额换算、汇率快照和汇兑损益计算，具体口径见<a href="#section-15">15. 金额汇兑</a>。回款管理不提供单独的回款导入入口。
...
| 返款信息列 | 返款时间、返款模式、关联返款账单、应付返款金额、来源实返货款金额、实付返款（准）、实付返款（正） | 来源实返货款金额仅用于追溯；实付返款（准）和实付返款（正）读取关联返款账单的计算结果，具体口径见<a href="#section-15">15. 金额汇兑</a>。 |
| 汇兑损益列 | 汇兑损益金额、汇兑损益币种 | 始终显示；两侧金额币种一致且数据完整时，显示`回款金额 - 实付返款（准）`的结果，否则显示`--`。具体口径见<a href="#section-15">15. 金额汇兑</a>。 |
...
4. 汇兑损益金额始终展示，并保持与回款 / 返款锁定快照一致，不随市场汇率刷新；正常签收且两侧金额币种一致时显示计算结果，异常签收、金额不完整或币种不一致时显示`--`。
...
正常签收的 COD 包裹同时存在回款和返款两条金额链路：回款是尾程派送商将完整到付金额返给财务，返款是财务将代收货款返还给客户；`到付金额 = 代收货款 + 到付附加费`。两条链路的来源和计算方式不同：回款链路读取订单费用报表导入的`实回货款`和`回款汇率`；返款链路按货款原始币种、货款结算币种和账单锁定汇率计算`实付返款`。完整业务定义见<a href="#section-11-2">11.2 核心概念</a>。

1. 只有正常签收的 COD 包裹才进入返款金额计算；异常签收的 COD 包裹不产生回款金额或返款金额。
2. 到付附加费和应付返款本金始终在原始到付币种中计算；`应付返款 = 到付金额 - 到付附加费总额`。
3. 返款账单先按`货款原始币种`匹配规则，再以`货款结算币种`作为实际返款币种；`实付返款（准） = 应付返款（原始币种） × 货款换算汇率`。货款换算汇率按<a href="#section-15-3-1">15.3.1 汇率层级</a>取得并随账单锁定。
4. 客户收款账户币种必须与货款结算币种一致；未配置同币种账户时，配置不得保存，受影响账单不得生成或执行返款。
5. `回款汇率`和订单费用报表导入的`返款汇率`均为来源事实，货币对固定为`目的国币种 → 店铺币种`；两者数值可以不同。导入返款汇率不等同于货款换算汇率，后者由货款原始币种和货款结算币种确定，可以是任意有效货币对。
6. 无回款汇率时，导入的实回货款继续按目的国币种展示。导入返款汇率为空或为`--`，只表示来源未提供该汇率，不影响返款账单按货款换算汇率计算，也不等同于账单换算汇率为`1`。
7. 当前版本的`实返货款`和`返款汇率`均来自订单费用报表导入，但该报表没有提供实返货款币种字段。系统必须保留导入值，不得回写或覆盖；返款账单不得用它们决定货款结算币种、货款换算汇率、实付返款（准）或实付返款（正）。来源值与账单计算值不一致时，不得因此阻止账单生成、替换或审核。
8. 订单费用报表中的回款字段可用于核对`回款金额 = 到付金额 × 回款汇率`；来源返款字段只用于追溯和人工核对，不因金额关系式成立而变为返款账单计算输入。金额链的识别和扣除顺序见<a href="#section-11-2-3">11.2.3 到付金额、代收货款和实际返款如何区分？</a>。
9. `实付返款（正） = 实付返款（准） - 返款账单指定扣减费项`。指定扣减费项不属于汇兑损益，必须先换算到货款结算币种后再扣减。
10. 回款金额和实付返款（准）必须分别记录实际结算币种；只有币种一致的金额才可直接比较、相减或核销。若币种不一致，汇兑损益展示为`--`，不得用任意一侧汇率补算。
11. 返款账单中的扣减前返款、实付返款、已返金额与待返金额均以本规则计算结果为准；来源实返货款不参与返款核销。

<div id="section-15-7-2"></div>

#### 15.7.2 返款账单换算示例

1. 在没有到付附加费和返款账单指定扣减费项的情况下，若到付金额为`100 TWD`、店铺币种为`CNY`，来源导入并锁定的回款汇率为`0.23000`，返款账单配置为`TWD → CNY`且锁定货款换算汇率为`0.21500`，则`回款金额 = 100 TWD × 0.23000 = 23.00 CNY`，`实付返款（准） = 100 TWD × 0.21500 = 21.50 CNY`。导入的返款汇率即使为空，也不影响上述返款账单计算。
2. 若返款账单配置为`TWD → TWD`，则同币种货款换算汇率固定为`1`，`实付返款（准） = 到付金额 - 到付附加费总额`；只有在没有到付附加费时，扣减前返款才等于完整到付金额。
...
1. 回款管理中的回款金额、`实付返款（准）`和未回款金额，均引用<a href="#section-15-7-1">15.7.1 返款账单换算总则</a>的换算结果。
2. 回款管理仅在回款金额与`实付返款（准）`的实际结算币种一致时，比较两者并计算汇兑损益；返款账单指定扣减费项不参与汇兑损益计算。
3. 当两者币种一致时，汇兑损益金额 = 回款金额 - `实付返款（准）`；当两者币种不一致时，汇兑损益展示为`--`，不得直接相减或用任意一侧汇率进行补算。
4. 回款管理中的汇兑损益金额始终显示，只做结果展示，不重新计算原始到付金额；不满足计算条件时显示`--`。
5. 回款管理的包裹级状态与返款账单状态独立计算，不互相替代。

示例：

1. 若到付金额为`100 TWD`，来源导入并锁定的回款汇率为`0.23000`，返款账单配置为`TWD → CNY`且锁定货款换算汇率为`0.21500`，则`汇兑损益金额 = 23.00 CNY - 21.50 CNY = 1.50 CNY`。
2. 若回款金额和`实付返款（准）`的实际结算币种均与到付币种一致，则 `汇兑损益金额 = 0`。
...
<table class="fee-source-table">
  ...
  <tbody>
    ...
    <tr class="source-package-fee"><td><code>ofp_ofdb1.sale_order_package_fee.collection</code></td><td>实返货款</td><td>尾程包裹</td><td>非费项</td><td><code>否</code></td><td>订单费项报表导入</td><td>来源报表没有提供实返货款币种字段；不作为返款账单币种或金额计算依据</td></tr>
    <tr class="source-package-fee"><td><code>ofp_ofdb1.sale_order_package_fee.recovery_money</code></td><td>实回货款</td><td>尾程包裹</td><td>非费项</td><td><code>否</code></td><td>订单费项报表导入</td><td>原始口径为目的国币种；有回款汇率时，导入结果为店铺币种</td></tr>
...
9. 订单费用报表导入的普通费项金额都必须确定原始币种；其中`仓租费`和`航空费`固定为人民币，其它普通费项金额采用目的国币种。`实返货款`属于来源事实，来源报表没有提供其币种字段，不纳入普通费项金额的原始币种规则。`返款汇率`和`回款汇率`均表示`目的国币种 -> 店铺币种`，不是金额币种；两者的货币对必须一致，但可因资金时点不同而使用不同汇率数值。
...
返款相关金额同时存在于订单侧和订单费用报表导入形成的包裹侧。订单侧字段记录下单、核价或改价时形成的业务背景；包裹侧字段记录实际回款、来源实返货款、手续费和来源汇率等事实。返款账单以订单侧到付金额、到付附加费、货款币种配置和账单锁定汇率计算返款结果；包裹侧导入的实返货款和返款汇率仅用于来源追溯和人工核对，订单侧字段同时用于辅助核对、订单追溯和缺失数据排查。

| 来源字段 | 当前业务含义 | 返款账单用途 |
| :--- | :--- | :--- |
| `ofp_ofdb1.sale_order_header.collection_price` | 订单侧代收货款本金 | 用于核对客户下单或改价时维护的货款本金，不直接替代包裹侧实返货款。 |
| `ofp_ofdb1.sale_order_header.collection_premium_amount` | 订单侧代收货款手续费 | 仅作为手续费参考来源；若包裹侧已经导入同一手续费，不得重复扣减。 |
...
| `ofp_ofdb1.sale_order_package_fee.collection` | 包裹侧导入的来源实返货款 | 来源报表没有提供实返货款币种字段；仅用于追溯和人工核对，不作为`实付返款（准）`的计算输入。 |
| `ofp_ofdb1.sale_order_package_fee.recovery_money` | 包裹侧导入的实回货款 | 当前版本直接作为`实收回款`来源；回款返款模式下，用于判断是否已回款并核对回款金额。 |
| `ofp_ofdb1.sale_order_package_fee.receivable_collection_amount` | 包裹侧代收货款手续费 | 命中返款账单指定扣减规则时作为扣减费项；不得再在应收账单中作为可核销费用重复结算。 |
| `ofp_ofdb1.sale_order_package_fee.payment_collect where type = '1'` | 包裹侧导入的返款汇率 | 来源口径为`目的国币种 → 店铺币种`；仅用于追溯和人工核对，不作为货款换算汇率，账单生成时不得用它替代账单锁定汇率。 |
| `ofp_ofdb1.sale_order_package_fee.payment_collect where type = '2'` | 包裹侧导入的回款汇率 | 当前版本直接作为回款汇率来源；账单生成时随任务或账单快照固化，不由系统按金额关系自动生成或覆盖。 |
...
4. 同一笔手续费同时存在于订单侧和包裹侧时，只允许形成一笔返款扣减；系统应保留采用的来源字段并防止重复归集。
5. 包裹侧导入的`实返货款`、`实回货款`、`回款汇率`和`返款汇率`属于资金或换算事实，不属于客户应付费用，不得直接进入应收账单核销金额；BMS不得用计算值覆盖导入值。其中，`实回货款`和`回款汇率`继续用于回款事实；`实返货款`和`返款汇率`仅用于来源追溯和人工核对，不参与返款账单的结算币种、汇率或金额计算。
6. 返款账单详情应保留订单号、尾程运单号、来源字段、来源金额、来源币种（若来源提供）和最终账单计算金额，便于财务区分导入事实与账单结果。来源报表未提供实返货款币种时，应明确展示为“来源未提供”，不得根据返款汇率自动推断。
