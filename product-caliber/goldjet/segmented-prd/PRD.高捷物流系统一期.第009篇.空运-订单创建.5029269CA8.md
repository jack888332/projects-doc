...
- P1：航班确认与提单号绑定对应步骤 9、10 的同步处理。
- P2：步骤 14（填写分单信息）的进入关系待确认。
...
- P1：服务异常取消后，费用录入必做，重新勾选可选；未规定两者的先后或同步等待，因此分别展示。
...
本图终点仅表示本次航班差异检查与处理结束，不表示订单业务完成；原表未说明的后续节点不补接。订单状态、允许修改的字段和其他异常处理如下。
...
1. 客户信息：客户必须取自合作方档案中与高捷合作关系为“客户”的公司主体。支持通过“+”录入多个联系人邮箱，录入后同步维护到合作方档案的业务联系人信息。订单节点触发的邮件按[消息推送规则](PRD.高捷物流系统一期.第030篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-ef4930471a81)发送。客户授信额度低于0时禁止下单；低于20%时提示“此客户额度不足”，点击提示可查看详情。客户信息填写错误时，在费用录入页面修改客户名称并备注理由；财务已审核的，按调账流程处理。
...
空运订单与内部供应商进行数据交互；字段定义见 [数据字典 · 主订单创建内部供应商接口字段](PRD.高捷物流系统一期.第034篇.数据字典.7A3D9E1F50.md#doc-7A3D9E1F50-mappings-mapping-internal-supplier-order)。
...
3. 发送：客服可选择入仓图、航班信息及收件人后发送；默认不选择发送内容。收件人取合作方档案中的业务联系人。通知渠道、接收对象、附件取值与消息内容见[消息推送规则](PRD.高捷物流系统一期.第030篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-ef4930471a81)。
...
客服只能查看本人创建的订单。需要转交他人跟进时，勾选订单并点击“客服派单”，从用户中心选择接单客服，填写不超过500字的备注并确认。接单客服可接受或拒绝：接受后，订单加入其所属列表；拒绝时必须填写备注。派单及拒绝结果通知按[消息推送规则](PRD.高捷物流系统一期.第030篇.消息推送.5F950AB3B1.md#doc-5F950AB3B1-section-ef4930471a81)发送。
