# 2026-08-11 BMS 对账报表导出增加尾程包裹数据行技术调整方案与计划

**日期：** 2026-08-11
**需求依据：** `aidocs` 提交 `43aba95028864bc8a9458a6fd564233512764d81`（对账报表导出文件增加尾程包裹数据行）
...
**文档状态：** 技术方案待评审；实施前需先确认 §8 疑问点
**方案性质：** 只调整对账报表导出的行粒度、数据组织和 Excel 表现，不改变账单生成、金额计算、汇率、审核、核销、返款、调账及当前有效账单结果口径。
...
1. 账单生成、费项入池、结果版本、当前有效账单、汇总金额和核销口径。
2. 返款账单只提供客户导出，不新增内部导出。
3. 导出任务创建、异步执行、进度、失败重试、文件替换、失效和下载鉴权机制。
4. 历史导出任务及其已固化快照不追溯重建。
...
6. 历史任务旧快照可正常下载，不抛错。
7. 新任务重试失败项后，重新生成文件且内容一致。
8. 相关代码通过 `mvn -f bms/pom.xml -pl web -am test-compile` 编译校验（本项目无现成 lint/单测入口，按 BMS 规范做编译与手工回归）。
...
PRD 统一明细字段表没有新增“包裹标识”固定列，但 19.6.1.4.1 明确要求包裹行展示“包裹标识”。是否将 CXMS 包裹号写入现有`子运单号`列，还是新增一列？
...
`业务订单号>` 是否固定无空格格式？Excel 行分组默认展开还是折叠？方案默认展开 + `>`标记。
