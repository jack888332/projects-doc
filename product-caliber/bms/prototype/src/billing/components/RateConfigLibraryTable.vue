...
<template>
  <el-table :data="props.rows" class="clean-table rate-table" border height="100%" row-key="id">
    <el-table-column prop="no" label="配置编号" min-width="230" show-overflow-tooltip />
    ...
    <el-table-column label="当前规则摘要 / 默认汇率预览" min-width="265"><template #default="scope"><div class="rule-summary" :title="scope.row.ruleSummary"><div v-for="(line, index) in String(scope.row.ruleSummary || '--').split('；')" :key="index">{{ line }}</div></div></template></el-table-column>
