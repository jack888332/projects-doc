...
export function buildConfigGenerationScopes({ config, references, tasks, cutoff, referenceAt = '2026-08-27 16:45:00', customerCode = '' }) {
  ...
  const rows = exactReferences.flatMap(reference => schemesFor(config).map((scheme) => {
    ...
    return {
      ...
      memberCode: reference.memberCode,
      store: reference.store,
      group: reference.group,
      memberCodes:relationSummary.memberCodes,
      stores:relationSummary.stores,
      groups:relationSummary.groups,
      customerRelations:relationSummary.relations,
...
export function createConfigBatchTaskRows({ config, scopes, skippedCount, frozenCustomerCount, cutoff, existingTasks }) {
  ...
  const rows = scopes.map((scope, index) => ({
    ...
    memberCodes:scope.memberCodes || [scope.memberCode].filter(Boolean),
    shops:scope.stores || [scope.store].filter(Boolean),
    customerGroups:scope.groups || [scope.group].filter(Boolean),
    customerRelations:scope.customerRelations || [],
    sourceShopSnapshots:[],
