...
客户级账单配置、客户级汇率配置、费项归集、对账报表和核销均沿用本节定义的对象边界。核心对象关系由下方概览和对象说明共同定义。

<a id="doc-A231A39E11-business-baseline-cced28d2"></a>
#### 1.1.2 供应链、店铺、客户与集运单的层级
...
| 客户组 | 在店铺内创建的客户 / 会员归集分组，即集运客门户中的子账号分组。一个店铺可创建多个客户组，一个客户组可包含多个会员；一个会员在同一时点至多从属其所属店铺下的一个客户组，不得加入其他店铺的客户组。 | 客户组用于查询、报表归集和配置分配向导选客。BMS 只读取订单及客户主数据已沉淀的客户组，不重新划分客户组，也不按客户组动态决定配置引用。 |
| 客户 / 会员 | 同一业务主体在不同系统语境下的称谓。WMS 从货主关系称为客户或货主，使用客户编码；集运客门户从账号关系称为会员，使用会员编码。客户主数据与会员身份一一对应，一个会员在同一时点只能从属一个店铺。 | 客户 / 会员是订单下单、财务对账、合同和配置引用的共同主体。BMS 同时保留客户编码和会员编码，用于跨系统定位同一主体；不得把会员当作客户下的另一层多账号对象。客户编码由系统自动生成，固定采用`OG`加 4 位数字的格式（如`OG4155`），文档示例统一使用该格式。 |
...
| 集运单 | 会员或客户在店铺下形成的核心业务单据，承接包裹、仓库、线路、承运商、费用和履约节点。 | 是 BMS 账单生成的核心执行对象。应收账单、返款账单和对账报表都围绕集运单及其费用结果展开。 |

<a id="doc-A231A39E11-business-baseline-9a85a108"></a>
#### 1.1.4 OIS 对象关系约束

BMS 必须遵守以下 OIS 业务对象约束：
...
8. 一个店铺对应一个集运客门户二级域名和主账号，一个会员对应一个集运客门户账号。会员只能在其唯一所属店铺中使用，并且不得加入其他店铺创建的客户组；会员转店或改组由 OIS 管理，BMS 不反向维护该归属。
...
1. 若费项已通过预存款扣款或收方到付完成支付，不得再次作为寄方应收金额进入应收账单。
...
订单完成出库前属于费项重算窗口，费项金额尚未稳定，BMS 不得把该阶段的金额视为最终出账依据。订单完成出库后，费项横表进入 BMS 同步；同步成功后固化为账单生成、冲正和追溯的原始口径。

<a id="doc-A231A39E11-business-baseline-81c08953"></a>
#### 1.3.2 费项出库同步与后续变更规则

业务订单从首次计费到二次计费完成存在多个状态，计费状态由 OIS 字段 `sale_order_header.calc_fee_status` 标识。对于费项横表而言，业务订单完成出库前，所有机算费项都可能因为包裹回传、支付变化或补录动作再次发生计算，因此这一阶段的费项金额并不稳定。

`BMS费项池`在业务订单完成出库时同步费项横表，避免入池后原始金额继续变化。出库前允许二次计费；出库后进入 BMS 同步和后续账单流程。费项横表成功同步后不得修改、补写或整行重采；后续新发生的费用必须通过新来源记录或调账记录进入 BMS。
...
上图说明订单从首次计费到二次计费完成的状态流转。只要订单仍可能从 `PAID`、`CALCED` 或 `MORE_CALCED` 回到 `MORE_READY`，费用就仍可能被重新计算或补充，当前费项金额不能视为最终稳定结果。

BMS 判断同步时点时，同时采用本节的重算窗口和[客户侧费项的 OIS 数据源](PRD.账单系统.第021篇.OIS客户侧费项数据.A6A29BE637.md#doc-A6A29BE637-ois-fee-data-ad5529f1)中的字段产生时机，不得提前采集仍可能变化的费项金额。
...
OIS 根据订单下尾程包裹的 TMS 物流末条轨迹判断订单是否命中广义签收状态，并由定时任务维护订单完结时间。若订单只有一个尾程包裹，该包裹命中广义签收后，以其末条轨迹时间作为订单完结时间。若订单有多个尾程包裹，必须全部尾程包裹均命中广义签收，订单才算完结，并以满足条件时的最新末条轨迹时间作为订单完结时间。

订单完结时间记录在`ofp_ofdb1.sale_order_header_extend.order_completed_time`，`ofp_ofdb1.sale_order_header`主表不包含该字段。该字段用于 BMS 归集的口径见[BMS 归集费项时使用哪些非金额字段？](PRD.账单系统.第021篇.OIS客户侧费项数据.A6A29BE637.md#doc-A6A29BE637-ois-fee-data-cce16964)。

广义签收是判断`订单完结`的业务口径，不是新的物流状态，也不等同于狭义的“正常签收”。当订单下全部尾程包裹的末条轨迹均属于下表状态时，该订单达到广义签收口径，可以进入应收账单归集。
