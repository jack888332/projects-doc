...
3. 二维表表头按 Excel 原始行号范围定义，支持多行及合并单元格表头；字段名按范围内各级标题组合，实际数据从表头结束行的下一行开始，数据结束行按最后一条数据的原始行号定义。
...
1. 详情页按“基本信息 + 成本费项汇总/成本费项明细页签”布局。
2. 汇总卡片展示成本明细数、标准成本费项数、币种数和原始工作表数。
3. 汇总表按标准成本费项及币种聚合，展示原始费项名称、明细数量、原始金额和生效金额。
4. 支持从详情页下载供应商原始账单。
...
2. 二维表映射调整为“表头起止行/数据结束行 + 至多一个关键单号列 + 多个成本金额列”，支持合并单元格和多行组合表头，移除错误的“费项名称列”模型。
