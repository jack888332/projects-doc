...
| 部分结清 / 差额 | 客户部分付款、财务向客户部分返款，或回款 / 返款金额与账单金额不一致 | 登记资金结果并进入核销流程。 | 后续资金核销流程 |
...
系统按以下顺序处理每个正常签收的 COD 包裹：

| 处理步骤 | 处理口径 | 不满足时的处理 |
| :--- | :--- | :--- |
| 1. 匹配原始币种 | 先按`应付返款`的真实原始币种匹配明确的`货款原始币种`规则；没有明确规则时，才匹配`其他`兜底规则。`其他`不是可参与换算的真实币种。 | 没有匹配规则时，受影响包裹不得进入本次返款账单。 |
...
| 4. 计算扣减前返款 | `实付返款（准） = 应付返款（原始币种） × 货款换算汇率`。 | 缺少应付返款本金时，受影响包裹不得进入本次返款账单。 |
| 5. 校验收款账户 | 客户收款账户币种必须等于`货款结算币种`。 | 未配置同币种客户收款账户时，配置不得保存，受影响账单不得生成。 |

配置保存时，系统必须阻止同一配置版本内相同`货款原始币种`存在多个不同的`货款结算币种`规则；否则同一包裹会得到多个结算结果。`其他`只负责兜底匹配未单独配置的原始币种，计算时仍须使用包裹真实的原始币种取得汇率。例如，配置为`其他 → USD`时，TWD 包裹应使用`TWD → USD`的账单锁定汇率，不能把`其他`当作货币，也不能使用导入的`TWD → CNY`返款汇率。

同一批包裹中，导入`返款汇率`或`实返货款`是否有值，不影响货款币种规则的匹配和返款账单计算。来源值与账单计算值不一致时，不得因此阻止账单生成、替换或审核；系统应在账单明细中分别展示来源值和账单计算值，供财务人工核对。
...
返款账单模块用于把正常签收 COD 包裹的结算事实，整理为客户可核对、财务可审核、可执行返款和可追溯结清的账单结果。模块的核心结果不是“把到付总额原样返给客户”，而是依次确定到付总额、到付附加费、应付返款本金、汇兑后的扣减前返款金额、返款账单指定扣减费和最终实付返款。
...
| `应付返款（即代收货款）` | 从到付总额中扣除到付附加费后的代收货款本金 | 该金额以货款原始币种记录，是计算`实付返款（准）`的原始金额。 |
| `实收回款` | 订单费项报表导入的包裹侧`实回货款` | 表示尾程派送商实际返给财务的金额；当前版本直接读取导入值，不由系统按公式生成。 |
...
2. 只有正常签收的 COD 包裹才进入返款账单金额计算；异常签收包裹不产生回款金额、返款金额或返款扣减结果，并由<a href="#section-12">12. 回款管理</a>保留状态追踪。
...
1. 订单侧金额用于识别下单、核价或改价时记录的到付金额、代收货款等业务背景。
...
6. 返款账单明细必须保留金额来源和挂靠包裹，并明确区分订单侧参考金额、包裹侧导入事实和本账单计算结果。
