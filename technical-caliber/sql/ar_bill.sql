...
CREATE TABLE `ar_bill` (
  ...
  `active_bill_guard` tinyint(1) GENERATED ALWAYS AS ((case when (`is_deleted` = 0) then 1 else NULL end)) STORED COMMENT '未删除唯一约束辅助列：未删除时为1，已删除时为NULL',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ar_bill_no` (`bill_no`),
  UNIQUE KEY `uk_ar_bill_period_sector_country` (`bill_type`,`bill_config_id`,`billing_period_start_date`,`billing_period_end_date`,`business_sector`,`destination_country`,`active_bill_guard`),
...
CREATE TABLE `bill_exchange_rate` (
  ...
  `currency_pair_key` varchar(40) GENERATED ALWAYS AS ((case when `bill_currency` = `conversion_currency` then NULL else concat(least(`bill_currency`,`conversion_currency`),'|',greatest(`bill_currency`,`conversion_currency`)) end)) STORED COMMENT '无方向货币对唯一键，同币种不参与唯一约束',
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_bill_rate_pair` (`bill_type`,`bill_id`,`currency_pair_key`),
...
CREATE TABLE `business_type_fee_index` (
  ...
  `fee_type` varchar(16) NOT NULL COMMENT '费用类型：AR应收，ARD应收扣减，AP成本，AR_COLLECT代收，AR_PAY代付，ARAP待迁移旧值，NON_FEE非费项',
...
-- ----------------------------
-- Table structure for bill_order_waybill_snapshot
-- ----------------------------
DROP TABLE IF EXISTS `bill_order_waybill_snapshot`;
...
CREATE TABLE `bill_export_task` (
  ...
  `data_frozen_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '数据冻结完成时间',
...
CREATE TABLE `bill_export_task_item` (
  ...
  `bill_no` varchar(64) NOT NULL COMMENT '账单编号',
  `item_seq` int NOT NULL DEFAULT '0' COMMENT '用户提交顺序',
  ...
  `result_name` varchar(255) DEFAULT NULL COMMENT '客户文件名或内部Sheet名',
