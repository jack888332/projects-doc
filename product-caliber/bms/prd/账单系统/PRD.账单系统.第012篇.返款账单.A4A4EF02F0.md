...
因此，上游来源数据必须先在目的国币种中形成订单级`应付返款`。返款账单直接读取该结果，再扣除明确指定的扣减费项，得到换算前的`实付返款（准）`；最后按[返款账单怎样确定结算币种和金额？](PRD.账单系统.第005篇.客户级账单配置.B6349C60A4.md#doc-B6349C60A4-billing-config-e5cbf17a)锁定返款汇率，并换算为客户实际收款的货款结算币种。
...
| `实付返款` | 按返款汇率换算后的最终返款金额 | `实付返款 = 实付返款（准） × 返款汇率`；以货款结算币种记录，是账单级返款支付与核销的目标金额。 |
| `已付返款` | 财务已实际支付给客户并完成返款核销的金额 | 以货款结算币种记录，只由账单侧返款支付与核销事实形成，不由包裹回款记录推导。 |
| `待付返款` | 实付返款中尚未完成支付与核销的金额 | `待付返款 = 实付返款 - 已付返款`；以货款结算币种记录。 |
...
2. 返款账单按 COD 业务订单归集`应付返款`和`代收货款手续费`；尾程包裹只提供签收和`实收回款`事实，回款记录在返款账单详情的`汇兑损益`中呈现。回款记录只用于判断`回款返款`模式的归集条件以及财务跟踪和汇兑损益计算，不影响返款金额、扣减核销或账单结清判断。包裹回款记录属于来源侧资金事实，直接挂靠尾程包裹，不属于核销、返款登记或收款等账单侧资金事实，分类见[资金事实分为哪两类？](PRD.账单系统.第010篇.账单生成重算与变更分流.4CA3C606DB.md#doc-4CA3C606DB-billing-calculation-funds-fact-classification)。返款账单替换生成或提前收口时，回款记录不作为阻断条件，但必须保持与来源导入记录、尾程包裹和业务订单的映射并只读呈现，不复制、不删除、不修改：替换成功后新账单继续引用同一批回款记录，被移出账单范围的记录不再随新账单展示，提前收口截断后晚到的回款只更新跟踪展示，不改变账单结果。异常签收包裹不形成自身回款事实，在`汇兑损益`中标注异常原因作跟踪；业务订单全部包裹均异常签收时整单不进入返款计算，一票多件部分异常时整票仍按订单级金额返款。
...
1. 订单级`应付返款`在来源数据准备阶段按`cod_price - dest_country_surcharge_amount`形成并直接挂靠业务订单；返款账单读取该结果并按业务订单归集，不读取或展示`cod_price`与`dest_country_surcharge_amount`。`代收货款手续费`由 OIS 机算形成并直接挂靠业务订单，BMS 只读取结果并按业务订单归集。订单侧其他名称相近的金额字段（例如`collection_price`）不作为应付返款计算输入。
...
| 到付附加费，例如`超材费（到付）` | 不作为客户账期应收费重复核销 | 不单独归集或展示；上游扣除后仅向返款账单提供`应付返款` | 该费用已随 COD 向收件人收取，用于确定代收货款本金，不属于返款账单指定扣减费项。 |
...
2. 到付附加费和返款账单指定扣减费项是两个不同层级。到付附加费在上游来源数据中决定`应付返款`，返款账单不单独归集或展示；指定扣减费项再换算到同一货款原始币种后影响`实付返款（准）`，详见[应付返款和实际返款如何区分？](#doc-A4A4EF02F0-refund-bill-a48f08d4)。
...
上述来源计算中，到付金额、到付附加费总额与应付返款（即代收货款）的原始币种始终一致，均为该业务订单的目的国币种；账单计算所称`货款原始币种`即该实际币种，不存在三笔金额分属不同币种的情形。
...
1. `应付返款（即代收货款） = 应收回款（即到付金额） - 到付附加费总额`（对应 OIS 订单侧 `cod_price - dest_country_surcharge_amount`），两个示例均为 `70 TWD - 10 TWD = 60 TWD`。该关系用于证明`应付返款`的来源，返款账单明细直接读取计算结果。
...
当业务订单按`实付返款（准）= 应付返款 - 返款账单指定扣减费项`计算，且金额可计算的结果小于零时，说明该订单的`应付返款`不足以覆盖本次指定的扣减费项。负数只在货款原始币种中产生；系统不得把负数换算为`实付返款`，也不得生成负数待付返款、负数返款核销记录或负数账单汇总。`cod_price`、`dest_country_surcharge_amount`等上游计算输入缺失或不可计算不属于本节范围，相关订单不得进入本期返款账单，按[返款金额从哪里取？](#doc-A4A4EF02F0-refund-bill-3b9f382c)的来源输入异常处理。
...
| 读取金额及来源事实 | 按业务订单读取已经形成的`应付返款`、代收货款手续费和指定扣减费项；同时读取各尾程包裹的实收回款和回款汇率，具体规则见[返款金额从哪里取？](#doc-A4A4EF02F0-refund-bill-3b9f382c)。返款账单不读取或展示到付金额、到付附加费总额。 | 得到订单级返款输入和可追溯的包裹级回款事实。 |
...
| 汇总金额分桶 | 必选条件`汇总金额币种`；币种、应付返款、扣减费项、实付返款、已付返款、待付返款；核销 | `汇总金额币种`不可清空，枚举为`返款结算币`、`财务本位币`，默认`返款结算币`，切换后立即刷新分桶金额。选择`返款结算币`时，一张账单包含几个实际货款结算币种就同时展示几个分桶，同一货款结算币种的数据合并到一个分桶，不同币种不得直接相加；分桶头部不展示核销状态，`核销`按钮位于对应分桶右上角并随分桶移动，币种标识与按钮等高。选择`财务本位币`时，先按各分桶锁定汇率换算后聚合为一个财务本位币分桶，不展示核销按钮，也不替代返款结算币的核销结果。每个分桶占详情内容区四分之一栅格宽；可用宽度不足时按整块换行，不压缩字段或金额。 |
| 账单汇率 | 左侧币种、右侧币种、汇兑方向、汇率来源、锁定汇率、汇率状态 | 读取账单锁定快照，不随后续配置或市场汇率变化。 |
| 返款明细（订单返款数据行） | 业务订单号、应付返款（即代收货款）、各指定扣减费项名称及扣减金额、实付返款（准）、实付返款 | 按业务订单逐行展示从`应付返款`开始的订单级金额链路；本账单实际占用的每个指定扣减费项都以独立金额列直接列在订单行中，没有命中时对应列显示零金额。订单行不展示包裹数、来源金额、到付金额、到付附加费总额、返款汇率、已付返款、待付返款或核销状态，也不提供包裹回款展开行；返款汇率统一在`账单汇率`展示，返款支付与核销进度统一在汇总金额分桶展示。 |
...
5. 返款账单列表的金额列名称为`返款结算币金额`。该列按实际货款结算币种分别展示币种、`应付返款`、`扣减费项`、`实付返款`、`已付返款`和`待付返款`，字段名称和顺序必须与详情页汇总金额分桶一致；一张账单存在多个货款结算币种时，同一单元格必须展示全部币种分桶，不得只展示默认币种或跨币种汇总金额。列表不展示负数承接记录或负数承接冲抵记录，相关事实统一在账单详情的`调整记录`中展示。
