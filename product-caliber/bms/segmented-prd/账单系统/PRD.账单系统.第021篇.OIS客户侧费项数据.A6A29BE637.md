...
6. 订单侧`collection_price`（代收货款）保留用于核对；来源数据准备阶段按`cod_price - dest_country_surcharge_amount`计算订单级`应付返款`，返款账单直接读取该结果，不展示或重新计算前述两个来源字段。`collection_price`与计算结果取值不一致时以订单级`应付返款`计算规则为准。`cod_price`、`dest_country_surcharge_amount`和`实收回款`属于非费项字段，其中前两者是订单侧应付返款计算输入，`实收回款`是包裹侧导入核对事实。`dest_country_surcharge_amount`不进入客户应收账单。费项类型和账单用途见[费项索引怎样定义费项类型？](PRD.账单系统.第009篇.来源范围与费项池.C986F40642.md#doc-C986F40642-fee-source-798b26a1)。
...
9. 订单费用报表导入的普通费项金额都必须确定原始币种；其中`仓租费`和`航空费`固定为人民币，其他普通费项金额采用目的国币种。后续订单费用报表只导入`实收回款`和`回款汇率`，不再导入`实付返款`和`返款汇率`。系统不再依赖`ofp_ofdb1.sale_order_package_fee.type`区分返款汇率和回款汇率，`payment_collect`当前导入值统一按回款汇率处理。当前回款汇率表示`目的国币种 → 财务本位币（CNY）`；返款账单所用返款汇率由返款原始币种和返款结算币种确定，见[返款账单换算总则](PRD.账单系统.第014篇.金额汇兑.27B80EF314.md#doc-27B80EF314-currency-exchange-83e6bbc9)。
...
返款相关金额同时存在于 OIS 输出的订单级机算字段和订单费用报表导入形成的包裹级金额中。来源数据准备阶段以直接挂靠业务订单的`cod_price`（到付金额）与`dest_country_surcharge_amount`（到付附加费总额）计算订单级`应付返款（即代收货款）= cod_price - dest_country_surcharge_amount`，并把`应付返款`作为返款账单的直接金额输入；返款账单不展示或重新计算前述两个来源字段。`collection_price`等订单侧字段保留用于核对，不作为应付返款计算输入。`ofp_ofdb1.sale_order_package_fee`中的所有金额均为尾程包裹级导入值，不是 OIS 机算值。包裹侧只导入新的实收回款、回款汇率和包裹级手续费等事实；历史实付返款和返款汇率只保留存量原记录。

| 来源字段 | 当前业务含义 | 返款账单用途 |
| :--- | :--- | :--- |
| `ofp_ofdb1.sale_order_header.collection_price` | 订单侧代收货款本金 | 挂靠业务订单并保留订单侧代收货款值用于核对；不作为订单级`应付返款`的计算输入。 |
| `ofp_ofdb1.sale_order_header.collection_premium_amount` | 订单侧代收货款手续费 | 用于核对业务订单级手续费结果；命中返款扣减配置时按业务订单计入一次。 |
| `ofp_ofdb1.sale_order_header.cod_price` | 收件人侧 COD 到付总额 | 在来源数据准备阶段与`dest_country_surcharge_amount`相减得到`应付返款（即代收货款）`；不作为返款账单明细或导出字段。 |
| `ofp_ofdb1.sale_order_header.cod_amount` | 历史字段中文名为`货到付款手续费` | 不得仅凭字段名直接认定为手续费；必须结合具体订单和包裹侧事实判断。 |
| `ofp_ofdb1.sale_order_header.dest_country_surcharge_amount` | 订单侧到付附加费总额 | 在来源数据准备阶段从`cod_price`中扣除以确定`应付返款（即代收货款）`；不作为返款账单明细或导出字段，也不作为客户应收费用进入应收账单。 |
...
1. 到付总额、代收货款本金、实际回款和实际返款必须按各自业务含义分开保存，不得因数值相同而合并为一个金额口径；返款账单只展示订单级`应付返款`及后续扣减、换算结果，不展示到付总额或到付附加费总额。
...
6. 返款账单详情的订单返款数据行须保留业务订单号、`应付返款`、返款原始币种、各指定扣减费项和最终账单计算金额，不展示尾程运单号、来源金额、到付金额或到付附加费总额。包裹级回款记录作为订单费用报表导入的核对事实，在返款账单详情的`汇兑损益`页签中按包裹逐行展示，展示规则见[COD 订单回款记录与汇兑损益怎样展示？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-remittance-712a4d3e)；本节只定义来源字段含义，不重复定义展示区域字段。
7. 订单级`应付返款`一律在来源数据准备阶段按`cod_price - dest_country_surcharge_amount`计算并提供给返款账单；`collection_price`只保留用于核对，不得作为应付返款计算输入或覆盖公式结果。系统不对`collection_price`与公式结果的取值差异进行异常提示或自动处置，默认各来源字段本身为正确的订单侧业务事实，差异仅保留用于核对。
8. COD 业务订单的返款计算以业务订单为单位：`cod_price`、`dest_country_surcharge_amount`等上游金额不按尾程包裹拆分；一票多件中部分包裹异常签收时整票仍按订单级金额计算，业务订单全部尾程包裹均异常签收时整单不进入返款计算，口径见[核心概念](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-e15cb291)。上游无法形成有效`应付返款`时，该订单不进入本期返款账单，按[返款金额从哪里取？](PRD.账单系统.第012篇.返款账单.A4A4EF02F0.md#doc-A4A4EF02F0-refund-bill-3b9f382c)处理。
